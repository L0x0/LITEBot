...
bot_vblko* bot_pkto_lst[BOT_SCKT_MAX] = { bot_pkto[0].nxt, bot_pkto[1].nxt, bot_pkto[2].nxt, bot_pkto[3].nxt, bot_pkto[4].nxt };

// MTX_SVPKI
bot_vblki bot_pkti[BOT_SCKT_MAX];
bot_vblki* bot_pkti_lst[BOT_SCKT_MAX] = { bot_pkti[0].nxt, bot_pkti[1].nxt, bot_pkti[2].nxt, bot_pkti[3].nxt, bot_pkti[4].nxt };

// MTX_SVPKO
bot_svblko bot_spkto[BOT_SCKT_MAX];
bot_svblko* bot_spkto_lst[BOT_SCKT_MAX] = { bot_spkto[0].nxt, bot_spkto[1].nxt, bot_spkto[2].nxt, bot_spkto[3].nxt, bot_spkto[4].nxt };

// MTX_SVPKI
bot_svblki bot_spkti[BOT_SCKT_MAX];
bot_svblki* bot_spkti_lst[BOT_SCKT_MAX] = { bot_spkti[0].nxt, bot_spkti[1].nxt, bot_spkti[2].nxt, bot_spkti[3].nxt, bot_spkti[4].nxt };
...
sint machine::LITEBot(carr_256 *ncar_, sint argc, _char* argv[])
{
	...
	if (argc > 1)
	{
		ullint inv = SOCK_STREAM;

		if (!strcmp(UCASE(argv[1]).c_str(), "TCP"))
		{
			inv = SOCK_STREAM;
		}
		else if (!strcmp(UCASE(argv[1]).c_str(), "UDP"))
		{
			inv = SOCK_DGRAM;
		}
		else if (!strcmp(UCASE(argv[1]).c_str(), "RAW"))
		{
			inv = SOCK_RAW;
		}
		else if (!strcmp(UCASE(argv[1]).c_str(), "PGM"))
		{
			inv = SOCK_RDM;
		}
		else {}
...
sint machine::GetInVec(void* val_, sint g_opt, bool cine, bool is_meta)
{
	...
	case MTX_CV:
	{
		...
		else
		{
			...
			{
				if (!strcmp(np.name.c_str(), vp->name.c_str()))
				{
					vp->Renew(&np);
					return siz;
				}
	...
	if (xc < 0)
	{
		if (!strcmp(UCASE(typ).c_str(), "INT"))
		{
			if (val)
			{
				...
				xc = mv.AddVar(nm, &var, BOT_RTV_SLLINT, BOT_RTV_SLLINT);
			}
			else
			{
				sllint var = 0;
		...
		else if (!strcmp(UCASE(typ).c_str(), "REAL"))
		{
			if (val)
			{
				...
				xc = mv.AddVar(nm, &var, BOT_RTV_REAL, BOT_RTV_REAL);
			}
			else
			{
				float var = 0.0f;
		...
		else if (!strcmp(UCASE(typ).c_str(), "TEXT"))
		{
			...
			{
				std::string var(val);
				xc = mv.AddVar(nm, &var, BOT_RTV_STR, BOT_RTV_STR);
			}
			else
			{
				std::string var;
		...
		else if (!strcmp(UCASE(typ).c_str(), "BLOB"))
		{
			if (val)
			{
				...
				if (var)
				{
					...
					xc = mv.AddVar(nm, var, BOT_RTV_BLOB, BOT_RTV_BLOB);
					free(var);
				}
			}
			else
		...
		else if (!strcmp(UCASE(typ).c_str(), "VINT"))
		{
			...
			{
				size_t vl = bot_cstrlen(val);
				xc = ArgSep(&vec, &mv.vars, false, 0, vl, val, BOT_RTV_CHAR, ' ');
				xc = mv.AddVar(nm, &vec, BOT_RTV_VSLLINT, BOT_RTV_VARG);
			}
			else
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VSLLINT, BOT_RTV_SLLINT);
			}
		}
		else if (!strcmp(UCASE(typ).c_str(), "VREAL"))
		{
			...
			if (val)
			{
				size_t vl = bot_cstrlen(val);
				xc = ArgSep(&vec, &mv.vars, false, 0, vl, val, BOT_RTV_CHAR, ' ');
				xc = mv.AddVar(nm, &vec, BOT_RTV_VREAL, BOT_RTV_VARG);
			}
			else
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VREAL, BOT_RTV_REAL);
			}
		}
		else if (!strcmp(UCASE(typ).c_str(), "VTEXT"))
		{
			...
			if (val)
			{
				size_t vl = bot_cstrlen(val);
				xc = ArgSep(&vec, &mv.vars, false, 0, vl, val, BOT_RTV_CHAR, ' ');
				xc = mv.AddVar(nm, &vec, BOT_RTV_VSTR, BOT_RTV_VARG);
			}
			else
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VSTR, BOT_RTV_VARG);
		...
		else if (!strcmp(UCASE(typ).c_str(), "VBLOB"))
		{
			...
			if (val)
			{
				size_t vl = bot_cstrlen(val);
				xc = ArgSep(&vec, &mv.vars, false, 0, vl, val, BOT_RTV_CHAR, ' ');
				xc = mv.AddVar(nm, &vec, BOT_RTV_VBLOB, BOT_RTV_VARG);
			}
			else
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VBLOB, BOT_RTV_VBLOB);
			}
		}
		else {}
	}
	...
	if (xc < 0)
	{
		...
		else if (typ == BOT_RTV_VREAL)
		{
			...
			if (val)
			{
				size_t vl = bot_cstrlen(val);
				xc = ArgSep(&vec, &mv.vars, false, 0, vl, val, BOT_RTV_CHAR, ' ');
				xc = mv.AddVar(nm, &vec, BOT_RTV_VREAL, BOT_RTV_VREAL);
			}
			else
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VREAL, BOT_RTV_REAL);
		...
		else if (typ == BOT_RTV_VSTR)
		{
			...
			{
				xc = mv.AddVar(nm, &vec, BOT_RTV_VSTR, BOT_RTV_VSTR);
			}
		}
...
sint machine::PVAR(c_char* nm, ...)
{
	...
	if (xc > -1)
	{
		...
		for (size_t siz = 0; siz < outs.size(); siz++)
		{
			...
			{
				std::string xcar("INT ");
				xcar.append(mv.vars[xc].n);

				if (!mv.vars[xc].v.empty())
				{
					if (!mv.vars[xc].v[0].empty())
					{
						...
						if (nv)
						{
							...
							{
								memcpy((void*)&nv[z], (void*)&mv.vars[xc].v[0][z], sizeof(_char));
							}
				...
				sint xp = Output(true, xcar.c_str(), outs[siz], -1);
				break;
			}
			case BOT_RTV_REAL:
			{
			...
			case BOT_RTV_STR:
			{
				...
				if (!mv.vars[xc].v.empty())
				{
					if (!mv.vars[xc].v[0].empty())
					{
						...
						for (size_t z = 0; z < mv.vars[xc].v[0].size(); z++)
						{
							xcar.push_back(mv.vars[xc].v[0][z]);
						}
					}
				}
				sint xp = Output(true, xcar.c_str(), outs[siz], -1);
			...
			case BOT_RTV_BLOB:
			{
				...
				if (!mv.vars[xc].v.empty())
				{
					if (!mv.vars[xc].v[0].empty())
					{
						...
						for (size_t z = 0; z < mv.vars[xc].v[0].size(); z++)
						{
							sint xp = bot_sprintfs(&xcar, false, "0x%02X ", mv.vars[xc].v[0][z]);
						}
						xcar.pop_back();
					}
				}
			...
			case BOT_RTV_VSLLINT:
			{
				...
				if (!mv.vars[xc].v.empty())
				{
					...
					if (np)
					{
						...
						for (size_t z = 0; z < mv.vars[xc].v.size(); z++)
						{
							sint xp = bot_sprintfs(&xcar, false, "[%u] ", z);

							for (size_t w = 0; w < mv.vars[xc].v[z].size(); w++)
							{
								memcpy((void*)&np[w], (void*)&mv.vars[xc].v[z][w], sizeof(_char));
							}
				...
				if (!mv.vars[xc].v.empty())
				{
					...
					if (nv)
					{
						for (size_t z = 0; z < mv.vars[xc].v.size(); z++)
						{
							...
							{
								memcpy((void*)&nv[w], (void*)&mv.vars[xc].v[z][w], sizeof(_char));
							}
							float val;
							memcpy((void*)&val, (void*)nv, sizeof(float));
			...
			case BOT_RTV_VSTR:
			{
				...
				if (!mv.vars[xc].v.empty())
				{
					...
					for (size_t z = 0; z < mv.vars[xc].v.size(); z++)
					{
						...
						for (size_t w = 0; w < mv.vars[xc].v[z].size(); w++)
						{
							xcar.push_back(mv.vars[xc].v[z][w]);
						}
						xcar.append(",\n\t");
					}
					xcar.pop_back();
...
sint machine::GVAR(c_char* nm, sint as_t, void* val)
{
	...
	return mv.GetVar(nm, as_t, val);
}
sint machine::SVAR(c_char* nm, sllint* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), sllint*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_SLLINT, true);
}
sint machine::SVAR(c_char* nm, float* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), float*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_REAL, true);
}
sint machine::SVAR(c_char* nm, std::string* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::string*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_STR, true);
}
sint machine::SVAR(c_char* nm, BOT_ARG* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), BOT_ARG*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, &val->arg, BOT_RTV_MAX, BOT_RTV_STR, true);
}
sint machine::SVAR(c_char* nm, _char* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), _char*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_BLOB, true);
}
sint machine::SVAR(c_char* nm, std::vector<sllint>* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::vector<sllint>*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_VSLLINT, true);
}
sint machine::SVAR(c_char* nm, std::vector<float>* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::vector<float>*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_VREAL, true);
}
sint machine::SVAR(c_char* nm, std::vector<std::string>* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::vector<std::string>*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_VSTR, true);
}
sint machine::SVAR(c_char* nm, std::vector<std::vector<_char>>* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::vector<std::vector<_char>>*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_VBLOB, true);
}
sint machine::SVAR(c_char* nm, std::vector<BOT_ARG>* val)
{
	if (debug_lvl >= 1000 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::SVAR(c_char* nm(%i), std::vector<BOT_ARG>*(%i))", (sint)nm, (sint)val);
		op = Output(outp.carr, 2);
	}
	if (!nm || !val)
	{
	...
	return mv.AddVar(nm, val, BOT_RTV_MAX, BOT_RTV_VARG, true);
}
...
sint machine::LVARS(sint n, ...)
{
	...
	for (size_t siz = 0; siz < outs.size(); siz++)
	{
		for (size_t x = 0; x < mv.vars.size(); x++)
		{
			std::string str;
			...
			case BOT_RTV_SLLINT:
			{
				...
				if (mv.vars[x].n)
				{
					str.append(mv.vars[x].n);
				}
				if (!mv.vars[x].v.empty())
				{
					_char* nv = (_char*)malloc(sizeof(sllint));
					...
					if (nv)
					{
						for (size_t z = 0; z < sizeof(sllint); z++)
						{
							memcpy((void*)&nv[z], (void*)&mv.vars[x].v[0][z], sizeof(_char));
						}
						sllint val;
						memcpy((void*)&val, (void*)nv, sizeof(sllint));
						sint xc = bot_sprintfs(&str, false, " = %lli", val);
						free(nv);
					}
				}
				break;
			...
			case BOT_RTV_REAL:
			{
				...
				if (!mv.vars[x].v.empty())
				{
					...
					if (nv)
					{
						...
						float val;
						memcpy((void*)&val, (void*)nv, sizeof(float));
						sint xc = bot_sprintfs(&str, false, " = %.9f", val);
						free(nv);
					}
				}
				break;
			...
			case BOT_RTV_STR:
			{
				...
				if (!mv.vars[x].v.empty())
				{
					...
					for (size_t y = 0; y < mv.vars[x].v[0].size(); y++)
					{
						str.push_back(mv.vars[x].v[0][y]);
					}
				}
				break;
			}
			case BOT_RTV_BLOB:
			{
				...
				if (!mv.vars[x].v.empty())
				{
					...
					for (size_t y = 0; y < mv.vars[x].v[0].size(); y++)
					{
						sint xc = bot_sprintfs(&str, false, "0x%02X ", mv.vars[x].v[0][y]);
					}
					str.pop_back();
				}
				break;
			...
			case BOT_RTV_VSLLINT:
			{
				...
				if (nv)
				{
					...
					{
						str.append("\n{\n\t");
						for (size_t y = 0; y < mv.vars[x].v.size(); y++)
						{
							sint xc = bot_sprintfs(&str, false, "[%u] ", y);
							for (size_t z = 0; z < sizeof(sllint); z++)
							{
								memcpy((void*)&nv[z], (void*)&mv.vars[x].v[y][z], sizeof(_char));
							}
							sllint val;
							memcpy((void*)&val, (void*)nv, sizeof(sllint));
			...
			case BOT_RTV_VSTR:
			{
				...
				if (!mv.vars[x].v.empty())
				{
					...
					for (size_t y = 0; y < mv.vars[x].v.size(); y++)
					{
						sint xc = bot_sprintfs(&str, false, "[%u] %v%c%,\n\t", y, &mv.vars[x].v[y]);
					}
					str.pop_back();
					str.pop_back();
					str.pop_back();
			...
			case BOT_RTV_VBLOB:
			{
				...
				if (!mv.vars[x].v.empty())
				{
					...
					for (size_t y = 0; y < mv.vars[x].v.size(); y++)
					{
						sint xc = bot_sprintfs(&str, false, "[%u] %v0x%02X %,\n\t", y, &mv.vars[x].v[y]);
					}
					str.pop_back();
					str.pop_back();
					str.pop_back();
			...
			sint xc = Output(true, str.c_str(), outs[siz], -1);
		}
	}
	return 0;
}
...
sint machine::Command(std::vector<BOT_ARG>* vec_)
{
	...
	case 1:
	{
		...
		if (vec_->size() > 1)
		{
			...
			while (GetVecEle(&ips, MTX_CIPS, xc++, false) > -1)
			{
				...
				sint rc = ArgSep(&av, 0, false, 0, bot_strlen(ips.carr), ips.carr, BOT_RTV_CHAR, ' ');
	...
	case 2:
	{
		...
		if (vec_->size() > 2)
		{
			...
			{
				BOT_CLIENT ProcClient(vec_->at(1).arg.c_str());
				sint rc = GetLogin(&ProcClient, vec_->at(2).arg.c_str());

				if (ProcClient.loggedin)
				{
	...
	case 31:
	{
		...
		if (!vec_->empty())
		{
			...
			xc = Output(true, "Test Threads complete.", 2, 0);
		}
		else
		{
			carr_64 xcar;
	...
	case 102:
	{
		...
		{
			sint inv = SOCK_STREAM;

			if (!strcmp(UCASE(vec_->at(3).arg.c_str()).c_str(), "TCP"))
			{
				inv = SOCK_STREAM;
			}
			else if (!strcmp(UCASE(vec_->at(3).arg.c_str()).c_str(), "UDP"))
			{
				inv = SOCK_DGRAM;
			}
			else if (!strcmp(UCASE(vec_->at(3).arg.c_str()).c_str(), "RAW"))
			{
				inv = SOCK_RAW;
			}
			else if (!strcmp(UCASE(vec_->at(3).arg.c_str()).c_str(), "PGM"))
			{
				inv = SOCK_RDM;
			}
			else {}
	...
	case 105:
	{
		...
		if (vec_->size() > 4)
		{
			...
			xc = AddSVPKO(inv, oblk.siz, oblk.blk, vec_->at(1).arg.c_str(), vec_->at(2).arg.c_str());
		}
		else
		{
			carr_192 xcar;
	...
	case 106:
	{
		...
		if (vec_->size() > 4)
		{
			BOT_FILE_M nfile(vec_->at(4).arg.c_str(), "", "", BOT_FILE_READ, BOT_F_CON);
			sint ox = -1;
			xc = BOTOpenFile(&nfile, &ox, true, false, false, true);

			if (xc > -1)
			{
				bot_vblko oblk((_char)BOT_OP_FHD, vec_->at(1).arg.c_str(), vec_->at(2).arg.c_str(), 1);
				xc = oblk.Add(0, BOT_RTV_STR, &nfile.name, BOT_RTV_MAX);
				xc = oblk.Add(0, BOT_RTV_STR, &nfile.type, BOT_RTV_MAX);
				xc = oblk.Add(0, BOT_RTV_SLLINT, (sllint)nfile.encode, BOT_RTV_MAX);
				xc = oblk.Add(0, BOT_RTV_SLLINT, (sllint)nfile.dsiz, BOT_RTV_MAX);
				sint inv = SOCK_STREAM;
				...
				xc = AddSVPKO(inv, oblk.siz, oblk.blk, vec_->at(1).arg.c_str(), vec_->at(2).arg.c_str(), 1);

				bot_vblko fblk((_char)BOT_OP_FCK, vec_->at(1).arg.c_str(), vec_->at(2).arg.c_str(), 1);
				xc = fblk.Add(0, BOT_RTV_CHARP, nfile.datp, BOT_RTV_MAX);
			}
		}
		else
		{
	...
	case 200:
	{
		...
		if (vec_->size() > 1)
		{
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "Length exceeds 5 characters", 2, 0);
				}
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "Incorrect format.", 2, 0);
				}
				else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
				{
					return 0;
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "Non numerical", 2, 0);
				}
				else if (resp.length() > 5)
				{
					xc = Output(true, "Length exceeds 5 characters", 2, 0);
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "incorrect format, use MMDDYYYY or YYYYMMDD or  MM/DD/YYYY", 2, 0);
				}
				else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
				{
					return 0;
	...
	case 201:
	{
		...
		if (vec_->size() > 1)
		{
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "Length exceeds 1 characters", 2, 0);
				}
				else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
				{
					return 0;
			...
			if (!isvar)
			{
				...
				while (!done)
				{
					...
					{
						xc = Output(true, "Value non numerical", 2, 0);
					}
					else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
					{
						return 0;
			...
			xc = ArgSep(&vvec, 0, false, 0, 0, vec_->at(1).arg.c_str(), BOT_RTV_CHAR, ' ');
			...
			if (xc > -1)
			{
				...
				if (xc > -1)
				{
					xc = BOTFileOUT(&nfile, nfile.fst.e_loc, false, BOT_RTV_STR, &fout, BOT_RTV_MAX);
	...
	case 202:
	{
		...
		if (vec_->size() > 1)
		{
			...
			while (!done)
			{
				...
				{
					xc = Output(true, "Length exceeds 1 character", 2, 0);
				}
				else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
				{
					return 0;
			...
			case 0:
			{
				...
				while (!done)
				{
					...
					xc = ArgSep(&istm.cols, 0, false, 0, resp.length(), resp.c_str(), BOT_RTV_VCHAR, &msy.nrts_sep);
				}
				break;
			}
			case 1:
			{
				xc = Output(true, "UPDATE vehicleadmin.inspect SET [a]=[b] WHERE ", 2, 0);
				...
				{
					xc = Output(true, "Length exceeds max characters", 2, 0);
				}
				else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
				{
					return 0;
				...
				while (!resp.empty())
				{
					...
					if (resp.length() > 1)
					{
						xc = Output(true, "Length exceeds one characters", 2, 0);
						xc = Input("conditions: 0:inspdate 1:insptype 2:ramp 3:empno - ", &resp);
					}
					else if (!strcmp(UCASE(resp.c_str()).c_str(), "EXIT"))
					{
				...
				if (!vec_->at(1).arg.empty())
				{
					...
					if (xc > -1)
					{
						for (size_t x = 0; x < mv.vars[xc].v.size(); x++)
						{
							fout.push_back('\n');
							std::string vstr;
							for (size_t y = 0; y < mv.vars[xc].v[x].size(); y++)
							{
								vstr.push_back(mv.vars[xc].v[x][y]);
							}
				...
				if (xc > -1)
				{
					xc = BOTFileOUT(&nfile, nfile.fst.e_loc, false, BOT_RTV_STR, &istm.stmt, BOT_RTV_MAX);
				...
				if (xc > -1)
				{
					xc = BOTFileOUT(&nfile, nfile.fst.e_loc, false, BOT_RTV_STR, &dstm.stmt, BOT_RTV_MAX);
...
sint machine::ArgSep(std::vector <std::string>* ret_, std::vector<BOT_VAR_T>* uvars, bool ksep, size_t f, size_t t, c_char* val, ...)
{
	if (debug_lvl >= 1100 && debug_m)
	{
		carr_256 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::ArgSep(std::vector <std::string>* ret_(%i), std::vector<BOT_VAR_T>* uvars, bool ksep(%u), size_t f(%u), size_t t(%u), c_char* val(%s), ...)", (sint)ret_, (sint)uvars, (uint)ksep, f, t, (sint)val);
		op = Output(outp.carr, 2);
	}
	...
	if (uvars)
	{
		for (size_t x = 0; x < ret_->size(); x++)
		{
			if (ret_->at(x)[0] == '$')
			{
				ret = ArgRep(&ret_->at(x));
			}
		}
	}
	return ret;
}
sint machine::ArgSep(std::vector <BOT_ARG>* ret_, std::vector<BOT_VAR_T>* uvars, bool ksep, size_t f, size_t t, c_char* val, ...)
{
	if (debug_lvl >= 1100 && debug_m)
	{
		carr_256 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::ArgSep(std::vector <BOT_ARG>* ret_(%i), std::vector<BOT_VAR_T>* uvars(%i), bool ksep(%u), size_t f(%u), size_t t(%u), c_char* val(%s), ...)", (sint)ret_, (sint)uvars, (uint)ksep, f, t, (sint)val);
		op = Output(outp.carr, 2);
	}
	...
	if (uvars)
	{
		for (size_t x = 0; x < ret_->size(); x++)
		{
			if (ret_->at(x).arg[0] == '$')
			{
				ret = ArgRep(&ret_->at(x).arg);
			}
		}
	}
	return ret;
}
sint machine::ArgRep(std::string* arg)
{
	if (debug_lvl >= 650 && debug_m)
	{
		carr_128 ncar;
		sint oc = bot_sprintf(ncar.carr, ncar.siz, "::ArgRep(std::string* arg(%i))", (sint)arg);
		oc = Output(ncar.carr, 2);
	}
	if (!arg)
	{
	...
	sint ret = mv.FindVar(arg->substr(1, arg->length() - 1).c_str());

	if (ret > -1)
	{
		std::string rep;
		mv.GetVar(ret, BOT_RTV_STR, &rep);
		arg->clear();
		arg->append(rep.c_str());
	}
	return ret;
}
// File Directory
					...
					if (dtp)
					{
						...
						if (hx > -1)
						{
							if (fileo_vec.dvec[mfile_->lid].Alloc(mfile_->fst.filestats.st_size + 1))
							{
								...
								gc = BOTFileIN(mfile_, false, 0, mfile_->fst.filestats.st_size, BOT_RTV_CHARP, fileo_vec.dvec[mfile_->lid].dat, BOT_RTV_MAX);
							}
...
sint machine::Output(bool newl, c_char* op_, ...)
{
	...
	for (uint x = 0; x < (uint)BOT_OUT_OPTS; x++)
	{
		rc = va_arg(args, sint);

		if (rc < 0 || rc >(uint)BOT_OUT_OPTS)
		{
			x = (uint)BOT_OUT_OPTS;
		}
		else
...
sint machine::Input(c_char* prp, std::string* np, bool itrp)
{
	...
	if (prp)
	{
		sint op = Output(true, prp, 2, 0);
	}
	sint x = BOTCInput(np, &tdata);

	if (np->length() > 1)
	{
		ret = ArgSep(&vec, &mv.vars, false, 0, np->length() - 1, np->c_str(), BOT_RTV_VSTR, &args);
...
sint machine::AnalyzeStmt(BOT_STMT *t_)
{
	...
	xc = ArgSep(&rets, 0, true, 0, stmt.length() - 1, stmt.c_str(), BOT_RTV_VSTR, &args);
	...
	if (xc > -1)
	{
		...
		if (t_->it_type > -1)
		{
			...
			case 0:
			{
				if (args.size() > 1)
				{
					sint rc = ArgSep(&rets, 0, false, 0, args[1].length() - 1, args[1].c_str(), BOT_RTV_VCHAR, &msy.sql_opers_keywords);
					rc = -1;
				}
				break;
			}
			...
			case 2:
			{
				if (args.size() > 4)
				{
					...
					if (rc > -1)
					{
						...
						rc = ArgSep(&t_->cols, 0, false, 0, args[2].length() - 2, args[2].substr(1,args[2].length() - 2).c_str(), BOT_RTV_VCHAR, &nargs);
						rc = ArgSep(&t_->vals, 0, false, 0, args[4].length() - 2, args[4].substr(1, args[4].length() - 2).c_str(), BOT_RTV_VCHAR, &nargs);
					}
				}
				break;
			}
			case 3:
			{
				if (args.size() > 1)
				{
					...
					sint rc = ArgSep(&rets, 0, false, 0, args[1].length() - 1, args[1].c_str(), BOT_RTV_VSTR, &msy.sql_obj_qual_keywords);
...
sint machine::GetLogin(BOT_CLIENT* c_, c_char* str_)
{
	if (debug_lvl >= 700 && debug_m)
	{
		carr_256 ncar;
		sint xc = bot_sprintf(ncar.carr, ncar.siz, "::GetLogin(BOT_CLIENT* c(%i), c_char* str_(%i))", (sint)c_, (sint)str_);
		xc = Output(ncar.carr, 2);
	}
					...
					while (!done)
					{
						...
						case SQLITE_ROW:
						{
							...
							if (sqlite3_column_text(ppstmt, 2) != NULL)
							{
								...
								if (xc > -1)
								{
									c_->id = sqlite3_column_int(ppstmt, xc);
								}
								...
								if (!strcmp(str_, reinterpret_cast <c_char*> (sqlite3_column_text(ppstmt, 2))))
								{
									...
									if (xc > -1)
									{
										c_->priv = sqlite3_column_int(ppstmt, xc);
									}
									...
									if (xc > -1)
									{
										c_->totallogins = sqlite3_column_int(ppstmt, xc) + 1;
									}

									c_->lastlogin.clear();
									c_->lastlogin.append(dated);
								}
								else
								{
									_char dated[128]{ 0 };
									xc = GetDateTimeMS(dated);
									c_->last_failed_login.clear();
									c_->last_failed_login.append(dated);

									xc = FindColumn("FAILED_LOGINS", ppstmt);

									if (xc > -1)
									{
										c_->failed_logins = sqlite3_column_int(ppstmt, xc) + 1;
									}
								}
							}
							break;
...
sint machine::UpdateAccount(BOT_CLIENT *Client_)
{
	...
	t.spec = 0;
	t.act = 0;
	sint xc = t.AddCond(0, "LOGIN_NAME", msy.sql_comp_keywords[5].c_str(), EStr(UCASE(Client_->name.c_str()).c_str()).c_str());
	xc = t.AddCond(0, "ID", msy.sql_comp_keywords[5].c_str(), stool.ITOA(Client_->id).c_str());

	if (Client_->loggedin)
	{
		if (Client_->changename)
		{
			xc = t.AddCol("LOGIN_NAME", EStr(UCASE(Client_->name.c_str()).c_str()).c_str());
		}
	...
	if (p > -1)
	{
		...
		case 0:
		{
			for (size_t t = 0; t < bot_strt.litebot_tablenames.size(); t++)
			{
				...
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.litebot_columns[t]), bot_strt.litebot_columns[t], BOT_RTV_CHAR, ' ');
				}
				else
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.litebot_columns[(sint)bot_strt.litebot_columns.size() - 1]), bot_strt.litebot_columns[(sint)bot_strt.litebot_columns.size() - 1], BOT_RTV_CHAR, ' ');
				}
		...
		case 1:
		{
			for (size_t t = 0; t < bot_strt.math_tablenames.size(); t++)
			{
				...
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.math_columns[t]), bot_strt.math_columns[t], BOT_RTV_CHAR, ' ');
				}
				else
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.math_columns[(sint)bot_strt.math_columns.size() - 1]), bot_strt.math_columns[(sint)bot_strt.math_columns.size() - 1], BOT_RTV_CHAR, ' ');
				}
		...
		case 2:
		{
			...
			if (np > -1)
			{
				for (size_t t = 1; t < bot_symbol_types.size(); t++)
				{
					...
					{
						sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.code_columns[t]), bot_strt.code_columns[t], BOT_RTV_CHAR, ' ');
					}
					else
					{
						sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.code_columns[(sint)bot_strt.code_columns.size() - 1]), bot_strt.code_columns[(sint)bot_strt.code_columns.size() - 1], BOT_RTV_CHAR, ' ');
					}
		...
		case 3:
		{
			for (size_t t = 0; t < bot_strt.syms_tablenames.size(); t++)
			{
				...
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.syms_cols[t]), bot_strt.syms_cols[t], BOT_RTV_CHAR, ' ');
				}
				else
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.syms_cols[(sint)bot_strt.syms_cols.size() - 1]), bot_strt.syms_cols[(sint)bot_strt.syms_cols.size() - 1], BOT_RTV_CHAR, ' ');
				}
		...
		case 4:
		{
			for (size_t t = 0; t < bot_strt.scripts_tablenames.size(); t++)
			{
				...
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.scripts_cols[t]), bot_strt.scripts_cols[t], BOT_RTV_CHAR, ' ');
				}
				else
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.scripts_cols[(sint)bot_strt.scripts_cols.size() - 1]), bot_strt.scripts_cols[(sint)bot_strt.scripts_cols.size() - 1], BOT_RTV_CHAR, ' ');
				}
		...
		case 5:
		{
			for (size_t t = 0; t < bot_strt.vars_tablenames.size(); t++)
			{
				...
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.vars_cols[t]), bot_strt.vars_cols[t], BOT_RTV_CHAR, ' ');
				}
				else
				{
					sint xp = ArgSep(&cols, 0, false, 0, bot_cstrlen(bot_strt.vars_cols[(sint)bot_strt.vars_cols.size() - 1]), bot_strt.vars_cols[(sint)bot_strt.vars_cols.size() - 1], BOT_RTV_CHAR, ' ');
				}
...
sint machine::ThreadRest(sllint dur, sint qt_lvl)
{
	...
	{
		sint rc = GetVecEleMem((void*)&qt, MTX_TTS, lid, BOT_TTS_SIG);
...
sint machine::MT(std::vector<ullint>* vals)
{
	...
	while (!done)
	{
		sint xc = GetDate(&ses_date, false);
...
sint machine::GetClientByPort(BOT_CLIENT* c_, _char* addr, sint portn, sint stype)
{
	if (debug_lvl >= 90 && debug_m)
	{
		carr_128 outp;
		sint op = bot_sprintf(outp.carr, outp.siz, "::GetClientByPort(BOT_CLIENT* c_(%i), _char* addr(%i), sint portn(%i), sint stype(%i))", (sint)c_, (sint)addr, portn, stype);
		op = Output(outp.carr, 2);
	}
	...
	for (sint xc = 0; GetVecEle((void*)&n, MTX_CV, xc, false) > -1; xc++)
	{
...
bool machine::GetClientLoggedIn()
{
	if (debug_lvl >= 90 && debug_m)
	{
		sint op = Output("::GetClientLoggedIn()", 2);
	}
	bool x = false;
	sint lx = -1;
	sint xc = LockGMutex(MTX_PCL, &lx);
...
sint machine::GetVINs(BOT_FILE_M* file_, c_char* ramp, c_char* emp, c_char* fdate, c_char* ftyp, bool as_var)
{
	...
	if (ox > -1)
	{
		...
		ox = ArgSep(&args, 0, false, 0, file_->dsiz - 1, file_->datp, BOT_RTV_CHAR, '\n');
			...
			else
			{
				...
				if (ox > -1)
				{
					...
					if (f < 0)
					{
						...
						ox = BOTFileOUT(&pcs, pcs.dsiz - 1, true, BOT_RTV_STR, &vstr);
					}
					else
					{
						for (size_t x = 0; x < args.size(); x++)
						...
						ox = BOTFileOUT(&pcs, pcs.fst.e_loc, true, BOT_RTV_STR, &vstr);
					}
			...
			if (xc > -1)
			{
				...
				if (pkt.op[0] == (_char)BOT_OP_ESTA)
				{
					...
					if (ret < 0)
					{
						...
						if (!str.empty())
						{
							bot_vblko oblk((_char)BOT_OP_ESTA, addr, port);
							bot_vblko pblk((_char)BOT_OP_ESTB, addr, port);
							oblk.Add(0, BOT_RTV_STR, &str, BOT_RTV_MAX);
							str.clear();
							xc = AddSVPKO(type, oblk.siz, oblk.blk, addr, port, 1);
							...
							while (str.length() < 5 || str.length() > 255)
							{
								...
								if (str.length() > 5 && str.length() < 256)
								{
									...
									xc = AddSVPKO(type, pblk.siz, pblk.blk, addr, port, 1);
								}
							}
	...
	if (xc > -1)
	{
		...
		else
		{
			...
			if (xc > -1)
			{
				...
				if (xc < 0)
				{
					sint oc = Output(true, "Failed to commit banned ip", 1, 0);
				}
				else
				{
					carr_256 ncar;
					sint oc = bot_sprintf(ncar.carr, ncar.siz, "Added ip (%s) to banned list.", ipstr);
					oc = Output(true, ncar.carr, 2);
				}
			}
			else
			{
				sint oc = Output(true, "Failed to find banned ip", 1, 0);
			}
		}
	}
	return xc;
	...
	if (xc > -1)
	{
		...
		else
		{
			...
			if (xc > -1)
			{
				...
				if (xc < 0)
				{
					sint oc = Output(true, "Failed to commit remove banned ip", 1, 0);
				}
				else
				{
					carr_256 ncar;
...
sint machine::AddVPKI(sint stype, size_t len, _char* dat, c_char* addr, c_char* port, _char ack)
{
	...
	if (lx > -1)
	{
		...
		if (!bot_pkti_lst[stype])
		{
			bot_pkti[stype].nxt = (bot_vblki*)malloc(sizeof(bot_vblki));

			if (bot_pkti[stype].nxt)
			{
				bot_pkti_lst[stype] = bot_pkti[stype].nxt;
			}
		}
		else
		{
			while (bot_pkti_lst[stype])
			{
				bot_pkti_lst[stype] = bot_pkti_lst[stype]->nxt;
			}

			bot_pkti_lst[stype] = (bot_vblki*)malloc(sizeof(bot_vblki));
		}

		if (bot_pkti_lst[stype])
		{
			memset((void*)bot_pkti_lst[stype], 0, sizeof(bot_vblki));
			bot_pkti_lst[stype]->addr.siz = 64;
			bot_pkti_lst[stype]->servc.siz = 64;
			bot_pkti_lst[stype]->ack = ack;
			bot_sprintf(bot_pkti_lst[stype]->addr.carr, bot_pkti_lst[stype]->addr.siz, "%s", addr);
			bot_sprintf(bot_pkti_lst[stype]->servc.carr, bot_pkti_lst[stype]->servc.siz, "%s", port);
			bot_pkti_lst[stype]->SetBlk(dat, len);
			lx = 0;
		}
		if (!hx)
		{
...
sint machine::AddVPKO(sint stype, size_t len, _char* dat, c_char* addr, c_char* port, _char ack)
{
	...
	if (lx > -1)
	{
		...
		if (!bot_pkto_lst[stype])
		{
			bot_pkto[stype].nxt = (bot_vblko*)malloc(sizeof(bot_vblko));

			if (bot_pkto[stype].nxt)
			{
				bot_pkto_lst[stype] = bot_pkto[stype].nxt;
			}
		}
		else
		{
			while (bot_pkto_lst[stype])
			{
				bot_pkto_lst[stype] = bot_pkto_lst[stype]->nxt;
			}

			bot_pkto_lst[stype] = (bot_vblko*)malloc(sizeof(bot_vblko));
		}

		if (bot_pkto_lst[stype])
		{
			memset((void*)bot_pkto_lst[stype], 0, sizeof(bot_vblko));
			bot_pkto_lst[stype]->addr.siz = 64;
			bot_pkto_lst[stype]->servc.siz = 64;
			bot_pkto_lst[stype]->ack = ack;
			bot_sprintf(bot_pkto_lst[stype]->addr.carr, bot_pkto_lst[stype]->addr.siz, "%s", addr);
			bot_sprintf(bot_pkto_lst[stype]->servc.carr, bot_pkto_lst[stype]->servc.siz, "%s", port);
			bot_pkto_lst[stype]->SetBlk(dat, len);
			lx = 0;
		}
		if (!hx)
		{
				...
				if (lx > -1)
				{
					...
					if (lx > -1)
					{
						...
						if (lx > -1)
						{
							...
							if (lx > -1)
							{
								BOT_HDR_PKT hdr(BOT_OP_SIZ, np->siz, 0, np->ack);
...
sint machine::AddSVPKI(sint stype, size_t len, _char* dat, c_char* addr, c_char* port, _char ack)
{
	...
	if (lx > -1)
	{
		...
		if (!bot_spkti_lst[stype])
		{
			bot_spkti[stype].nxt = (bot_svblki*)malloc(sizeof(bot_svblki));

			if (bot_spkti[stype].nxt)
			{
				bot_spkti_lst[stype] = bot_spkti[stype].nxt;
			}
		}
		else
		{
			while (bot_spkti_lst[stype])
			{
				bot_spkti_lst[stype] = bot_spkti_lst[stype]->nxt;
			}

			bot_spkti_lst[stype] = (bot_svblki*)malloc(sizeof(bot_svblki));
		}

		if (bot_spkti_lst[stype])
		{
			memset((void*)bot_spkti_lst[stype], 0, sizeof(bot_svblki));
			bot_spkti_lst[stype]->addr.siz = 64;
			bot_spkti_lst[stype]->servc.siz = 64;
			bot_spkti_lst[stype]->ack = ack;
			bot_sprintf(bot_spkti_lst[stype]->addr.carr, bot_spkti_lst[stype]->addr.siz, "%s", addr);
			bot_sprintf(bot_spkti_lst[stype]->servc.carr, bot_spkti_lst[stype]->servc.siz, "%s", port);
			bot_spkti_lst[stype]->SetBlk(dat, len);
			lx = 0;
		}
		if (!hx)
		{
...
sint machine::AddSVPKO(sint stype, size_t len, _char* dat, c_char* addr, c_char* port, _char ack)
{
	...
	if (lx > -1)
	{
		...
		if (!bot_spkto_lst[stype])
		{
			bot_spkto[stype].nxt = (bot_svblko*)malloc(sizeof(bot_svblko));

			if (bot_spkto[stype].nxt)
			{
				bot_spkto_lst[stype] = bot_spkto[stype].nxt;
			}
		}
		else
		{
			while (bot_spkto_lst[stype])
			{
				bot_spkto_lst[stype] = bot_spkto_lst[stype]->nxt;
			}

			bot_spkto_lst[stype] = (bot_svblko*)malloc(sizeof(bot_svblko));
		}

		if (bot_spkto_lst[stype])
		{
			memset((void*)bot_spkto_lst[stype], 0, sizeof(bot_svblko));
			bot_spkto_lst[stype]->addr.siz = 64;
			bot_spkto_lst[stype]->servc.siz = 64;
			bot_spkto_lst[stype]->ack = ack;
			bot_sprintf(bot_spkto_lst[stype]->addr.carr, bot_spkto_lst[stype]->addr.siz, "%s", addr);
			bot_sprintf(bot_spkto_lst[stype]->servc.carr, bot_spkto_lst[stype]->servc.siz, "%s", port);
			bot_spkto_lst[stype]->SetBlk(dat, len);
			lx = 0;
		}
		if (!hx)
		{
		...
		while (np)
		{
			...
			if (np->blk)
			{
				...
				case BOT_OP_NPT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received new port packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, sp.siz);
						rc = Output(true, outp.carr, 2, 0);
					}
				...
				case BOT_OP_GDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received gdat packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, sp.siz);
						rc = Output(true, outp.carr, 2, 0);
					}
				...
				case BOT_OP_NDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received EST packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, sp.siz);
						rc = Output(true, outp.carr, 2, 0);
					}
				...
				case BOT_OP_SDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received sdat packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}

					if (np->siz < BOT_FILESIZE_MAX)
					{
						if (debug_lvl >= 1 && debug_m)
						{
							carr_128 outp;
				...
				case BOT_OP_WDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received wdat packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}
				...
				case BOT_OP_VDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received vdat packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}

					std::string datstr;
					bot_sprintfs(&datstr, false, "%s %s %i data:", np->addr.carr, np->servc.carr, stype);
					size_t i = 1;
					...
					if (debug_lvl >= 1 && debug_m)
					{
						sint rc = Output(true, datstr.c_str(), 2, 0);
					}
					break;
				}
				default:
		...
		while (np)
		{
			...
			if (np->blk)
			{
				...
				case BOT_OP_NPT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received new port packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}
					...
					if (np->siz < BOT_FILESIZE_MAX)
					{
						...
						{
							carr_128 outp;
							sint rc = bot_sprintf(outp.carr, outp.siz, "data: %s", &np->blk[1]);
							rc = Output(true, outp.carr, 2, 0);
						}
					}
					break;
				...
				case BOT_OP_GDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received gdat packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}
				...
				case BOT_OP_NDAT:
				{
					...
					{
						carr_128 outp;
						sint rc = bot_sprintf(outp.carr, outp.siz, "received EST packet (%s port:%s) - data: %u bytes", np->addr.carr, np->servc.carr, np->siz);
						rc = Output(true, outp.carr, 2, 0);
					}
					...
					if (np->siz < BOT_FILESIZE_MAX)
					{
						...
						for (size_t iter = 1; iter < np->siz; iter += sizeof(ulint))
						{
							ulint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[iter], sizeof(ulint));
							val = ntohl(xval);
							uvec.push_back(val);
							val = 0;
						}
				...
				case BOT_OP_VDAT:
				{
					...
					while (i < np->siz)
					{
						...
						case BOT_RTV_SINT:
						{
							i += sizeof(_char);
							sint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(sint));
							i += sizeof(sint);
							sint rc = bot_sprintfs(&datstr, false, "\n%i", xval);
							break;
						}
						case BOT_RTV_SLINT:
						{
							i += sizeof(_char);
							slint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(slint));
							i += sizeof(slint);
							sint rc = bot_sprintfs(&datstr, false, "\n%li", xval);
							break;
						}
						case BOT_RTV_SLLINT:
						{
							i += sizeof(_char);
							sllint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(sllint));
							i += sizeof(sllint);
							sint rc = bot_sprintfs(&datstr, false, "\n%lli", xval);
							break;
						}
						case BOT_RTV_UINT:
						{
							i += sizeof(_char);
							uint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(uint));
							i += sizeof(uint);
							sint rc = bot_sprintfs(&datstr, false, "\n%u", xval);
							break;
						}
						case BOT_RTV_ULINT:
						{
							i += sizeof(_char);
							ulint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(ulint));
							i += sizeof(ulint);
							sint rc = bot_sprintfs(&datstr, false, "\n%lu", xval);
							break;
						}
						case BOT_RTV_ULLINT:
						{
							i += sizeof(_char);
							ullint xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(ullint));
							i += sizeof(ullint);
							sint rc = bot_sprintfs(&datstr, false, "\n%llu", xval);
							break;
						}
						case BOT_RTV_REAL:
						{
							i += sizeof(_char);
							float xval = 0;
							memcpy((void*)&xval, (void*)&np->blk[i], sizeof(float));
							i += sizeof(float);
							sint rc = bot_sprintfs(&datstr, false, "\n%.9f", xval);
							break;
						}
...
sint machine::BOTServR(std::vector<ullint>* vals)
{
	...
	while (!ret)
	{
		...
		if (servp.ptr->ai_family == AF_INET6)
		{
			ret = 1;
		}
		else
									...
									if (xret < 256)
									{
										...
										if (nm.siz > 2)
										{
											...
											case BOT_OP_ESTB:
											{
												if (nm.blk[1] == BOT_RTV_STR)
												{
													rc = GetLogin(&cli, &nm.blk[2]);

													if (cli.loggedin)
													{
														cli.updatelogin = true;
														rc = UpdateAccount(&cli);
														...
														bot_vblko rep((_char)BOT_OP_ESTC, cli.prt.oipstr.carr, nport.carr);
														rc = AddSVPKO(servp.prt.stype, rep.siz, rep.blk, cli.prt.oipstr.carr, nport.carr, 1);
														
														//xret = servp.Send((_char*)&est, sizeof(BOT_HDR_PKT));
						...
						if (np->siz < BOT_FILESIZE_MAX - 1)
						{
							...
							if (ind)
							{
								...
								if (xret > 0)
								{
									...
									{
										rc = AddSVPKI(servp.prt.stype, np->siz, ind, servp.prt.oipstr.carr, servp.servc.carr);
									}
								}
								bot_strclr(ind, np->siz);
								free(ind);
						...
						if (np->siz < BOT_FILESIZE_MAX - 1)
						{
							...
							if (ind)
							{
								...
								if (xret > 0)
								{
									...
									if (xret < BOT_FILESIZE_MAX)
									{
										rc = AddVPKI(servp.prt.stype, np->siz, ind, servp.prt.oipstr.carr, servp.servc.carr);
									}
								}
								bot_strclr(ind, np->siz);
								free(ind);
				...
				if (logm.lsiz)
				{
					...
					if (p > -1)
					{
						...
						if (rc > -1)
						{
							...
							if (rc > -1)
							{
								sint op = LITEBot.BOTFileOUT(&xfile, xfile.fst.e_loc, false, BOT_RTV_VSTR, &bot_log_vec.dvec[siz].log_q, BOT_RTV_MAX);
