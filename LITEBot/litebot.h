...
sllint bot_findin(_char val[] = 0, size_t offs = 0, _char inv[] = 0);
...
public:
	...
	size_t Alloc(ullint siz = 0)
	{
		...
		if (siz < (ullint)BOT_FILESIZE_MAX)
		{
			if (dat)
			{
				dat = (_char*)realloc(dat, (size_t)siz);
			}
			else
			{
	...
	void SetDat(_char* val_ = 0, ullint siz = 0)
	{
		...
		{
			dat = (_char*)realloc(dat, fst.filestats.st_size + 1);
	...
	void AddDat(_char* val_ = 0, ullint f = 0, ullint to = 0)
	{
		...
		dat = (_char*)realloc(dat, (size_t)(ne - nor) + 1);
...
/*	u_char GetValInVecUC(std::vector<u_char> *vec_, sint ele = -1) { u_char ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }*/
	...
	sint GVIV(std::vector<BOT_FILE> *vec_ = 0, sint ele = -1, BOT_FILE* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			...
			ret = GMIV(vec_, ele, BOT_FS_DATP, &val->dat);
			...
			ret = GMIV(vec_, ele, BOT_FS_DSIZ, &val->dsiz);
			ret = GMIV(vec_, ele, BOT_FS_CRS, &val->crsv);
			ret = GMIV(vec_, ele, BOT_FS_CRSV, &val->crsv);
			ret = GMIV(vec_, ele, BOT_FS_SELV, &val->selv);
		}
		return ret;
	}
	...
	sint GVIV(std::vector<BOT_FILE_M> *vec_ = 0, sint ele = -1, BOT_FILE_M* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			...
			ret = GMIV(vec_, ele, BOT_FS_BTO, &val->bto);
			ret = GMIV(vec_, ele, BOT_FS_DSIZ, &val->dsiz);
			ret = GMIV(vec_, ele, BOT_FS_DATP, &val->datp);
			ret = GMIV(vec_, ele, BOT_FS_CRS, &val->crsv);
			ret = GMIV(vec_, ele, BOT_FS_CRSV, &val->crss);
			ret = GMIV(vec_, ele, BOT_FS_SELV, &val->sels);
		}
		return ret;
	}
	...
	sint GVIV(std::vector<bot_port> *vec_ = 0, sint ele = -1, bot_port* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			ret = GMIV(vec_, ele, BOT_PORT_LID, &val->lid);
			ret = GMIV(vec_, ele, BOT_PORT_FAM, &val->fam);
			ret = GMIV(vec_, ele, BOT_PORT_PTN, &val->portn);
			ret = GMIV(vec_, ele, BOT_PORT_PCL, &val->proto);
			ret = GMIV(vec_, ele, BOT_PORT_SOP, &val->sopts);
		}
		return ret;
	}
	...
	sint GVIV(std::vector<bot_plug> *vec_ = 0, sint ele = -1, bot_plug* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			ret = GMIV(vec_, ele, BOT_PLUG_LID, &val->lid);
			ret = GMIV(vec_, ele, BOT_PLUG_ERR, &val->emsg);
			ret = GMIV(vec_, ele, BOT_PLUG_BDP, &val->bndp);
			ret = GMIV(vec_, ele, BOT_PLUG_SRV, &val->isserv);
			ret = GMIV(vec_, ele, BOT_PLUG_CON, &val->con);
			ret = GMIV(vec_, ele, BOT_PLUG_DSC, &val->dsc);
			ret = GMIV(vec_, ele, BOT_PLUG_SOP, &val->sopt);
			ret = GMIV(vec_, ele, BOT_PLUG_QSZ, &val->qsiz);
			ret = GMIV(vec_, ele, BOT_PLUG_PRT, &val->prt);
			ret = GMIV(vec_, ele, BOT_PLUG_RES, &val->res);
			ret = GMIV(vec_, ele, BOT_PLUG_PTR, &val->ptr);
			ret = GMIV(vec_, ele, BOT_PLUG_HNT, &val->hints);
			ret = GMIV(vec_, ele, BOT_PLUG_STO, &val->sto);
			ret = GMIV(vec_, ele, BOT_PLUG_URL, &val->url);
			ret = GMIV(vec_, ele, BOT_PLUG_FAM, &val->ifam);
			ret = GMIV(vec_, ele, BOT_PLUG_IPS, &val->ipstr);
			ret = GMIV(vec_, ele, BOT_PLUG_SVC, &val->servc);
			ret = GMIV(vec_, ele, BOT_PLUG_SCK, &val->sck);
			ret = GMIV(vec_, ele, BOT_PLUG_TSCK, &val->tsck);
		}
		return ret;
	}
	...
	sint SVIV(std::vector<BOT_FILE> *vec_ = 0, sint ele = -1, BOT_FILE* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			...
			ret = SMIV(vec_, ele, BOT_FS_DATP, &val->dat);
			ret = SMIV(vec_, ele, BOT_FS_DSIZ, &val->dsiz);
			ret = SMIV(vec_, ele, BOT_FS_CRS, &val->crsv);
			ret = SMIV(vec_, ele, BOT_FS_CRSV, &val->crsv);
			ret = SMIV(vec_, ele, BOT_FS_SELV, &val->selv);
			ret = ele;
		}
		return ret;
	}
	...
	sint SVIV(std::vector<BOT_FILE_M> *vec_ = 0, sint ele = -1, BOT_FILE_M* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			...
			ret = SMIV(vec_, ele, BOT_FS_BTO, &val->bto);
			ret = SMIV(vec_, ele, BOT_FS_DSIZ, &val->dsiz);
			ret = SMIV(vec_, ele, BOT_FS_DATP, &val->datp);
			ret = SMIV(vec_, ele, BOT_FS_CRS, &val->crsv);
			ret = SMIV(vec_, ele, BOT_FS_CRSV, &val->crss);
			ret = SMIV(vec_, ele, BOT_FS_SELV, &val->sels);
	...
	sint SMIV(std::vector<BOT_FILE_M> *vec_ = 0, sint ele = -1, sint memb = -1, void* val = 0)
	{
		...
		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_FS_MAX)
		{
			...
			else if (memb == BOT_FS_DATP) { _char* val_ = reinterpret_cast<_char*>(val); vec_->at(ele).datp = val_; }
			else if (memb == BOT_FS_CRS) { BOT_CRS* val_ = reinterpret_cast<BOT_CRS*>(val); if (val_) { vec_->at(ele).SetCrs(val_); } }
			else if (memb == BOT_FS_CRSV) { size_t* val_ = reinterpret_cast<size_t*>(val); vec_->at(ele).crss = *val_; }
			else if (memb == BOT_FS_SELV) { size_t* val_ = reinterpret_cast<size_t*>(val); vec_->at(ele).sels = *val_; }
			else {}
			ret = ele;
		}
		return ret;
	...
	sint SVIV(std::vector<bot_port> *vec_ = 0, sint ele = -1, bot_port* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			ret = SMIV(vec_, ele, BOT_PORT_LID, &val->lid);
			ret = SMIV(vec_, ele, BOT_PORT_FAM, &val->fam);
			ret = SMIV(vec_, ele, BOT_PORT_PTN, &val->portn);
			ret = SMIV(vec_, ele, BOT_PORT_PCL, &val->proto);
			ret = SMIV(vec_, ele, BOT_PORT_SOP, &val->sopts);
			ret = ele;
		}
		return ret;
	}
	...
	sint SVIV(std::vector<bot_plug> *vec_ = 0, sint ele = -1, bot_plug* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			ret = SMIV(vec_, ele, BOT_PLUG_LID, &val->lid);
			ret = SMIV(vec_, ele, BOT_PLUG_ERR, &val->emsg);
			ret = SMIV(vec_, ele, BOT_PLUG_BDP, &val->bndp);
			ret = SMIV(vec_, ele, BOT_PLUG_SRV, &val->isserv);
			ret = SMIV(vec_, ele, BOT_PLUG_CON, &val->con);
			ret = SMIV(vec_, ele, BOT_PLUG_DSC, &val->dsc);
			ret = SMIV(vec_, ele, BOT_PLUG_SOP, &val->sopt);
			ret = SMIV(vec_, ele, BOT_PLUG_QSZ, &val->qsiz);
			ret = SMIV(vec_, ele, BOT_PLUG_PRT, &val->prt);
			ret = SMIV(vec_, ele, BOT_PLUG_RES, &val->res);
			ret = SMIV(vec_, ele, BOT_PLUG_PTR, &val->ptr);
			ret = SMIV(vec_, ele, BOT_PLUG_HNT, &val->hints);
			ret = SMIV(vec_, ele, BOT_PLUG_STO, &val->sto);
			ret = SMIV(vec_, ele, BOT_PLUG_URL, &val->url);
			ret = SMIV(vec_, ele, BOT_PLUG_FAM, &val->ifam);
			ret = SMIV(vec_, ele, BOT_PLUG_IPS, &val->ipstr);
			ret = SMIV(vec_, ele, BOT_PLUG_SVC, &val->servc);
			ret = SMIV(vec_, ele, BOT_PLUG_SCK, &val->sck);
			ret = SMIV(vec_, ele, BOT_PLUG_TSCK, &val->tsck);
			ret = ele;
		}
		return ret;
	}
...
private:
	...
	sint BOTFileIN(BOT_FILE_M* file_ = 0, ullint from = -1, ullint to = -1, ...);
