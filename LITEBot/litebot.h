...
#define BOT_FILE_CHUNK (4096 - (sizeof(slint) * (sizeof(size_t) + sizeof(size_t) + sizeof(sint) + sizeof(slint) + sizeof(_char) + sizeof(_char)))) / sizeof(slint)
#else
...
#ifndef BOT_FILE_HCHUNK
#define BOT_FILE_HCHUNK (4096 - (sizeof(slint) * (sizeof(slint) + sizeof(size_t) + sizeof(sint) + sizeof(_char) + sizeof(_char)))) / (sizeof(slint) * sizeof(slint))
#else
#if (BOT_FILE_HCHUNK > BOT_STRLEN_MAX)
#define BOT_FILE_HCHUNK BOT_STRLEN_MAX
#endif
#endif
...
sint BOTDateTimeMS(_char str_[] = 0, size_t siz = 0, bool form = true);
sint BOTDateTime(_char str_[] = 0, size_t siz = 0, bool form = true);
sint BOTTime(_char str_[] = 0, size_t siz = 0, bool forms = true);
sint BOTTimeMS(_char str_[] = 0, size_t siz = 0, bool forms = true);
sint BOTDate(_char str_[] = 0, size_t siz = 0, bool form = true);
sllint BOTCentSec();
...
public:
	...
	void Clear()
	{
		...
		sopts.clear();
		memset((void*)&maddr, 0, sizeof(sockaddr_storage));
	...
	void Renew(bot_port* nport = 0)
	{
		...
		xcl = nport->xcl;
		signal = nport->signal;
		maddrlen = nport->maddrlen;
		oaddrlen = nport->oaddrlen;
		lid = nport->lid;
		portn = nport->portn;
		rport = nport->rport;
		proto = nport->proto;
		...
		memcpy((void*)&maddr, (void*)&nport->maddr, sizeof(maddr));
...
public:
	...
	sint Open(sint flg = AI_PASSIVE, bool nbndp = false)
	{
		...
		if (!con && res != NULL)
		{
			...
			prt.fam = ptr->ai_family;
			memcpy((void*)&prt.maddr, ptr->ai_addr, ptr->ai_addrlen);
			prt.maddrlen = ptr->ai_addrlen;
		}
		else
		{
			PError(con);
	...
	sint Sock(bool xclsck = false)
	{
		...
		if (prt.fam == AF_INET)
		{
			prt.sck = socket(PF_INET, prt.stype, prt.proto);
		}
		else if (prt.fam == AF_INET6)
		{
			prt.sck = socket(PF_INET6, prt.stype, prt.proto);
		}
		else if (prt.fam == AF_UNSPEC)
		{
			prt.sck = socket(PF_UNSPEC, prt.stype, prt.proto);
		}
		else {}
		...
		if (prt.bndp)
		{
			...
			ret = bind(prt.sck, (struct sockaddr*)&prt.maddr, prt.maddrlen);
	...
	sint Conn()
	{
		...
		if (!isserv)
		{
			ret = connect(prt.sck, (struct sockaddr*)&prt.maddr, prt.maddrlen);
		}
		else
		{
	...
	sllint Send(_char *dat = 0, sint dlen = 0)
	{
		...
		else if (prt.proto == IPPROTO_UDP && con > 0)
		{
			if (!isserv)
			{
				for (; ret > -1 && ret < dlen;)
				{
					add = (sllint)sendto(prt.sck, dat, dlen, 0, (struct sockaddr*)&prt.maddr, prt.maddrlen);
	...
	void Renew(bot_file_hdr* nh = 0)
	{
		...
		if (nh)
		{
			...
			for (size_t x = 0; x < nh->rec.size(); x++)
			{
				if (nh->rec[x])
				{
					rec.push_back(nh->rec[x]);
				}
			}
		}
	}

	bot_file_hdr(BOT_FILE_M* file_ = 0)
	{
		lid = fid = -1;
		idh = 0;
		seq = -1;
		...
		if (file_)
		{
			...
			dsiz = file_->dsiz;
			carr_32 ncar;
			BOTTimeMS(ncar.carr, ncar.siz, false);
			idh = INT32_MAX - (slint)atoi(ncar.carr);
		}
	}
private:
};

typedef struct bot_file_chk
{
	slint idh;
	slint seq;
	size_t f;
	...
	bot_file_chk(slint nidh = 0, size_t nf = 0, size_t nt = 0, _char* ndat = 0)
	{
		seq = -1;
		idh = nidh;
		f = nf;
		...
		if (ndat && nf < nt)
		{
			dat = (_char*)malloc((nt - nf) + 1);

			if (dat)
			{
				_char trm = '\0';
				size_t y = nt - nf;
				memcpy((void*)dat, (void*)ndat, sizeof(_char) * y);
				memcpy((void*)&dat[y], (void*)&trm, sizeof(_char));
			}
		}
	}
	~bot_file_chk()
...
public:
	...
	sint AddVar(c_char* nn = 0, void* nv = 0, sint nt = -1, sint vt = -1, bool overw = false)
	{
		if (nn && nt > -1 && vt > -1)
		{
			if (nt == BOT_RTV_VAR)
			{
				BOT_VAR_T* var = reinterpret_cast<BOT_VAR_T*>(nv);
					...
					while (x < vars.size())
					{
						...
						else if (!strcmp(vars[x].n, nn))
						{
							if (nv && overw)
							{
								...
								for (size_t a = 0; a < var->v.size(); a++)
								{
									vars[x].v.push_back(nvec);

									for (size_t b = 0; b < var->v[a].size(); b++)
									{
										vars[x].v[a].push_back(var->v[a][b]);
									}
								}
							}
							x = vars.size();
						...
						else if (!strcmp(vars[x].n, nn))
						{
							if (nv && overw)
							{
								...
								case BOT_RTV_FHD:
								{
									...
									std::vector<_char> nvec;
									bot_file_hdr* hdr = reinterpret_cast<bot_file_hdr*>(nv);
									...
									if (hdr)
									{
										...
										{
											_char* xv = 0;
											_char* ov = (_char*)malloc(sizeof(slint));

											if (ov)
											{
												memcpy((void*)ov, (void*)&hdr->idh, sizeof(slint));

												for (size_t z = 0; z < sizeof(slint); z++)
												{
													vars[x].v[0].push_back(ov[z]);
												}
												xv = (_char*)realloc(ov, sizeof(sint));

												if (xv)
												{
													ov = xv;
													memcpy((void*)ov, (void*)&hdr->fid, sizeof(sint));

													for (size_t z = 0; z < sizeof(slint); z++)
													{
														vars[x].v[1].push_back(ov[z]);
													}
												}
												free(ov);
											}
	...
	sint FindVar(c_char* nn = 0)
	{
		...
		for (size_t x = 0; x < vars.size(); x++)
		{
			if (nn[0] == '$')
			{
				if (!strcmp(vars[x].n, &nn[1]) && vars[x].t > -1)
				{
					return (sint)x;
				}
			}
	...
	sint GetVar(c_char* nm = 0, sint as_t = -1, void* val = 0)
	{
		...
		if (ret > -1)
		{
			...
			{
				_char trm = '\0';
				BOT_VAR_T* var = reinterpret_cast<BOT_VAR_T*>(val);

				if (var)
				{
					size_t vl = bot_strlen(vars[ret].n);

					if (var->n)
					{
						bot_strclr(var->n);
						_char* nvar = (_char*)realloc(var->n, vl + 1);
					...
					{
						var->n = (_char*)malloc(vl + 1);
					}

					if (var->n)
					{
						memcpy((void*)var->n, (void*)vars[ret].n, vl);
						memcpy((void*)&var->n[vl], (void*)&trm, sizeof(_char));
					}
					...
					for (size_t a = 0; a < vars[ret].v.size(); a++)
					{
						var->v.push_back(nvec);
						for (size_t b = 0; b < vars[ret].v[a].size(); b++)
						{
							var->v[a].push_back(vars[ret].v[a][b]);
						}
					}
				}
				return 0;
			...
			case BOT_RTV_FHD:
			{
				...
				case BOT_RTV_FHD:
				{
					_char* sumv = (_char*)malloc(sizeof(slint));

					if (sumv)
					{
						bot_file_hdr* hdr = reinterpret_cast<bot_file_hdr*>(val);

						for (size_t x = 0; x < vars[ret].v[0].size(); x++)
						{
							memcpy((void*)&sumv[x], (void*)&vars[ret].v[0][x], sizeof(_char));
						}

						memcpy((void*)&hdr->idh, (void*)sumv, sizeof(slint));

						for (size_t x = 0; x < vars[ret].v[1].size(); x++)
						{
							memcpy((void*)&sumv[x], (void*)&vars[ret].v[1][x], sizeof(_char));
						}

						memcpy((void*)&hdr->fid, (void*)sumv, sizeof(sint));
						free(sumv);
					}
					break;
				}
	...
	sint GetVar(sint ele = 0, sint as_t = -1, void* val = 0)
	{
		if (ele < 0 || ele > (sint)vars.size() || as_t < 0 || !val)
		{
			return -1;
		}
		...
		if (as_t == BOT_RTV_VAR)
		{
			...
			if (var)
			{
				size_t vl = bot_strlen(vars[ele].n);
				...
				if (var->n)
				{
					memcpy((void*)var->n, (void*)vars[ele].n, vl);
					memcpy((void*)&var->n[vl], (void*)&trm, sizeof(_char));
				}
				...
				for (size_t a = 0; a < vars[ele].v.size(); a++)
				{
					var->v.push_back(nvec);
					for (size_t b = 0; b < vars[ele].v[a].size(); b++)
					{
						var->v[a].push_back(vars[ele].v[a][b]);
					}
				}
			}
			return 0;
		...
		case BOT_RTV_FHD:
		{
			...
			case BOT_RTV_FHD:
			{
				...
				if (hdr)
				{
					_char* sumv = (_char*)malloc(sizeof(slint) + sizeof(_char));

					if (sumv)
					{
						_char trm = '\0';
						memcpy((void*)&sumv[sizeof(slint) + sizeof(_char)], (void*)&trm, sizeof(_char));
						...
						slint av = 0;
						memcpy((void*)&av, (void*)sumv, sizeof(slint));
						hdr->idh = av;

						for (size_t x = 0; x < vars[ele].v[1].size(); x++)
						{
							memcpy((void*)&sumv[x], (void*)&vars[ele].v[1][x], sizeof(_char));
						}
						sint bv = 0;
						memcpy((void*)&bv, (void*)sumv, sizeof(sint));
						hdr->fid = bv;
						free(sumv);
					}
				}
	...
	sint RemVar(sint ele = -1)
	{
		sint ret = -1;

		if (ele > -1 && ele < (sint)vars.size())
		{
			ret = (sint)ele;
			vars[ele].t = -1;
			bot_strclr(vars[ele].n);
			vars[ele].v.clear();
		}
		CleanVars();
		return ret;
...
public:
	...
	void Clear()
	{
		loggedin = changename = updatepriv = updatelogin = false;
		atcons = true;
...
public:
	...
	sint AddBlk(_char nop = 0, c_char* naddr = 0, c_char* nport = 0, _char nack = 0)
	{
		if (!naddr || !nport)
		{
			return -1;
		...
		if (nxt)
		{
			bot_vblko* x = (bot_vblko*)realloc(nxt, sizeof(bot_vblko));
		...
		{
			nxt = (bot_vblko*)malloc(sizeof(bot_vblko));
		}

		if (nxt)
		{
			memset((void*)nxt, 0, sizeof(bot_vblko));
			nxt->addr.siz = 64;
			nxt->servc.siz = 64;
			nxt->ack = nack;
			nxt->siz = 0;
			...
			if (naddr)
			{
				bot_sprintf(nxt->addr.carr, nxt->addr.siz, "%s", naddr);
			}

			if (nport)
			{
				bot_sprintf(nxt->servc.carr, nxt->servc.siz, "%s", nport);
			}
			nxt->blk = NULL;
			nxt->nxt = NULL;
			nxt->blk = (_char*)malloc(2);
		...
		while (!done)
		{
			...
			case BOT_RTV_SINT:
			{
				...
				if (nblk)
				{
					...
					siz += sizeof(_char);
					memcpy((void*)&nblk[siz], (void*)&inv, sizeof(sint));
					siz += (sizeof(sint));
			...
			case BOT_RTV_VAR:
			{
				BOT_VAR_T* inv = va_arg(args, BOT_VAR_T*);

				if (inv)
				{
					size_t ln = bot_strlen(inv->n) + sizeof(_char);

					if (inv->t == BOT_RTV_SLLINT || inv->t == BOT_RTV_VSLLINT)
					{
						for (size_t y = 0; y < inv->v.size(); y++)
						{
							ln += sizeof(sllint);
						}
					}
					else if (inv->t == BOT_RTV_REAL || inv->t == BOT_RTV_VREAL)
					{
						for (size_t y = 0; y < inv->v.size(); y++)
						{
							ln += (sizeof(slint) + sizeof(slint));
						}
					}
					else
					{
						for (size_t y = 0; y < inv->v.size(); y++)
						{
							ln += (inv->v[y].size() + sizeof(_char));
						}
					}
					
					_char* nblk = (_char*)realloc(blk, siz + ln + sizeof(_char) + sizeof(_char) + sizeof(_char));

					if (nblk)
					{
						memcpy((void*)&nblk[siz], (void*)&vt, sizeof(_char));
						siz += sizeof(_char);
						memcpy((void*)&nblk[siz], (void*)&inv->t, sizeof(_char));
						siz += sizeof(_char);
						memcpy((void*)&nblk[siz], (void*)inv->n, bot_strlen(inv->n));
						siz += bot_strlen(inv->n);
						memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
						siz += sizeof(_char);
						...
						case BOT_RTV_SLLINT:
						{
							if (!inv->v.empty())
							{
								_char* xv = (_char*)malloc(sizeof(sllint) + sizeof(_char));

								if (xv)
								{
									memcpy((void*)&xv[sizeof(sllint)], (void*)&trm, sizeof(_char));
									for (size_t y = 0; y < sizeof(sllint); y++)
									{
										memcpy((void*)&xv[y], (void*)&inv->v[0][y], sizeof(_char));
									}
									sllint av = 0;
									memcpy((void*)&av, (void*)xv, sizeof(sllint));
									av = htonll(av);
									memcpy((void*)xv, (void*)&av, sizeof(sllint));

									for (size_t y = 0; y < sizeof(sllint); y++)
									{
										memcpy((void*)&nblk[siz], (void*)&xv[y], sizeof(_char));
										siz += sizeof(_char);
									}
						...
						case BOT_RTV_REAL:
						{
							if (!inv->v.empty())
							{
								_char* xv = (_char*)malloc(sizeof(float) + sizeof(_char));

								if (xv)
								{
									memcpy((void*)&xv[sizeof(float)], (void*)&trm, sizeof(_char));
									for (size_t y = 0; y < sizeof(float); y++)
									{
										memcpy((void*)&xv[y], (void*)&inv->v[0][y], sizeof(_char));
									}
									float av = 0.0f;
									memcpy((void*)&av, (void*)xv, sizeof(slint));
									BOT_FTIA fv(&av);
									fv.a[0] = htonl(fv.a[0]);
									fv.a[1] = htonl(fv.a[1]);
									_char* nxv = (_char*)malloc(sizeof(slint) + sizeof(_char));

									if (nxv)
									{
										memcpy((void*)&nxv[sizeof(slint)], (void*)&trm, sizeof(_char));
										memcpy((void*)nxv, (void*)&fv.a[0], sizeof(slint));

										for (size_t y = 0; y < sizeof(slint); y++)
										{
											memcpy((void*)&nblk[siz], (void*)&nxv[y], sizeof(_char));
											siz += sizeof(_char);
										}
										memcpy((void*)nxv, (void*)&fv.a[1], sizeof(slint));
						...
						case BOT_RTV_BLOB:
						{
							if (!inv->v.empty())
							{
								for (size_t y = 0; y < inv->v[0].size(); y++)
								{
									memcpy((void*)&nblk[siz], (void*)&inv->v[0][y], sizeof(_char));
									siz += sizeof(_char);
								}
								memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
								siz += sizeof(_char);
						...
						case BOT_RTV_VSLLINT:
						{
							if (!inv->v.empty())
							{
								...
								if (xv)
								{
									...
									for (size_t x = 0; x < inv->v.size(); x++)
									{
										for (size_t y = 0; y < sizeof(sllint); y++)
										{
											memcpy((void*)&xv[y], (void*)&inv->v[x][y], sizeof(_char));
										}
										sllint av = 0;
										memcpy((void*)&av, (void*)xv, sizeof(sllint));
										av = htonll(av);
						...
						case BOT_RTV_VBLOB:
						{
							if (!inv->v.empty())
							{
								for (size_t x = 0; x < inv->v.size(); x++)
								{
									for (size_t y = 0; y < inv->v[x].size(); y++)
									{
										memcpy((void*)&nblk[siz], (void*)&inv->v[x][y], sizeof(_char));
										siz += sizeof(_char);
									}
									memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
									siz += sizeof(_char);
						...
						_char ev = (_char)BOT_RTV_MAX;
						memcpy((void*)&nblk[siz], (void*)&ev, sizeof(_char));
						siz += sizeof(_char);
						memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
						blk = nblk;
					}
			...
			case BOT_RTV_FHD:
			{
				bot_file_hdr* inv = va_arg(args, bot_file_hdr*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + (sizeof(size_t)*(inv->rec.size())) + sizeof(slint) + sizeof(sint) + sizeof(size_t) + sizeof(size_t) + sizeof(_char));

					if (nblk)
					{
						memcpy((void*)&nblk[siz], (void*)&vt, sizeof(_char));
						siz += sizeof(_char);
						slint aval = htonl(inv->idh);
						memcpy((void*)&nblk[siz], (void*)&aval, sizeof(slint));
						siz += sizeof(slint) * sizeof(_char);
						aval = htonl(inv->seq);
						memcpy((void*)&nblk[siz], (void*)&aval, sizeof(slint));
						siz += sizeof(slint) * sizeof(_char);
						size_t bval = htonl(inv->dsiz);
						memcpy((void*)&nblk[siz], (void*)&bval, sizeof(size_t));
						siz += sizeof(size_t) * sizeof(_char);

						for (size_t z = 0; z < inv->rec.size() && z <= BOT_FILE_HCHUNK; z++)
						{
							aval = htonl(inv->rec[z]);
							memcpy((void*)&nblk[siz], (void*)&aval, sizeof(slint));
							siz += sizeof(slint) * sizeof(_char);
						}
						aval = htonl(INT32_MAX);
						memcpy((void*)&nblk[siz], (void*)&aval, sizeof(slint));
						siz += sizeof(slint);
						memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
						blk = nblk;
			...
			case BOT_RTV_FLC:
			{
				BOT_FILE_CHK* inv = va_arg(args, BOT_FILE_CHK*);

				if (inv)
				{
					size_t d = inv->t - inv->f;

					if (d <= BOT_FILE_CHUNK)
					{
						_char* nblk = (_char*)realloc(blk, siz + sizeof(BOT_FILE_CHK) + d + sizeof(_char));
						...
						if (nblk)
						{
							...
							memcpy((void*)&nblk[siz], (void*)&aval, sizeof(slint));
							siz += sizeof(slint) * sizeof(_char);
							size_t bval = htonl(inv->f);
							memcpy((void*)&nblk[siz], (void*)&bval, sizeof(size_t));
							siz += sizeof(size_t) * sizeof(_char);
							bval = htonl(inv->t);
							memcpy((void*)&nblk[siz], (void*)&bval, sizeof(size_t));
							siz += sizeof(size_t) * sizeof(_char);
							memcpy((void*)&nblk[siz], (void*)inv->dat, d);
							siz += d;
							memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
							blk = nblk;
						}
			...
			case BOT_RTV_VSINT:
			{
				std::vector<sint>* inv = va_arg(args, std::vector<sint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(sint)) + sizeof(_char) + sizeof(_char)));
					...
					if (nblk)
					{
						...
						{
							sint xv = htons(inv->at(x));
							memcpy((void*)&nblk[siz], (void*)&xv, sizeof(sint));
							siz += (sizeof(sint));
						}
						_char eval = (_char)BOT_RTV_MAX;
						memcpy((void*)&nblk[siz], (void*)&eval, sizeof(_char));
						siz += sizeof(_char);
						memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char));
						blk = nblk;
					}
			...
			case BOT_RTV_VSLINT:
			{
				std::vector<slint>* inv = va_arg(args, std::vector<slint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(slint)) + sizeof(_char) + sizeof(_char)));
			...
			case BOT_RTV_VSLLINT:
			{
				std::vector<sllint>* inv = va_arg(args, std::vector<sllint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(sllint)) + sizeof(_char) + sizeof(_char)));
			...
			case BOT_RTV_VUINT:
			{
				std::vector<uint>* inv = va_arg(args, std::vector<uint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(uint)) + sizeof(_char) + sizeof(_char)));
			...
			case BOT_RTV_VULINT:
			{
				std::vector<ulint>* inv = va_arg(args, std::vector<ulint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(ulint)) + sizeof(_char) + sizeof(_char)));
			...
			{
				std::vector<ullint>* inv = va_arg(args, std::vector<ullint>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(ullint)) + sizeof(_char) + sizeof(_char)));
			...
			case BOT_RTV_VREAL:
			{
				std::vector<float>* inv = va_arg(args, std::vector<float>*);

				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + (((inv->size() * sizeof(slint)) * 2) + sizeof(_char) + sizeof(_char)));
			...
			case BOT_RTV_VCHAR:
			{
				...
				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(_char)) + sizeof(_char) + sizeof(_char) + sizeof(_char)));
					...
					if (nblk)
					{
						...
						for (size_t x = 0; x < inv->size(); x++)
						{
							memcpy((void*)&nblk[siz], (void*)&inv->at(x), sizeof(_char));
							siz += sizeof(_char);
							memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char)); // terminates our string
							siz += sizeof(_char);
						}
			...
			case BOT_RTV_VUCHAR:
			{
				...
				if (inv)
				{
					_char* nblk = (_char*)realloc(blk, siz + ((inv->size() * sizeof(u_char)) + sizeof(_char) + sizeof(_char) + sizeof(_char)));
					...
					if (nblk)
					{
						...
						for (size_t x = 0; x < inv->size(); x++)
						{
							memcpy((void*)&nblk[siz], (void*)&inv->at(x), sizeof(u_char));
							siz += sizeof(_char);
							memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char)); // terminates our string
							siz += sizeof(_char);
						}
			...
			case BOT_RTV_VBLOB:
			{
				std::vector<std::vector<_char>>* inv = va_arg(args, std::vector<std::vector<_char>>*);

				if (inv)
				{
					size_t ln = 0;
					for (size_t y = 0; y < inv->size(); y++)
					{
						ln += inv->at(y).size() + sizeof(_char);
					}

					_char* nblk = (_char*)realloc(blk, siz + ln + sizeof(_char) + sizeof(_char) + sizeof(_char)); // extra element to terminate our string
					...
					if (nblk)
					{
						...
						for (size_t x = 0; x < inv->size(); x++)
						{
							for (size_t y = 0; y = inv->at(x).size(); y++)
							{
								memcpy((void*)&nblk[siz], (void*)&inv->at(x)[y], sizeof(_char));
								siz += sizeof(_char);
							}
							memcpy((void*)&nblk[siz], (void*)&trm, sizeof(_char)); // terminates our string
							siz += sizeof(_char);
			...
			case BOT_RTV_VSTR:
			{
				std::vector<std::string>* inv = va_arg(args, std::vector<std::string>*);
				...
				if (inv)
				{
					...
					if (nblk)
					{
						...
						for (size_t x = 0; x < inv->size(); x++)
						{
							for (size_t y = 0; y = inv->at(x).length(); y++)
							{
								memcpy((void*)&nblk[siz], (void*)&inv->at(x)[y], sizeof(_char));
								siz += sizeof(_char);
							}
			...
			if (blk)
			{
				...
				while (x < nsiz && y < siz)
				{
					...
					case BOT_RTV_REAL:
					{
						...
						y += sizeof(float);
						siz -= (sizeof(slint) * 2) - sizeof(float);
					...
					case BOT_RTV_VAR:
					{
						memcpy((void*)&blk[y], (void*)&ndat[x], sizeof(_char)); // op
						x += sizeof(_char);
						y += sizeof(_char);
						_char typ = ndat[x];
						memcpy((void*)&blk[y], (void*)&ndat[x], sizeof(_char)); // t
						x += sizeof(_char);
						y += sizeof(_char);
						size_t ln = bot_strlen(&ndat[x]);
						memcpy((void*)&blk[y], (void*)&ndat[x], sizeof(_char) * ln); // n
						x += sizeof(_char) * ln;
						y += sizeof(_char);
						memcpy((void*)&blk[y], (void*)&trm, sizeof(_char));
						x += sizeof(_char);
						...
						while (!done)
						{
							...
							case BOT_RTV_SLLINT:
							{
								...
								memcpy((void*)&xv, (void*)&ndat[x], sizeof(sllint));
								xv = ntohll(xv);
								memcpy((void*)&blk[y], (void*)&xv, sizeof(sllint)); // v
								x += sizeof(sllint);
								y += sizeof(sllint);
								break;
							}
							...
							case BOT_RTV_BLOB:
							{
								ln = bot_strlen(&ndat[x]);
								memcpy((void*)&blk[y], (void*)&ndat[x], sizeof(_char) * ln); // v
								x += sizeof(_char) * ln;
								y += sizeof(_char) * ln;
								memcpy((void*)&blk[y], (void*)&trm, sizeof(_char));
								x += sizeof(_char);
					...
					case BOT_RTV_FHD:
					{
						...
						y += sizeof(_char);
						slint xli = 0;
						memcpy((void*)&xli, (void*)&ndat[x], sizeof(_char) * sizeof(slint)); // idh
						x += sizeof(_char) * sizeof(slint);
						xli = ntohl(xli);
						memcpy((void*)&blk[y], (void*)&xli, sizeof(_char) * sizeof(slint));
						y += sizeof(_char) * sizeof(slint);
						xli = 0;
						memcpy((void*)&xli, (void*)&ndat[x], sizeof(_char) * sizeof(slint)); // seq
						x += sizeof(_char) * sizeof(slint);
						xli = ntohl(xli);
						memcpy((void*)&blk[y], (void*)&xli, sizeof(_char) * sizeof(slint));
						y += sizeof(_char) * sizeof(slint);
						size_t xs = 0;
						memcpy((void*)&xs, (void*)&ndat[x], sizeof(_char) * sizeof(size_t)); // dsiz
						x += sizeof(_char) * sizeof(size_t);
						xs = ntohl(xs);
						memcpy((void*)&blk[y], (void*)&xs, sizeof(_char) * sizeof(size_t));
						y += sizeof(_char) * sizeof(size_t);
						bool done = false;

						while (!done)
						{
							xli = 0;
							memcpy((void*)&xli, (void*)&ndat[x], sizeof(_char) * sizeof(slint)); // rec
							x += sizeof(_char) * sizeof(slint);
							xli = ntohl(xli);
							memcpy((void*)&blk[y], (void*)&xli, sizeof(_char) * sizeof(slint));
							y += sizeof(_char) * sizeof(slint);
					...
					case BOT_RTV_FLC:
					{
						...
						y += sizeof(_char) * sizeof(slint);
						size_t xf = 0;
						memcpy((void*)&xf, (void*)&ndat[x], sizeof(_char) * sizeof(size_t)); // f
						x += sizeof(_char) * sizeof(size_t);
						xf = ntohl(xf);
						memcpy((void*)&blk[y], (void*)&xf, sizeof(_char) * sizeof(size_t));
						y += sizeof(_char) * sizeof(size_t);
						size_t xt = 0;
						memcpy((void*)&xt, (void*)&ndat[x], sizeof(_char) * sizeof(size_t)); // t
						x += sizeof(_char) * sizeof(size_t);
						xt = ntohl(xt);
						memcpy((void*)&blk[y], (void*)&xt, sizeof(_char) * sizeof(size_t));
						y += sizeof(_char) * sizeof(size_t);

						if (xt > xf && xt - xf <= BOT_FILE_CHUNK)
						{
							xt -= xf;
							memcpy((void*)&blk[y], (void*)&ndat[x], xt);
							x += xt;
							y += xt;
						}
						memcpy((void*)&blk[y], (void*)&trm, sizeof(_char));
					...
					case BOT_RTV_VSINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(sint));
							xv = ntohs(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(sint));
							x += sizeof(sint);
							y += sizeof(sint);
					...
					case BOT_RTV_VSLINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(slint));
							xv = ntohl(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(slint));
							x += sizeof(slint);
							y += sizeof(slint);
					...
					case BOT_RTV_VSLLINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(sllint));
							xv = ntohll(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(sllint));
							x += sizeof(sllint);
							y += sizeof(sllint);
					...
					case BOT_RTV_VUINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(uint));
							xv = ntohs(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(uint));
							x += sizeof(uint);
							y += sizeof(uint);
					...
					case BOT_RTV_VULINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(ulint));
							xv = ntohl(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(ulint));
							x += sizeof(ulint);
							y += sizeof(ulint);
					...
					case BOT_RTV_VULLINT:
					{
						...
						while (!done)
						{
							...
							memcpy((void*)&xv, (void*)&ndat[x], sizeof(ullint));
							xv = ntohll(xv);
							memcpy((void*)&blk[y], (void*)&xv, sizeof(ullint));
							x += sizeof(ullint);
							y += sizeof(ullint);
...
public:
	...
	sint VIV(std::vector<BOT_DB> *vec_ = 0, BOT_DB* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz).lid > -1 && val->lid == vec_->at(siz).lid) { vn = siz; *val = vec_->at(siz); siz = vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz).lid > -1 && val->lid == vec_->at(siz).lid) { vn = siz; val->Renew(&vec_->at(siz)); siz = -1; } siz--; } } return vn; }
	...
	sint VIV(std::vector<BOT_CLIENT> *vec_ = 0, BOT_CLIENT* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz).lid > -1 && !strcmp(val->name.c_str(), vec_->at(siz).name.c_str()) && vec_->at(siz).prt.lid == val->prt.lid) { vn = siz; *val = vec_->at(siz); siz = vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz).lid > -1 && !strcmp(val->name.c_str(), vec_->at(siz).name.c_str()) && vec_->at(siz).prt.lid == val->prt.lid) { vn = siz; val->Renew(&vec_->at(siz)); siz = -1; } siz--; } } return vn; }
	...
	sint VIV(std::vector<lok_req> *vec_ = 0, lok_req* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz).lid > -1 && val->gmtx == vec_->at(siz).gmtx && val->ele == vec_->at(siz).ele && val->is_meta == vec_->at(siz).is_meta && val->fid == vec_->at(siz).fid) { vn = siz; *val = vec_->at(siz); siz = vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz).lid > -1 && val->gmtx == vec_->at(siz).gmtx && val->ele == vec_->at(siz).ele && val->is_meta == vec_->at(siz).is_meta && val->fid == vec_->at(siz).fid) { vn = siz; val->Renew(&vec_->at(siz)); siz = -1; } siz--; } } return vn; }
	...
	sint VIV(std::vector<HELD_MTX*> *vec_ = 0, HELD_MTX* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz)) { if (vec_->at(siz) == val) { vn = siz; siz = (sint)vec_->size(); } } siz++; } } else { while (siz > -1) { if (vec_->at(siz)) { if (vec_->at(siz) == val) { vn = siz; siz = -1; } } siz--; } } return vn; }
	sint VIV(std::vector<BOTF_CONN*> *vec_ = 0, BOTF_CONN* val = 0, sint e_from = 0, bool forw = true) { if (!val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz)) { if (vec_->at(siz)->lid > -1 && vec_->at(siz)->flid == val->flid) { vn = siz; siz = (sint)vec_->size(); } } siz++; } } else { while (siz > -1) { if (vec_->at(siz)) { if (vec_->at(siz)->lid > -1 && vec_->at(siz)->flid == val->flid) { vn = siz; siz = -1; } } siz--; } } return vn; }
	...
	sint VIV(std::vector<bot_file_hdr> *vec_ = 0, bot_file_hdr* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz).lid > -1 && vec_->at(siz).idh == val->idh && vec_->at(siz).seq == val->seq) { vn = siz; *val = vec_->at(siz); siz = vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz).lid > -1 && vec_->at(siz).idh == val->idh && vec_->at(siz).seq == val->seq) { vn = siz; *val = vec_->at(siz); siz = -1; } siz--; } } return vn; }
	...
	sint UEE(std::vector<bot_file_hdr> *vec_ = 0, bot_file_hdr* val = 0) { if (!vec_ || !val) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid < 0) { vec_->at(siz) = *val; vn = (sint)siz; val->lid = vn; vec_->at(siz).lid = vn; siz = vec_->size(); } siz++; } return vn; }
	...
	sint AVTV(std::vector<bot_file_hdr> *vec_, bot_file_hdr* val, bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = VIV(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UEE(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } val->lid = viv; vec_->push_back(*val); return viv; }
...
/*	u_char GetValInVecUC(std::vector<u_char> *vec_, sint ele = -1) { u_char ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }*/
	...
	sint GMIV(std::vector<BOT_CLIENT> *vec_ = 0, sint ele = -1, sint memb = -1, void* val = 0)
	{
		...
		if (ele < (sint)vec_->size())
		{
			if (memb < BOT_C_MAX && memb > -1)
			{
				...
				else if (memb == BOT_C_U_ACON) { bool* vp = reinterpret_cast<bool*>(val); *vp = vec_->at(ele).atcons; }
				...
				else if (memb == BOT_C_KIPS) { std::string* vp = reinterpret_cast<std::string*>(val); vp->clear(); vp->append(vec_->at(ele).kips); }
	...
	bot_file_hdr GVIV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1) { bot_file_hdr ret; if (vec_ && ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GVIV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, bot_file_hdr* val = 0)
	{
		if (!vec_ || !val || ele < 0) { return -1; }
	...
	sint GMIV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!vec_ || !val || ele < 0) { return -1; }
		...
		if (ele < (sint)vec_->size())
		{
			if (memb < BOT_FHD_MAX && memb > -1)
			{
				if (memb == BOT_FHD_IDH) { slint* vp = reinterpret_cast<slint*>(val); *vp = vec_->at(ele).idh; }
				else if (memb == BOT_FHD_SEQ) { sint* vp = reinterpret_cast<sint*>(val); *vp = vec_->at(ele).seq; }
				else if (memb == BOT_FHD_DSZ) { size_t* vp = reinterpret_cast<size_t*>(val); *vp = vec_->at(ele).dsiz; }
				else if (memb == BOT_FHD_FID) { sint* vp = reinterpret_cast<sint*>(val); *vp = vec_->at(ele).fid; }
				else if (memb == BOT_FHD_LID) { sint* vp = reinterpret_cast<sint*>(val); *vp = vec_->at(ele).lid; }
				else if (memb == BOT_FHD_REC) { std::vector<size_t>* vp = reinterpret_cast<std::vector<size_t>*>(val); vp->clear(); for (size_t x = 0; x < vec_->at(ele).rec.size(); x++) { vp->push_back(vec_->at(ele).rec[x]); } }
				else {}
				ret = ele;
			}
		}
	...
	sint SMIV(std::vector<BOT_CLIENT> *vec_ = 0, sint ele = -1, sint memb = -1, void* val = 0)
	{
		...
		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_C_MAX)
		{
			...
			else if (memb == BOT_C_U_ACON) { bool* val_ = reinterpret_cast<bool*>(val); vec_->at(ele).atcons = *val_; }
			...
			else if (memb == BOT_C_KIPS) { std::string* val_ = reinterpret_cast<std::string*>(val); vec_->at(ele).kips.clear(); if (val_->length() < 128) { vec_->at(ele).kips.append(val_->c_str()); } }
	...
	sint SVIV(std::vector<bot_file_hdr*> *vec_ = 0, sint ele = -1, bot_file_hdr* val = 0) { if (!vec_ || !val || ele < 0) { return -1; } sint ret = -1; if (ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SVIV(std::vector<bot_file_hdr> *vec_ = 0, bot_file_hdr* val = 0) { if (!vec_ || !val) { return -1; }  if (val->lid < 0) { return -1; } sint ret = -1; if (val->lid < (sint)vec_->size()) { vec_->at((size_t)(val->lid))= *val; ret = val->lid; } return ret; }
	sint SVIV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, bot_file_hdr* val = 0)
	{
		if (!vec_ || !val || ele < 0) { return -1; }
	...
	sint SMIV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!vec_ || !val || ele < 0) { return -1; }

		sint ret = -1;

		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_FHD_MAX)
		{
			if (memb == BOT_FHD_IDH) { slint* val_ = reinterpret_cast<slint*>(val); if (val_) { vec_->at(ele).idh = *val_; } }
			else if (memb == BOT_FHD_SEQ) { sint* val_ = reinterpret_cast<sint*>(val); if (val_) { vec_->at(ele).seq = *val_; } }
			else if (memb == BOT_FHD_DSZ) { size_t* val_ = reinterpret_cast<size_t*>(val); if (val_) { vec_->at(ele).dsiz = *val_; } }
			else if (memb == BOT_FHD_FID) { sint* val_ = reinterpret_cast<sint*>(val); if (val_) { vec_->at(ele).fid = *val_; } }
			else if (memb == BOT_FHD_LID) { sint* val_ = reinterpret_cast<sint*>(val); if (val_) { vec_->at(ele).lid = *val_; } }
			else if (memb == BOT_FHD_REC) { std::vector<size_t>* val_ = reinterpret_cast<std::vector<size_t>*>(val); if (val_) { vec_->at(ele).rec.clear(); for (size_t x = 0; x < val_->size(); x++) { vec_->at(ele).rec.push_back(val_->at(x)); } } }
			else {}
			ret = ele;
		}
		return ret;
		...
		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_C_MAX)
		{
			...
			else if (memb == BOT_C_KIPS)
			{
				if (opt == 1)
				{
					...
					if (val_)
					{
						vec_->at(ele).kips.push_back(*val_);
					}
				}
				else if (opt == 2)
				{
	...
	sint PTM(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, sint memb = -1, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_FHD_MAX)
		{
			if (memb == BOT_FHD_REC)
			{
				if (opt == 2)
				{
					...
					if (val_)
					{
						vec_->at(ele).rec.push_back(*val_);
					}
				}
				else if (opt == 3)
				{
					std::vector<size_t>* val_ = reinterpret_cast<std::vector<size_t>*>(val);

					if (val_)
					{
						for (size_t x = 0; x < val_->size(); x++)
						{
							vec_->at(ele).rec.push_back(val_->at(x));
						}
					}
				}
				else {}
	...
	sint CFV(std::vector<bot_file_hdr> *vec_ = 0, bot_file_hdr* val = 0, bool clear_all = false) { sint ret = -1; if (!vec_ || !val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { vec_->at(siz).dsiz = 0; vec_->at(siz).rec.clear(); vec_->at(siz).seq = vec_->at(siz).lid = -1; vec_->at(siz).idh = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	...
	sint CEFV(std::vector<BOT_FILE_M> *vec_ = 0, sint ele = -1) { sint ret = -1; if (vec_ && ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).Clear(); ret = ele; } return ret; }
	...
	sint CEFV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1) { sint ret = -1; if (vec_ && ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).dsiz = 0; vec_->at(ele).rec.clear(); vec_->at(ele).lid = vec_->at(ele).seq = -1; vec_->at(ele).idh = 0; ret = ele; } return ret; }
	sint CMV(std::vector<bot_file_hdr> *vec_ = 0, sint ele = -1, sint memb = -1)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
		...
		if (ele < (sint)vec_->size() && memb > -1 && memb < BOT_FHD_MAX)
		{
			...
			else if (memb == BOT_FHD_SEQ)
			{
				vec_->at(ele).seq = -1;
				ret = ele;
			}
			else if (memb == BOT_FHD_DSZ)
			{
				vec_->at(ele).dsiz = 0;
				ret = ele;
			}
			else if (memb == BOT_FHD_FID)
			{
				vec_->at(ele).fid = -1;
				ret = ele;
			}
			else if (memb == BOT_FHD_LID)
			{
				vec_->at(ele).lid = -1;
				ret = ele;
			}
			else if (memb == BOT_FHD_REC)
			{
				vec_->at(ele).rec.clear();
				ret = ele;
			}
			else {}
		}
	...
	sint CLNV(std::vector<bot_file_hdr> *vec_ = 0, bool compactify = false) { if (!vec_) { return -1; } if (compactify && vec_->size() > 1) { for (size_t x = 0; x + 1 < vec_->size(); x++) { if (vec_->at(x).lid < 0) { for (size_t y = x + 1; y < vec_->size(); y++) { if (vec_->at(y).lid > -1) { sint z = SVIV(vec_, (sint)x, &vec_->at(y)); z = CEFV(vec_, (sint)y); } } } } } sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } return (sint)vec_->size(); }
...
private:
	...
	sint BOTOpenObject(sint nobj = -1, sint nele = -1, sint is_meta = -1, sint* omode = 0, sint* r_opt = 0, sint* n_opt = 0);
	sint BOTCloseObject(sint nobj = -1, sint nele = -1, sint is_meta = -1, sint* r_opt = 0, sint* n_opt = 0);
	...
	sint SVAR(c_char* nm = 0, sint nt = -1, void* val = 0);
	sint RVAR(c_char* nm = 0);
	sint LVARS(sint n, ...);

	// Commands

	sint Command(BOT_CLIENT_M* cli = 0, std::vector<BOT_ARG>* vec_ = 0);
	...
	sint BOTFileOUT(BOT_FILE_M* file_ = 0, size_t f = 0, size_t t = 0, bool to_fdat = false, ...);
	...
	sint GetPCli(BOT_CLIENT_M* val = 0);
	sint GetClientByPort(BOT_CLIENT_M* c_ = 0, _char* addr = 0, sint portn = 0, sint stype = 0);
	// Console I/O Functions

	sint UNRTS();
	sint BOTCOutput(std::string* np = 0);
	sint BOTNOutput(std::string* np = 0, BOT_CLIENT_M* cli = 0);
	sint BOTNOutput(std::string* np = 0, std::vector<BOT_CLIENT_M>* cliv = 0);
	sint Output(c_char* op_ = "", sint opt = -1, bool newl = true, BOT_CLIENT_M* cli = 0);
	sint Output(BOT_CLIENT_M* cli = 0, bool newl = true, c_char* op_ = "", ...);
	...
	sint ArgRep(_char* arg = 0, sint rett = -1, void* ret = 0);
	...
	sint GetCommand(BOT_CLIENT_M* cli = 0, BOTCOMMAND *Command_ = 0, std::vector<std::string> *conds_ = 0);
	sint GetCommands(BOT_CLIENT_M* cli = 0, std::vector<BOTCOMMAND> *Commands_ = 0, std::vector<std::string> *conds_ = 0);
	sint BOTGetLogin(BOT_CLIENT_M* c = 0, c_char* str_ = 0);
	sint BOTUpdateAccount(BOT_CLIENT_M* c = 0);
	...
	sint AddVPKO(sint stype = -1, bot_vblko* dat = 0);
	...
	sint AddSVPKO(sint stype = -1, bot_vblko* dat = 0);
	sint SVPKO(sint stype = -1);
	sint BOTServR(std::vector<ullint>* vals = 0);
	sint BOTServPS(sint stype = -1);
	sint BOTServP(sint stype = -1);
	sint ClearDSum(slint nidh = 0, sint seq = -1, slint sumv = 0);
	sint ChunkDat(_char* dat = 0, size_t f = 0, size_t* len = 0, bot_file_hdr* hdr = 0, BOT_FILE_CHK* chk = 0);

	sint GetVINs(BOT_FILE_M* file_ = 0, c_char* ramp = "", c_char* emp = "", c_char* fdate = "", c_char* ftyp = "", bool as_var = false);
	sint GetStringV(BOT_FILE_M* file_ = 0, bool as_var = false);
