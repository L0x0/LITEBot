...
sint bot_randstr(char str_[] = 0, size_t siz = 0, char fstr_[] = 0, size_t fsiz = 0)
{
	if (!siz || !str_)
	{
		return -1;
	...
	if (!fstr_)
	{
		...
		char cfr[64] = { 'W', 'E', 'R', 'n', 'I', 'Z', 'O', 'm', 'P', 'S', 'D', 'F', '5', 'u', 'J', 'K', 'v', 'b', 'y', 'i', '4', '2', 's', 'B', '_', 'L', 'X', 'C', '9', 'V', 'M', 'q', 'c', 'w', 'T', 'Y', 'e', 'r', 'f', '6', 't', 'U', 'A', 'o', 'Q', 'p', 'a', 'F', 'u', 'd', '1', '0', 'g', 'N', 'j', 'k', 'l', 'z', '8', 'x', '3', '7', 'h', '\0' };
		siz--;
...
sint bot_sprintf(char inchr[] = 0, size_t in_siz = 0, c_char* fstr = 0, ...)
{
...
errno_t bot_dtoa(double i, char in_chr[] = 0, size_t in_siz = 0, sint radix = 10) { if (!in_chr || !in_siz) { return -1; } return (errno_t)bot_sprintf(in_chr, in_siz, "%d", i); }
errno_t bot_dtoa(long double i, char in_chr[] = 0, size_t in_siz = 0, sint radix = 10) { if (!in_chr || !in_siz) { return -1; } return (errno_t)bot_sprintf(in_chr, in_siz, "%ld", i); }
...
public:
	...
	sint SetCol(u_char val[] = 0, sint col = -1, sint row = -1)
	{
		...
		if (col < (sint)cvec.size())
		{
			for (uint y = 0; y < cvec[col].rows.size(); y++)
			{
				...
				if (sizeof(val) == cvec[col].rows[y].size())
				{
	...
	sint SetCol(char val[] = 0, sint col = -1, sint row = -1)
	{
		if (col < 0 || !val)
		{
			return -1;
...
typedef struct bot_fstate
{
	...
	void ClearStats() { filestats.st_atime = 0; filestats.st_ctime = 0; filestats.st_dev = 0; filestats.st_ino = 0; filestats.st_mode = 0; filestats.st_mtime = 0; filestats.st_nlink = 0; filestats.st_rdev = 0; filestats.st_size = 0; filestats.st_uid = 0; }
	void Clear() { ClearStats(); e_loc = oa_loc = ia_loc = 0; exists = omode = -1; compv.clear(); cts.clear(); }
	...
	sint cmpto(bot_fstate* est = 0)
	{
		...
		for (uint x = 0; x < compv.size(); x++)
		{
			...
			{
				if (cts[x] != est->cts[x] || memcmp((void*)&compv[x], (void*)&est->compv[x], sizeof(compv[x])))
				{
					y = est->compv.size();
				}
				y++;
	...
	bot_fstate(sint no_mode = -1, ullint ne_loc = 0, ullint noa_loc = 0, ullint nia_loc = 0, std::vector<u_char>* ncv = 0, std::vector<sint>* ncts = 0)
	{
		exists = -1;
		omode = no_mode;
		e_loc = ne_loc;
		...
		if (ncv)
		{
			for (uint x = 0; x < ncv->size(); x++)
			{
				compv.push_back(ncv->at(x));
			}
		}
		if (ncts)
		{
			for (uint x = 0; x < ncts->size(); x++)
			{
				cts.push_back(ncts->at(x));
			}
		}
	};
...
public:
	...
	void Clear() { fst.Clear(); bto = -1; encode = cm = lid = c_by = omode = fcon = -1; if (!path.empty()) { path.clear(); } if (!type.empty()) { type.clear(); } if (!name.empty()) { name.clear(); } if (!strvec.empty()) { strvec.clear(); } }
	...
	void Renew(c_char* nm_ = 0, c_char* nt_ = 0, c_char* np_ = 0, sint nomode = -1, sint ncm = -1, sint nencode = -1, sint nlid = -1, sint nc_by = -1, sllint nbto = -1, bot_fstate* nstate = 0)
	{
	...
	BOT_FILE(c_char* nfn = "", c_char* nft = "", c_char* nfp = "", sint nomode = -1, sint ncm = -1, sint nencode = -1, sint nlid = -1, sint nc_by = -1, sllint nbto = -1, bot_fstate* nfst = 0)
	{
...
public:
	...
	void Clear() { fst.ClearStats(); bto = -1; encode = cm = lid = c_by = omode = fcon = -1; if (!path.empty()) { path.clear(); } if (!type.empty()) { type.clear(); } if (!name.empty()) { name.clear(); } strsiz = 0; }
	...
	void Renew(c_char* nm_ = 0, c_char* nt_ = 0, c_char* np_ = 0, sint nomode = -1, sint ncm = -1, sint nlid = -1, sint nencode = -1, sint nc_by = -1, sllint nbto = -1, bot_fstate* nstate = 0)
	{
		if (!nm_ || !nt_ || !np_ || nc_by < 0 || nomode < 0)
		{
			return;
	...
	BOT_FILE_M(c_char* nfn = "", c_char* nft = "", c_char* nfp = "", sint nomode = -1, sint ncm = -1, sint nencode = -1, sint nlid = -1, sint nc_by = -1, sllint nbto = -1, bot_fstate* nfst = 0)
	{
		if (strlen(nfn) && strlen(nfn) < BOT_FILENAME_MAX)
		{
			name.append(nfn);
...
public:
	...
	void Clear() { min_ratv = 0.0f; t_I = std::chrono::steady_clock::now(); opt = lid = c_by = -1; fin = -10; if (!thread_name.empty()) { thread_name.clear(); } func = NULL; }
	...
	void Renew(sint nopt = -1, c_char* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1, float nmin_ratv = 0.0f)
	{
		Clear();
	...
	TINFO(sint nopt = -1, c_char* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1, float nmin_ratv = 0.0f) { min_ratv = nmin_ratv; t_I = std::chrono::steady_clock::now(); func = nfunc; c_by = nc_by; opt = nopt; lid = -1; fin = -10; size_t x = strlen(ntn); if (x && x < 256) { thread_name.append(ntn); } };
...
public:
	...
	void Clear() { min_ratv = 0.0f; attr = thread = 0; t_I = std::chrono::steady_clock::now(); opt = lid = c_by = -1; fin = -10; if (!thread_name.empty()) { thread_name.clear(); } func = NULL; }
	...
	TINFO_M(sint nopt = -1, c_char* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1, float nmin_ratv = 0.0f) { min_ratv = nmin_ratv; t_I = std::chrono::steady_clock::now(); opt = lid = -1; fin = -10; if (nopt > -1) { opt = nopt; } if (strlen(ntn) && strlen(ntn) < 256) { thread_name.append(ntn); } func = nfunc; c_by = nc_by; };
...
public:
	...
	std::string CleanFromStr(c_char* inp_ = "", sint from = 0, sint lim = 0, ...)
	{
		...
		for (char fin = va_arg(args, char); fin != '\0'; fin = va_arg(args, char))
		{
			cvec.push_back(fin);
		}
		va_end(args);
	...
	sint VToStr(std::vector<ullint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
	...
	sint VToStr(std::vector<ulint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<uint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<sllint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<slint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<sint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<c_char*>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint VToStr(std::vector<std::string>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, c_char* sep = " ")
	{
		if (!vec_ || !instr_ || !i_siz || !sep)
		{
			return -1;
	...
	sint FindInStr(char f, c_char* str = 0, sint at = -1, sint lim = -1, bool fwd = true)
	{
		if (!str)
		{
			return -1;
	...
	sint FindVInStr(std::vector<char>* cvec_ = 0, std::string* str_ = 0, sint* loc = 0, sint lim = -1, sint* ele = 0, sint ele_lim = -1, bool fwd = true, bool v_fwd = true)
	{
	...
	sint FindVInStr(std::vector<char>*cvec_ = 0, c_char* str_ = 0, sint* loc = 0, sint lim = -1, sint* ele = 0, sint ele_lim = -1, bool fwd = true, bool v_fwd = true)
	{
		if (!cvec_ || !str_ || !loc || !ele)
		{
			return -1;
	...
	sint SeparateStrByChar(c_char *str_ = "", char chr = ' ', std::vector<std::string> *rvec_ = 0)
	{
		if (!str_ || !rvec_)
		{
			return -1;
...
public:
	...
	sint ValInVec(std::vector<char> *vec_, c_char* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz) == *val) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz) == *val) { vn = siz; siz = -1; } siz--; } } return vn; }
...
public:
	...
	std::vector<c_char*> symbol_types
	{
		"enders",
		"vowels",
		"consonants",
...
private:
		...
		if (!BOTOpenDir(dest_, &dir_))
		{
			...
			while ((dp_ = readdir(dir_)) != NULL)
			{
				...
				while (x < cvec.size())
				{
					...
					while (xcmp < cvec[x].length() && xcmp < dp_->d_namlen)
					{
				...
				if (x == cvec.size())
				{
					BOT_FILE_M xfile(dp_->d_name, "", dest_, 0, BOT_DIR_CON);
					sint xc = FileStats(&xfile);
							...
							if (!xc)
							{
								if (S_ISDIR(dstat.st_mode))
								{
									...
									else
									{
										...
										while (cmp < nvec.size())
										{
											if (nvec[cmp].length() == dp->d_namlen)
											{
												uint xcmp = 0;
	...
	sint ConnMethod(BOT_FILE_M* val_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_64 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::ConnMethod(BOT_FILE_M *val_(%i))", (sint)val_);
			oc = Output(ncar.carr, 2);
		}
	...
	sint GetOFConn(BOT_FILE_M* val_ = 0)
	{
		...
		{
			carr_64 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::GetOFConn(BOT_FILE_M *val_(%i))", (sint)val_);
			oc = Output(ncar.carr, 2);
		}
	...
	sint GetFConn(BOT_FILE_M* val_ = 0)
	{
		...
		{
			carr_64 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::GetFConn(BOT_FILE_M *val_(%i))", (sint)val_);
			oc = Output(ncar.carr, 2);
		}
	...
	sint FileStats(BOT_FILE_M *file_ = 0)
	{
		...
		bot_fstate nfst;
		nfst.oa_loc = file_->fst.oa_loc;
		nfst.ia_loc = file_->fst.ia_loc;

#ifdef _WIN32
		ret = _stat(str.c_str(), &nfst.filestats);
#else
		ret = stat(str.c_str(), &nfst.filestats);
#endif
		if (!ret)
		{
			MACSYM nsym;
			...
			{
				nsym.Renew(13, (u_char*)file_->path.c_str(), 0);
			}
			else
			{
				if (file_->fcon > -1 && file_->fcon < (sint)f_con.size())
				{
					boto_file = true;
					ret = UpdMTXPrio();
					...
					{
						nfst.e_loc = f_con[file_->fcon]->fstrm.end;
						u_char* nuchar = (u_char*)malloc((size_t)nfst.e_loc + 1);
						...
						if (nuchar)
						{
							...
							while (f_con[file_->fcon]->fstrm.get(*(char*)&nuchar[ct]) && ct < nfst.e_loc)
							{
								sint xc = vtool.AddValToVec(&nfst.compv, &nuchar[ct], true, true);

								if (xc > -1)
								{
									if (nfst.compv.size() > nfst.cts.size())
									{
										nfst.cts.push_back(1);
									}
									else
									{
										nfst.cts[xc]++;
							...
							memcpy((void*)&nuchar[ct], (void*)&term, sizeof(term));
							nsym.Renew(0, nuchar, file_->encode);
						}
			...
			{
				nsym.Proc();
				ret = AddSymbols(&nsym.cons);
			}

			ret = file_->fst.cmpto(&nfst);
			...
			if (ret > -1)
			{
				...
				case 1:
				{
					// change to omode and locations for io
					file_->fst.Renew(&nfst);
					break;
				}
				case 101:
				case 102:
				{
					// change to composition of file
					file_->fst.Renew(&nfst);
					break;
				}
				case 103:
				{
					...
					file_->fst.RenewStats(&nfst.filestats);
					break;
				}
				case 104:
				{
		...
		if (file_->lid > -1)
		{
			BOT_FILE nf(file_->name.c_str(), file_->type.c_str(), file_->path.c_str(), file_->omode, file_->cm, file_->encode, file_->lid, lid, file_->bto, &file_->fst);
	...
	sint BOTRemoveFile(BOT_FILE_M *xfile_ = 0)
	{
		...
		case BOT_F_CON:
		{
			...
			{
				f_con[xfile_->fcon]->fstrm.put((char)76);
				x++;

				if (x < f_con[xfile_->fcon]->fstrm.end)
				{
					f_con[xfile_->fcon]->fstrm.put((char)79);
					x++;

					if (x < f_con[xfile_->fcon]->fstrm.end)
					{
						f_con[xfile_->fcon]->fstrm.put((char)88);
						x++;
	...
	sint BOTOpenFile(BOT_FILE_M* mfile_ = 0, sint *was_open = 0, bool cine = false, bool r_to_st = false)
	{
		...
		for (uint gc = mfile_->name.length() - 1; gc; gc--)
		{
			char term = '.';

			if (!memcmp((void*)&mfile_->name[gc], (void*)&term, 1))
			{
		...
		if (mfile_->lid < 0)
		{
			std::vector<MACSYM> fvec;

			if (!mfile_->path.empty())
			{
				MACSYM fc(13, (u_char*)mfile_->path.c_str(), 0);
				fvec.push_back(fc);
			}

			if (!mfile_->name.empty())
			{
				MACSYM fc(4, (u_char*)mfile_->name.c_str(), 0);
				fvec.push_back(fc);
			}

			if (!mfile_->type.empty())
			{
				MACSYM fc(5, (u_char*)mfile_->type.c_str(), 0);
				fvec.push_back(fc);
			}

			if (!fvec.empty())
			{
				sint gc = AddCode(&fvec);
			}
		...
		if (mfile_->lid < 0)
		{
			BOT_FILE nfile(mfile_->name.c_str(), mfile_->type.c_str(), mfile_->path.c_str(), mfile_->omode, -1, mfile_->encode, -1, lid);
			...
			if (nfile.cm == BOT_F_CON)
			{
				...
				while (!done)
				{
					...
					case BOT_RTV_SINT:
					{
						...
						if (val_ != NULL)
						{
							if (f_con[fcon]->fstrm.get(*((char*)&ud)))
							{
					...
					case BOT_RTV_UCHAR:
					{
						...
						if (val_ != NULL)
						{
							...
							if (f_con[fcon]->fstrm.get(*((char*)&nc)))
							{
								memcpy((void*)val_, (void*)&nc, sizeof(nc));
							}
						}
					...
					case BOT_RTV_VSINT:
					{
						...
						if (val_ != NULL)
						{
							while (f_con[fcon]->fstrm.get(*((char*)&ud)) && val_->size() < BOTVEC_LIM)
							{
								val_->push_back(ud);
							}
						}
					...
					case BOT_RTV_VUCHAR:
					{
						...
						if (val_ != NULL)
						{
							...
							while (f_con[fcon]->fstrm.get(*((char*)&nc)) && val_->size() < BOTVEC_LIM)
							{
								val_->push_back(nc);
							}
						}
	...
	bool XEnclosedInY(std::vector<size_t>* litlocs_ = 0, c_char* statements_ = "", sint sc_loc = -1, sint x_loc = -1, char fy_char = '\0')
	{
		sint opens = 0;
				...
				if (t_->it_type < 3)
				{
					if (t_->maint)
					{
						...
						if (t_->it_type != 3 && t_->ic_type > 0)
						{
							...
							struct timespec ti;
							sint xc = clock_gettime(CLOCK_REALTIME, &ti);
							xc = t_->AddCol("LAST_MAINTAINED_NS", LLITOA((sllint)ti.tv_nsec).c_str());
#endif
					...
					else
					{
						...
						if (t_->ic_type > 0)
						{
							...
							struct timespec ti;
							sint xc = clock_gettime(CLOCK_REALTIME, &ti);
							xc = t_->AddCol("LAST_MODIFIED_NS", LLITOA(ti.tv_nsec).c_str());
#endif
	...
	sint OpenDB(BOT_DB_M *db_ = 0, sint x = -1, sint* was_open = 0, sint* fwas_locked = 0, bool r_to_st = false, bool is_verify = false)
	{
		...
		BOT_FILE_M nfilem(db_->name.c_str(), ".db", "", x, 1, -1, -1, lid);
		sint xc = GetInVec(&nfilem, MTX_FO, false, true);

		if (xc < 0)
		{
			BOT_FILE nfile(db_->name.c_str(), ".db", "", x, 1, -1, -1, lid);
					...
					case SQLITE_ROW:
					{
						...
						for (uint iter = 0; iter < qval_->fcols.size(); iter++)
						{
							...
							if (ct == SQLITE_INTEGER)
							{
								...
								case BOT_RTV_CHAR:
								{
									char* inv = reinterpret_cast<char*>(qval_->rblk);
									*inv = (char)sqlite3_column_int(ppstmt, ret);
									break;
								}
								case BOT_RTV_UINT:
								{
								...
								case BOT_RTV_VCHAR:
								{
									std::vector<char>* inv = reinterpret_cast<std::vector<char>*>(qval_->rblk);
									inv->push_back((char)sqlite3_column_int(ppstmt, ret));
									break;
								}
								case BOT_RTV_VUINT:
								{
							...
							else if (ct == SQLITE_FLOAT)
							{
								...
								{
									char* inv = reinterpret_cast<char*>(qval_->rblk);
									*inv = (char)sqlite3_column_double(ppstmt, ret);
									break;
								}
								case BOT_RTV_UINT:
								{
								...
								{
									std::vector<char>* inv = reinterpret_cast<std::vector<char>*>(qval_->rblk);
									inv->push_back((char)sqlite3_column_double(ppstmt, ret));
									break;
								}
								case BOT_RTV_VUINT:
								{
							...
							else if (ct == SQLITE_TEXT)
							{
								...
								{
									sint* inv = reinterpret_cast<sint*>(qval_->rblk);
									*inv = atoi(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									break;
								}
								case BOT_RTV_CHAR:
								{
									...
									{
										char* inv = reinterpret_cast<char*>(qval_->rblk);
										*inv = (char)*sqlite3_column_text(ppstmt, ret);
									}
									break;
								}
								case BOT_RTV_UINT:
								{
									uint* inv = reinterpret_cast<uint*>(qval_->rblk);
									*inv = (uint)atoi(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									break;
								}
								case BOT_RTV_UCHAR:
								{
								...
								{
									std::string* inv = reinterpret_cast<std::string*>(qval_->rblk);
									inv->append(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									break;
								}
								case BOT_RTV_REAL:
								{
									float* inv = reinterpret_cast<float*>(qval_->rblk);
									*inv = (float)atof(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									break;
								}
								case BOT_RTV_VSINT:
								{
									std::vector<sint>* inv = reinterpret_cast<std::vector<sint>*>(qval_->rblk);
									sint i = atoi(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									inv->push_back(i);
									break;
								}
								case BOT_RTV_VCHAR:
								{
									...
									{
										std::vector<char>* inv = reinterpret_cast<std::vector<char>*>(qval_->rblk);
										inv->push_back((char)*sqlite3_column_text(ppstmt, ret));
									}
									break;
								}
								case BOT_RTV_VUINT:
								{
									std::vector<uint>* inv = reinterpret_cast<std::vector<uint>*>(qval_->rblk);
									sint i = (uint)atoi(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									inv->push_back(i);
									break;
								}
								case BOT_RTV_VUCHAR:
								...
								case BOT_RTV_VSTR:
								{
									std::vector<std::string>* inv = reinterpret_cast<std::vector<std::string>*>(qval_->rblk);
									std::string nstr;
									nstr.append(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									inv->push_back(nstr);
									break;
								}
								case BOT_RTV_VREAL:
								{
									std::vector<float>* inv = reinterpret_cast<std::vector<float>*>(qval_->rblk);
									float xval = (float)atof(reinterpret_cast<const char*>(sqlite3_column_text(ppstmt, ret)));
									inv->push_back(xval);
									break;
								}
								default:
	...
	sint VerifyDatabases()
	{
		...
		{
			BOT_FILE_M ndb(nms[x].c_str(), ".db", "", 3, 1, -1, -1, lid);
			sint of = -1;
	...
	sint GetSymbols(std::vector <MICSYM> *syms_ = 0, c_char x_cond_[] = 0, bool maint = false)
	{
		...
		for (size_t nsiz = 0; nsiz < syms_->size(); nsiz++)
		{
			if (syms_->at(nsiz).id > -2)
			{
				...
				{
					t.cspec = 0;
					char dval[(sizeof(syms_->at(nsiz).sym) * 2) + 4]{ 0 };
	...
	sint AddSymbols(std::vector <MICSYM> *syms_ = 0)
	{
		...
		for (size_t x = 0; x < eles.size(); x++)
		{
			...
			else
			{
				...
				char str_[(sizeof(syms_->at(eles[x]).sym) * 2) + 4]{ 0 };
		...
		for (size_t x = 0; x < eles.size(); x++)
		{
			...
			for (size_t uc = 0; uc < codes_->at(eles[x]).usages.size(); uc++)
			{
				...
				if (dbs.lid < 0)
				{
					...
					if (p > -1)
					{
						t.Renew(codes_->at(eles[x]).ud_last_maintained, 1, "code", bot_strt.code_tablenames[codes_->at(eles[x]).usages[uc]], 1);
	...
	sint PushToDBList(BOT_DB *dbs_ = 0)
	{
		...
		if (p > -1)
		{
			...
			case 2:
			{
				...
				{
					ret = vtool.AddValToVec(&dbs_->tables, bot_strt.code_tablenames[t], false, false);
					std::vector<std::vector<std::string>> tcols;
					dbs_->columns.push_back(tcols);
					std::vector<std::string> cols, colss;
					dbs_->columns[t].push_back(colss);
	...
	sint CheckPrintFs()
	{
		...
		BOT_FILE_M xfile("litebot", ".h", pth.c_str(), BOT_FILE_READ, 0, -1, -1, lid);
		sint of = -1;
		...
		if (lx > -1)
		{
			std::string pth;
			BOTPath(&pth, BOT_MOD_NM);
			xfile.Renew("litebot", ".cpp", pth.c_str(), BOT_FILE_READ, 0, -1, -1, lid);
			sint of = -1;
	...
	sint ProcErrLog(sint opt = -1, c_char* in_file_p = "", c_char* in_file_n = "", c_char* in_file_t = "", c_char* rampid = "", c_char* insp_type = "", c_char* date_ = "", c_char* eid_ = "", c_char* toval_ = "")
	{
		...
		*/
		...
		BOT_FILE_M xfile(in_file_n, in_file_t, in_file_p, BOT_FILE_READ, 0, -1, -1, lid);
		of = -1;
		sint lx = BOTOpenFile(&xfile, &of);
		...
		if (lx > -1)
		{
			...
			std::string pth;
			BOTPath(&pth, BOT_MOD_NM);
			xfile.Renew(nm_str.c_str(), ".txt", pth.c_str(), BOT_FILE_APPEND, 0, -1, -1, lid);
			of = -1;
	...
	sint AddDlrCode(c_char* to_vin = "", c_char* in_file = "")
	{
		...
		std::string pth;
		BOTPath(&pth, BOT_MOD_NM);
		BOT_FILE_M xfile(in_file, ".txt", pth.c_str(), BOT_FILE_OVERW, 0, -1, -1, lid);
		sint of = -1;
		sint lx = BOTOpenFile(&xfile, &of);
