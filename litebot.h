...
#ifdef _WIN32
#include <windows.h>
#include "../win_LITEBotV/_includes/dirent.h"
#include "../win_LITEBotV/_includes/sched.h"
#include "../win_LITEBotV/_includes/semaphore.h"
#include "../win_LITEBotV/_includes/pthread.h"
#include "../win_LITEBotV/_includes/sqlite3.h"
...
#endif
#ifndef BOT_LOG_PATH
#define BOT_LOG_PATH "c:\\dev-litebot\\win_LITEBotV\\_logs\\"
#else
...
#endif
#ifndef BOT_MOD_PATH
#define BOT_MOD_PATH "c:\\dev-litebot\\win_LITEBotV\\_mods\\"
#else
...
public:
	...
	void Clear() { locked = false; w_mtx = lid = -1; if (!h_mems.empty()) { h_mems.clear(); } if (!h_mems.empty()) { h_mems.clear(); } }

	HELD_MTX(sint nw_mtx = -1, sint nlid = -1, bool nlocked = false) { w_mtx = nw_mtx; locked = nlocked; lid = nlid; };
...
public:
	...
	void Renew(lok_req* nreq = 0)
	{
		...
		if (!nreq->hmtx.empty())
		{
			for (size_t csiz = 0; csiz < nreq->hmtx.size(); csiz++)
			{
				if (nreq->hmtx[csiz].locked || !nreq->hmtx[csiz].h_mems.empty() || !nreq->hmtx[csiz].m_mems.empty())
				{
					HELD_MTX nmtx(nreq->hmtx[csiz].w_mtx, lid, nreq->hmtx[csiz].locked);
					size_t hsiz = hmtx.size();
					hmtx.push_back(nmtx);

					for (size_t siz = 0; siz < nreq->hmtx[csiz].h_mems.size(); siz++)
					{
						hmtx[hsiz].h_mems.push_back(nreq->hmtx[csiz].h_mems[siz]);
					}
					for (size_t siz = 0; siz < nreq->hmtx[csiz].m_mems.size(); siz++)
					{
						hmtx[hsiz].m_mems.push_back(nreq->hmtx[csiz].m_mems[siz]);
					}
				}
			}
		}
	...
	lok_req(sint nele = -1, sint wmtx = -1, sint nid = -1, sint nmeta = -1, std::vector<HELD_MTX>* cur_ = 0)
	{
		...
		if (cur_)
		{
			for (size_t csiz = 0; csiz < cur_->size(); csiz++)
			{
				if (cur_->at(csiz).locked || !cur_->at(csiz).h_mems.empty() || !cur_->at(csiz).m_mems.empty())
				{
					HELD_MTX nmtx;
					nmtx.w_mtx = cur_->at(csiz).w_mtx;
					nmtx.locked = cur_->at(csiz).locked;

					for (size_t siz = 0; siz < cur_->at(csiz).h_mems.size(); siz++)
					{
						nmtx.h_mems.push_back(cur_->at(csiz).h_mems[siz]);
					}
					for (size_t siz = 0; siz < cur_->at(csiz).m_mems.size(); siz++)
					{
						nmtx.m_mems.push_back(cur_->at(csiz).m_mems[siz]);
					}
					hmtx.push_back(nmtx);
				}
			}
...
public:
	...
	sint CheqReqs(sint fid = -1, std::vector<lok_req>* nreqs_ = 0, std::vector<HELD_MTX>* cur_ = 0)
	{
		if (!nreqs_ || !cur_)
		{
			return -1;
		}
		for (uint x = 0; x < cur_->size(); x++)
		{
			for (uint z = 0; z < reqs.size(); z++)
			{
				if (reqs[z].fid != fid)
				{
					if (reqs[z].gmtx == cur_->at(x).w_mtx)
					{
						if (!reqs[z].is_meta)
						{
							for (uint y = 0; y < cur_->at(x).h_mems.size(); y++)
							{
								if (cur_->at(x).h_mems[y] == reqs[z].ele)
								{
									nreqs_->push_back(reqs[z]);
								}
							}
						}
						else
						{
							for (uint y = 0; y < cur_->at(x).m_mems.size(); y++)
							{
								if (cur_->at(x).m_mems[y] == reqs[z].ele)
								{
									nreqs_->push_back(reqs[z]);
								}
							}
	...
	sint Req(lok_req* nreq = 0, bool wait = false)
	{
		if (!nreq)
		{
			return -1;
		...
		nreq->at_time = std::chrono::steady_clock::now();
		uint f_ele = UINT_MAX;
		uint siz = 0;
		...
		while (siz < reqs.size())
		{
			...
			else if (nreq->fid == reqs[siz].fid)
			{
				if (nreq->gmtx == reqs[siz].gmtx && nreq->ele == reqs[siz].ele && nreq->is_meta == reqs[siz].is_meta)
				{
					nreq->at_time = reqs[siz].at_time;
					reqs[siz].Clear();
					reqs[siz].Renew(nreq);
					reqs[siz].lid = siz;
					siz = reqs.size();
		...
		if (siz == reqs.size())
		{
			...
			{
				reqs[f_ele].Renew(nreq);
			}
		}
		for (siz = 0; siz < reqs.size(); siz++)
		{
			if (reqs[siz].lid > -1 && nreq->fid != reqs[siz].fid)
			{
				for (uint hsiz = 0; hsiz < reqs[siz].hmtx.size(); hsiz++)
				{
					if (reqs[siz].hmtx[hsiz].w_mtx == nreq->gmtx)
					{
						if (nreq->is_meta > 0)
						{
							for (uint dsiz = 0; dsiz < reqs[siz].hmtx[hsiz].m_mems.size(); dsiz++)
							{
								if (reqs[siz].hmtx[hsiz].m_mems[dsiz] == nreq->ele)
								{
									if (!wait)
									{
										return -1;
									...
									{
										for (uint _hsiz = 0; _hsiz < nreq->hmtx.size(); _hsiz++)
										{
											if (nreq->hmtx[_hsiz].w_mtx == reqs[siz].gmtx)
											{
												if (reqs[siz].is_meta > 0)
												{
													for (uint _dsiz = 0; _dsiz < nreq->hmtx[_hsiz].m_mems.size(); _dsiz++)
													{
														if (nreq->hmtx[_hsiz].m_mems[_dsiz] == reqs[siz].ele)
														{
															if (nreq->at_time > reqs[siz].at_time)
															{
																// here we set nreq to the mutex values we need to unlock so we don't have to lose all of them.
																nreq->gmtx = reqs[siz].gmtx;
																nreq->ele = reqs[siz].ele;
																nreq->is_meta = reqs[siz].is_meta;
																nreq->fid = reqs[siz].fid;
																return -1; //rest and release nreq too
															}
															else
															{
																return 1; //rest and keep your locks, the other thread should release the lock on our request
															}
														}
													}
												}
												...
												{
													for (uint _dsiz = 0; _dsiz < nreq->hmtx[_hsiz].h_mems.size(); _dsiz++)
													{
														if (nreq->hmtx[_hsiz].h_mems[_dsiz] == reqs[siz].ele)
														{
															if (nreq->at_time > reqs[siz].at_time)
															{
																nreq->gmtx = reqs[siz].gmtx;
						...
						{
							for (uint dsiz = 0; dsiz < reqs[siz].hmtx[hsiz].h_mems.size(); dsiz++)
							{
								if (reqs[siz].hmtx[hsiz].h_mems[dsiz] == nreq->ele)
								{
									if (!wait)
									{
										return -1;
	...
	sint Withd(sint nele = -1, sint wmtx = -1, sint nid = -1, sint nmeta = -1)
	{
		if (nele < 0 || wmtx < 0 || nid < 0 || nmeta < 0)
		{
			return -1;
		}
		...
		for (uint siz = 0; siz < reqs.size(); siz++)
		{
			if (reqs[siz].gmtx == wmtx && reqs[siz].ele == nele && reqs[siz].fid == nid && reqs[siz].is_meta == nmeta)
			{
				reqs[siz].Clear();
				ret = 0;

				if (siz == (sint)reqs.size() - 1)
				{
					reqs.pop_back();
				}
			}
...
public:
	...
	void Clear() { omode = lok = c_by = lid = exists = -1; if (!name.empty()) { name.clear(); } if (!tables.empty()) { tables.clear(); } if (!columns.empty()) { columns.clear(); } }
	void Renew(cchar* nstr = "", sint ntype = -1, sint nomode = -1) { Clear(); omode = nomode; type = ntype; if (strlen(nstr) < 128) { name.append(nstr); } }

	BOT_DB(cchar* nstr = "", sint ntype = -1) { omode = lok = c_by = lid = exists = -1; type = ntype; if (strlen(nstr) < 128) { name.append(nstr); } };
...
public:
	...
	BOT_CLIENT(cchar* nstr = "") { lok = -1; nameset = loggedin = changename = updatepriv = updatelogin = false; totallogins = failed_logins = priv = 0; lid = c_by = -1; size_t siz = strlen(nstr); if (siz < 128) { name.append(nstr); if (siz) { nameset = true; } } };
...
public:
	...
	sint RenderCons(uchar sym[], sint encoding = -1)
	{
		...
		if (sizeof(sym))
		{
			...
			for ( size_t rc = 0; rc < sizeof(sym); rc++)
			{
...
typedef struct parse_symbol_struct
{
	bool confirmed;
	sint id;
	std::string sym;
	...
	sint order_open;
	sint encloseris;
	std::vector<std::string> constituent_to;
	std::vector<std::string> constituent_types;
	std::vector<sint> usages;

	parse_symbol_struct() { confirmed = false; id = loc = enc_loc = closer_loc = open = order_open = encloseris = -1; };
...
typedef struct symbol_struct
{
	...
	symbol_struct() { id = loc = enc_loc = closer_loc = open = order_open = encloseris = usage = -1; };

}SYMBOL_STRUCT;

typedef struct macro_symbol_struct
{
	sint id;
	std::string sym;
	std::string enc;
	...
	sint encloseris;
	sint usage;
	std::vector<SYMBOL_STRUCT> constituents;

	macro_symbol_struct() { id = loc = enc_loc = closer_loc = open = encloseris = usage = -1; };

}MACSS;

typedef struct basic_idents
{
	std::vector<std::string> specs;
	std::vector<std::vector<PARSS>> idents;

	basic_idents() {};
	/*std::vector<PARSS> enders; //0
	std::vector<PARSS> consonants; //1
	std::vector<PARSS> vowels; //2
	std::vector<PARSS> numbers; //3
	std::vector<PARSS> literal_encs; //4
	std::vector<PARSS> order_encs;//5
	std::vector<PARSS> terminators;//6
	std::vector<PARSS> assignment;//7
	//std::vector<PARSS> mystery; //8 let's not init this vector as it could be huge. instead get these in SeparateSymbolsII from code or maybe common syms.*/
}BIDENTS;

typedef struct language_idents
{
	std::vector<std::string> u_case_consonants;
	std::vector<std::string> l_case_consonants;
	std::vector<std::string> o_case_consonants;
	std::vector<std::string> u_case_vowels;
	std::vector<std::string> l_case_vowels;
	std::vector<std::string> o_case_vowels;
...
typedef struct word_struct
{
	...
	bool ud_synonyms;
	std::vector<long double> number_equiv;
	bool ud_number_equiv;
	bool as_name;
	bool ud_as_name;
	bool as_title;
	bool ud_as_title;
	std::vector<std::string> interchangeables;
	bool ud_interchangeables;
	std::vector<std::string> conjunction_of;
	bool ud_conjunction_of;
	std::vector<std::string> common_preceeding;
	bool ud_common_preceeding;
	std::vector<std::string> common_following;
	bool ud_common_following;
	std::vector<std::string> history_preceeding;
	bool ud_history_preceeding;
	std::vector<std::string> history_following;
	bool ud_history_following;
	std::vector<std::string> definitions;
	bool ud_definitions;
	std::vector<std::string> examples;
	bool ud_examples;
	std::vector<std::string> alt_lang_equivs;
	bool ud_alt_lang_equivs;
	std::string assoc_sound;
	bool ud_assoc_sound;
	std::string assoc_img;
	bool ud_assoc_img;
	bool profanity;
	bool ud_profanity;
	bool as_code;
	bool ud_as_code;
	std::vector<sint> used_as_code;
	bool ud_used_as_code;
	std::vector<std::string> usages;
	bool ud_usages;
	sllint last_used;
	bool ud_last_used;
	std::vector<std::string> last_used_as;
	bool ud_last_used_as;
	sint last_maintained;
	bool ud_last_maintained;
	sint last_modified;
	bool ud_last_modified;
	...
	{
		id = -1;
		last_maintained = last_modified = last_maintained_ns = last_modified_ns = used = 0;
		last_used = 0;
		ud_id = ud_used = ud_word = ud_languages = ud_functions = ud_rootwords = ud_phoenetics = ud_syllables = ud_duplicate = ud_syntax = ud_synonyms = ud_number_equiv = ud_as_name = ud_as_title = ud_interchangeables = ud_conjunction_of = ud_common_preceeding = ud_common_following = ud_history_preceeding = ud_history_following = ud_definitions = ud_examples = ud_alt_lang_equivs = ud_assoc_sound = ud_assoc_img = ud_profanity = ud_as_code = ud_used_as_code = ud_usages = ud_last_used = ud_last_used_as = ud_last_maintained = ud_last_modified = ud_last_maintained_ns = ud_last_modified_ns = false;
	};
...
typedef struct code_struct
{
	...
	sint used;
	bool ud_history_preceeding;
	std::vector<uchar> history_preceeding;
	bool ud_history_following;
	std::vector<uchar> history_following;
	bool ud_alters;
	std::vector<std::vector<uchar>> alters;
	bool ud_encodings;
	std::vector<uint> encodings;
	bool ud_usages;
	std::vector<uint> usages;
	bool ud_used_as;
	sint used_as;
	bool ud_languages;
	std::vector<std::string> languages;
	...
	code_struct() { used_as = id = -1; last_maintained = last_modified = last_maintained_ns = last_modified_ns = used = 0; ud_encodings = ud_id = ud_used = ud_alters = ud_usages = ud_used_as = ud_languages = ud_functions = ud_last_maintained = ud_last_modified = ud_last_maintained_ns = ud_last_modified_ns = false; };
}CODE_STRUCT;
...
typedef struct input_struct
{
	_mtx *mtx_;
	std::string *istr_;
	std::vector<SYMBOL_STRUCT> *SymVec_;
	std::string *ol_str_;

}INPUT_STRUCT;

typedef struct p_i_s
{
	std::string *istr_;
	std::vector<MICSYM> *MSymVec_;
	std::vector<PARSS> *SymVec_;
	std::vector<SYMBOL_STRUCT> *AltSymsVec_;
	std::string *ol_str_;

	p_i_s() { MSymVec_ = 0; SymVec_ = 0; AltSymsVec_ = 0; ol_str_ = 0; };
...
public:
	...
	sint comp_t_struct(BOT_STMT* valI = 0, BOT_STMT* valII = 0)
	{
		...
		for (Isiz = 0; Isiz < valI->cols.size(); Isiz++)
		{
			...
			else if (!strcmp(valI->cols[Isiz].c_str(), "USED"))
			{
				igc = (sint)Isiz;
			}
			else
			{
				if (strcmp(valI->cols[Isiz].c_str(), valII->cols[Isiz].c_str())) { Isiz = valI->cols.size(); }
			}
		}
	...
	void VecPop(std::vector<carr_2> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_4> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_8> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_16> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_32> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_64> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_128> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_256> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_512> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_1024> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_2048> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<carr_4096> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<uchar> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<void*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<sint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<slint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<sllint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<uint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<ulint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<ullint> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<float> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<cchar*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<std::string> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_DB> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_CLIENT> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_LOGDET> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_FILE> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<TINFO> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<HELD_MTX> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<lok_req> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_DB*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_CLIENT*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_LOGDET*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_FILE*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<TINFO*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<HELD_MTX*> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_DB_M> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_CLIENT_M> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_LOGDET_M> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_FILE_M> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<TINFO_M> *vec_) { vec_->pop_back(); }
	void VecPop(std::vector<BOT_STMT> *vec_) { vec_->pop_back(); }
	
	void BuildEmptyVec(std::vector<carr_2> *vec_, size_t siz) { carr_2 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_4> *vec_, size_t siz) { carr_4 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_8> *vec_, size_t siz) { carr_8 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_16> *vec_, size_t siz) { carr_16 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_32> *vec_, size_t siz) { carr_32 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_64> *vec_, size_t siz) { carr_64 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_128> *vec_, size_t siz) { carr_128 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_256> *vec_, size_t siz) { carr_256 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_512> *vec_, size_t siz) { carr_512 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_1024> *vec_, size_t siz) { carr_1024 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_2048> *vec_, size_t siz) { carr_2048 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<carr_4096> *vec_, size_t siz) { carr_4096 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } }
	void BuildEmptyVec(std::vector<uchar> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<void*> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<sint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<slint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<sllint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<uint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<ulint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<ullint> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<float> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(0); } }
	void BuildEmptyVec(std::vector<cchar*> *vec_, size_t siz) { while (vec_->size() < siz) { vec_->push_back(""); } }
	void BuildEmptyVec(std::vector<std::string> *vec_, size_t siz) { while (vec_->size() < siz) { std::string str; str.clear(); vec_->push_back(str); } }
	void BuildEmptyVec(std::vector<BOT_DB> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_DB val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_CLIENT> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_CLIENT val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_LOGDET> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_LOGDET val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_FILE> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_FILE val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<TINFO> *vec_, size_t siz) { while (vec_->size() < siz) { TINFO val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<HELD_MTX> *vec_, size_t siz) { while (vec_->size() < siz) { HELD_MTX val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<lok_req> *vec_, size_t siz) { while (vec_->size() < siz) { lok_req val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_DB_M> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_DB_M val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_CLIENT_M> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_CLIENT_M val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_LOGDET_M> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_LOGDET_M val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_FILE_M> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_FILE_M val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<TINFO_M> *vec_, size_t siz) { while (vec_->size() < siz) { TINFO_M val; vec_->push_back(val); } }
	void BuildEmptyVec(std::vector<BOT_STMT> *vec_, size_t siz) { while (vec_->size() < siz) { BOT_STMT val; vec_->push_back(val); } }
	
	void FixVectorSize(std::vector<carr_2> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_2 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_4> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_4 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_8> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_8 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_16> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_16 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_32> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_32 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_64> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_64 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_128> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_128 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_256> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_256 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_512> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_512 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_1024> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_1024 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_2048> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_2048 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<carr_4096> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { carr_4096 ncar; while (vec_->size() < siz) { vec_->push_back(ncar); } } else {} }
	void FixVectorSize(std::vector<uchar> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<void*> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<sint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<slint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<sllint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<uint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<ulint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<ullint> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<float> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(0); } } else {} }
	void FixVectorSize(std::vector<cchar*> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { vec_->push_back(""); } } else {} }
	void FixVectorSize(std::vector<std::string> *vec_, size_t siz) { if (vec_->size() > siz) { while (vec_->size() > siz) { vec_->pop_back(); } } else if (vec_->size() < siz) { while (vec_->size() < siz) { std::string str; str.clear(); vec_->push_back(str); } } else {} }
	
	sint VectorSize(std::vector<carr_2> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_4> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_8> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_16> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_32> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_64> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_128> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_256> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_512> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_1024> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_2048> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<carr_4096> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<uchar> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<void*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<sint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<slint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<sllint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<uint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<ulint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<ullint> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<float> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<cchar*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<std::string> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_DB> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_CLIENT> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_LOGDET> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_FILE> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<TINFO> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<HELD_MTX> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<lok_req> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_DB*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_CLIENT*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_LOGDET*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_FILE*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<TINFO*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<HELD_MTX*> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_DB_M> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_CLIENT_M> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_LOGDET_M> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_FILE_M> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<TINFO_M> *vec_) { return (sint)vec_->size(); }
	sint VectorSize(std::vector<BOT_STMT> *vec_) { return (sint)vec_->size(); }
	
	sint ValInVec(std::vector<carr_2> *vec_, char val[2], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4> *vec_, char val[4], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_8> *vec_, char val[8], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_16> *vec_, char val[16], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_32> *vec_, char val[32], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_64> *vec_, char val[64], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_128> *vec_, char val[128], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_256> *vec_, char val[256], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_512> *vec_, char val[512], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_1024> *vec_, char val[1024], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_2048> *vec_, char val[2048], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4096> *vec_, char val[4096], sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_2> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_8> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_16> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_32> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_64> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_128> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_256> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_512> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_1024> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_2048> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4096> *vec_, cchar* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).carr, val)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_2> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_8> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_16> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_32> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_64> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_128> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_256> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_512> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_1024> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_2048> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<carr_4096> *vec_, void* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { cchar* val_ = reinterpret_cast<cchar*>(val); if (!strcmp(vec_->at(siz).carr, val_)) { vn = siz; siz = -1; } siz--; } } return vn; }
	...
	sint ValInVec(std::vector<uchar> *vec_, const sint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } const size_t valsiz = sizeof(val)+1; uchar val_[valsiz]; memcpy_s((void*)&val_[siz], valsiz, (void*)&val, valsiz); if (forw) { while (siz < (sint)vec_->size()) { if (vec_->at(siz) == *val_) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (vec_->at(siz) == *val_) { vn = siz; siz = -1; } siz--; } } return vn; }
	...
	sint ValInVec(std::vector<cchar*> *vec_, sint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%i", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, slint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%li", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, sllint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%lli", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, uint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%u", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, ulint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%lu", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, ullint val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[21]{ 0 }; sint siz = snprintf(mid, 21, "%llu", val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	sint ValInVec(std::vector<cchar*> *vec_, float* val, sint e_from = 0, bool forw = true) { if (!vec_) { return -1; } sint vn = -1; char mid[32]{ 0 }; sint siz = snprintf(mid, 32, "%0.3f", *val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	...
	sint ValInVec(std::vector<std::string> *vec_ = 0, float* val = 0, sint e_from = 0, bool forw = true) { if (!vec_ || !val) { return -1; } sint vn = -1; char mid[32]{ 0 }; sint siz = snprintf(mid, 32, "%0.3f", *val); siz = e_from; if (e_from < 0) { siz = ((sint)vec_->size() + e_from); } if (forw) { while (siz < (sint)vec_->size()) { if (!strcmp(vec_->at(siz).c_str(), mid)) { vn = siz; siz = (sint)vec_->size(); } siz++; } } else { while (siz > -1) { if (!strcmp(vec_->at(siz).c_str(), mid)) { vn = siz; siz = -1; } siz--; } } return vn; }
	...
	sint UseEmptyEle(std::vector<carr_2> *vec_, char val[2]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_4> *vec_, char val[4]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_8> *vec_, char val[8]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_16> *vec_, char val[16]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_32> *vec_, char val[32]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_64> *vec_, char val[64]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_128> *vec_, char val[128]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_256> *vec_, char val[256]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_512> *vec_, char val[512]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_1024> *vec_, char val[1024]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_2048> *vec_, char val[2048]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<carr_4096> *vec_, char val[4096]) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).carr[0] == '\0') { strcpy_s(vec_->at(siz).carr, val); vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	...
	sint UseEmptyEle(std::vector<uchar> *vec_, uchar* val) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (!vec_->at(siz)) { vec_->at(siz) = *val; vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<void*> *vec_, void* val) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (!vec_->at(siz)) { vec_->at(siz) = val; vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	...
	sint UseEmptyEle(std::vector<float> *vec_, float* fp) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == 0) { vec_->at(siz) = *fp; vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	sint UseEmptyEle(std::vector<cchar*> *vec_, void* vp) { if (!vec_) { return -1; } sint vn = -1; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz), " ") || !strlen(vec_->at(siz))) { vec_->at(siz) = 0; cchar* mid = reinterpret_cast<cchar*>(vp); vec_->at(siz) = mid; vn = (sint)siz; siz = vec_->size(); } siz++; } return vn; }
	...
	sint AddValToVec(std::vector<carr_2> *vec_, char val[2], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_2 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_4> *vec_, char val[4], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_4 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_8> *vec_, char val[8], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_8 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_16> *vec_, char val[16], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_16 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_32> *vec_, char val[32], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_32 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_64> *vec_, char val[64], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_64 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_128> *vec_, char val[128], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_128 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_256> *vec_, char val[256], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_256 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_512> *vec_, char val[512], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_512 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_1024> *vec_, char val[1024], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_1024 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_2048> *vec_, char val[2048], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_2048 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	sint AddValToVec(std::vector<carr_4096> *vec_, char val[4096], bool check, bool use_empty) { if (!vec_ || !val) { return -1; } sint viv = -1; if (check) { viv = ValInVec(vec_, val); if (viv > -1) { return viv; } } if (use_empty) { viv = UseEmptyEle(vec_, val); if (viv > -1) { return viv; } } viv = (sint)vec_->size(); if (viv > BOTVEC_LIM) { return -1; } carr_4096 ncar; strcpy_s(ncar.carr, val); vec_->push_back(ncar); return viv; }
	...
	char *GetValInVec(std::vector<carr_2> *vec_, sint ele) { if (!vec_) { return ""; } char ret[2]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_4> *vec_, sint ele) { if (!vec_) { return ""; } char ret[4]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_8> *vec_, sint ele) { if (!vec_) { return ""; } char ret[8]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_16> *vec_, sint ele) { if (!vec_) { return ""; } char ret[16]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_32> *vec_, sint ele) { if (!vec_) { return ""; } char ret[32]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_64> *vec_, sint ele) { if (!vec_) { return ""; } char ret[64]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_128> *vec_, sint ele) { if (!vec_) { return ""; } char ret[128]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_256> *vec_, sint ele) { if (!vec_) { return ""; } char ret[256]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_512> *vec_, sint ele) { if (!vec_) { return ""; } char ret[512]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_1024> *vec_, sint ele) { if (!vec_) { return ""; } char ret[1024]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_2048> *vec_, sint ele) { if (!vec_) { return ""; } char ret[2048]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
	char *GetValInVec(std::vector<carr_4096> *vec_, sint ele) { if (!vec_) { return ""; } char ret[4096]{ 0 }; if (ele > -1 && ele < (sint)vec_->size()) { strcpy_s(ret, vec_->at(ele).carr); } return ret; }
/*	uchar GetValInVecUC(std::vector<uchar> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }*/
	uchar GetValInVecUC(std::vector<void*> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, vec_->at(ele), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<sint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<slint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<sllint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<uint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<ulint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<ullint> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<float> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<cchar*> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = sizeof(vec_->at(ele)); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uchar GetValInVecUC(std::vector<std::string> *vec_, sint ele) { uchar ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { cchar* cret = vec_->at(ele).c_str(); const size_t inv_siz = sizeof(&cret); uchar ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&cret), inv_siz); ret = *ret_; } return ret; }
	void* GetValInVecV_(std::vector<uchar> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<void*> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	void* GetValInVecV_(std::vector<sint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<slint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<sllint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<uint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<ulint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<ullint> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<float> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<cchar*> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = reinterpret_cast<void*>(&vec_->at(ele)); } return ret; }
	void* GetValInVecV_(std::vector<std::string> *vec_, sint ele) { void* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { cchar* cret = vec_->at(ele).c_str(); ret = reinterpret_cast<void*>(&cret); } return ret; }
	sint GetValInVecI(std::vector<uchar> *vec_, sint ele) { sint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); sint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	slint GetValInVecLI(std::vector<uchar> *vec_, sint ele) { slint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); slint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	sllint GetValInVecLLI(std::vector<uchar> *vec_, sint ele) { sllint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); sllint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	uint GetValInVecU(std::vector<uchar> *vec_, sint ele) { uint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); uint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	ulint GetValInVecLU(std::vector<uchar> *vec_, sint ele) { ulint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); ulint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	ullint GetValInVecLLU(std::vector<uchar> *vec_, sint ele) { ullint ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { const size_t inv_siz = (sizeof(vec_->at(ele))); ullint ret_[inv_siz]; memcpy_s((void*)ret_, inv_siz, reinterpret_cast<void*>(&vec_->at(ele)), inv_siz); ret = *ret_; } return ret; }
	sint GetValInVecI(std::vector<sint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<sint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<sint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<slint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<slint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<slint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<sllint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<sllint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<sllint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<uint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<uint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<uint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<ulint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<ulint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<ulint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<ullint> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<ullint> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<ullint> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<uint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<uint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<uint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<ulint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<ulint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<ulint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<ullint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<ullint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<ullint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<sint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<sint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<sint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<slint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<slint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<slint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<sllint> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<sllint> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<sllint> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	//void* GetValInVecV_(std::vector<float> *vec_, sint ele) { void* ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (void*)&vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<float> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sint)vec_->at(ele); } } return ret; }
	slint GetValInVecLI(std::vector<float> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (slint)vec_->at(ele); } } return ret; }
	sllint GetValInVecLLI(std::vector<float> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (sllint)vec_->at(ele); } } return ret; }
	uint GetValInVecU(std::vector<float> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (uint)vec_->at(ele); } } return ret; }
	ulint GetValInVecLU(std::vector<float> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ulint)vec_->at(ele); } } return ret; }
	ullint GetValInVecLLU(std::vector<float> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (ullint)vec_->at(ele); } } return ret; }
	sint GetValInVecI(std::vector<cchar*> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele)); } } return ret; }
	slint GetValInVecLI(std::vector<cchar*> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele)); } } return ret; }
	sllint GetValInVecLLI(std::vector<cchar*> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = _atoi64(vec_->at(ele)); } } return ret; }
	uint GetValInVecU(std::vector<cchar*> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele)); } } return ret; }
	ulint GetValInVecLU(std::vector<cchar*> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele)); } } return ret; }
	ullint GetValInVecLLU(std::vector<cchar*> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = _atoi64(vec_->at(ele)); } } return ret; }
	sint GetValInVecI(std::vector<std::string> *vec_, sint ele) { sint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele).c_str()); } } return ret; }
	slint GetValInVecLI(std::vector<std::string> *vec_, sint ele) { slint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele).c_str()); } } return ret; }
	sllint GetValInVecLLI(std::vector<std::string> *vec_, sint ele) { sllint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = _atoi64(vec_->at(ele).c_str()); } } return ret; }
	uint GetValInVecU(std::vector<std::string> *vec_, sint ele) { uint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele).c_str()); } } return ret; }
	ulint GetValInVecLU(std::vector<std::string> *vec_, sint ele) { ulint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = atoi(vec_->at(ele).c_str()); } } return ret; }
	ullint GetValInVecLLU(std::vector<std::string> *vec_, sint ele) { ullint ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = _atoi64(vec_->at(ele).c_str()); } } return ret; }
	float GetFValInVec(std::vector<uchar> *vec_, sint ele) { float ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { float* fp_ = reinterpret_cast<float*>(&vec_->at(ele)); ret = *fp_; } return ret; }
	float GetFValInVec(std::vector<void*> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { float* fp_ = reinterpret_cast<float*>(vec_->at(ele)); ret = *fp_; } } return ret; }
	float GetFValInVec(std::vector<float> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<sint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<slint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<sllint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<uint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<ulint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<ullint> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)vec_->at(ele); } } return ret; }
	float GetFValInVec(std::vector<cchar*> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)atof(vec_->at(ele)); } } return ret; }
	float GetFValInVec(std::vector<std::string> *vec_, sint ele) { float ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = (float)atof(vec_->at(ele).c_str()); } } return ret; }
	cchar* GetCValInVec(std::vector<uchar> *vec_, sint ele) { cchar* ret = 0; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = reinterpret_cast<cchar*>(&vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<void*> *vec_, sint ele) { cchar* ret = ""; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = reinterpret_cast<cchar*>(vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<sint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%i", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<slint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%li", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<sllint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%lli", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<uint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%u", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<ulint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%lu", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<ullint> *vec_, sint ele) { char ret[21]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 21, "%llu", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<float> *vec_, sint ele) { char ret[32]{ 0 }; if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(ret, 32, "%0.3f", vec_->at(ele)); } } return ret; }
	cchar* GetCValInVec(std::vector<cchar*> *vec_, sint ele) { cchar* ret = ""; if (ele > -1) { if (ele < (sint)vec_->size()) { ret = vec_->at(ele); } } return ret; }
	void GetCValInVec(std::vector<uchar> *vec_, sint ele, cchar* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val = reinterpret_cast<cchar*>(&vec_->at(ele)); } } }
	void GetCValInVec(std::vector<void*> *vec_, sint ele, cchar* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val = reinterpret_cast<cchar*>(vec_->at(ele)); } } }
	void GetCValInVec(std::vector<sint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%i", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<slint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%li", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<sllint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%lli", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<uint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%u", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<ulint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%lu", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<ullint> *vec_, sint ele, char val[21]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 21, "%llu", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<float> *vec_, sint ele, char val[32]) { if (ele > -1) { if (ele < (sint)vec_->size()) { sint xc = snprintf(val, 32, "%0.3f", vec_->at(ele)); } } }
	void GetCValInVec(std::vector<cchar*> *vec_, sint ele, cchar* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val = (vec_->at(ele)); } } }
	void GetCValInVec(std::vector<std::string> *vec_, sint ele, cchar* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val = (vec_->at(ele).c_str()); } } }
	std::string GetStrValInVec(std::vector<uchar> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { cchar* ret_ = reinterpret_cast<cchar*>(&vec_->at(ele)); ret.append(ret_); } } return ret; }
	std::string GetStrValInVec(std::vector<void*> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { ret.append(reinterpret_cast<cchar*>(vec_->at(ele))); } } return ret; }
	std::string GetStrValInVec(std::vector<sint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = (_itoa_s(vec_->at(ele), mid, 10)); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<slint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _i64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)), 10); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<sllint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _i64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)), 10); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<uint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = (_itoa_s(vec_->at(ele), mid,10)); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<ulint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _ui64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)), 10); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<ullint> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _ui64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)), 10); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<float> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[128]{ 0 }; sprintf_s(mid, "%0.3f", vec_->at(ele)); ret.append(mid); } } return ret; }
	std::string GetStrValInVec(std::vector<cchar*> *vec_, sint ele) { std::string ret; if (ele > -1) { if (ele < (sint)vec_->size()) { ret.append(vec_->at(ele)); } } return ret; }
	void GetStrValInVec(std::vector<uchar> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { cchar* ret_ = reinterpret_cast<cchar*>(&vec_->at(ele)); val->append(ret_); } } }
	void GetStrValInVec(std::vector<void*> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val->append(reinterpret_cast<cchar*>(vec_->at(ele))); } } }
	void GetStrValInVec(std::vector<sint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _itoa_s(vec_->at(ele), mid, 10); val->append(mid); } } }
	void GetStrValInVec(std::vector<slint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _i64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)),10); val->append(mid); } } }
	void GetStrValInVec(std::vector<sllint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _i64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)),10); val->append(mid); } } }
	void GetStrValInVec(std::vector<uint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _itoa_s(vec_->at(ele), mid, 10); val->append(mid); } } }
	void GetStrValInVec(std::vector<ulint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _ui64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)),10); val->append(mid); } } }
	void GetStrValInVec(std::vector<ullint> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[64]{ 0 }; errno_t err = _ui64toa_s(vec_->at(ele), mid, sizeof((sint)vec_->at(ele)),10); val->append(mid); } } }
	void GetStrValInVec(std::vector<float> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { char mid[128]{ 0 }; sprintf_s(mid, "%0.3f", vec_->at(ele)); val->append(mid); } } }
	void GetStrValInVec(std::vector<cchar*> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val->append(vec_->at(ele)); } } }
	void GetStrValInVec(std::vector<std::string> *vec_, sint ele, std::string* val) { if (ele > -1) { if (ele < (sint)vec_->size()) { val->append(vec_->at(ele)); } } }

	DB_CONN* GetValInVec(std::vector<DB_CONN*> *vec_, sint ele) { DB_CONN* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }

	BOT_DB* GetValInVec(std::vector<BOT_DB*> *vec_, sint ele) { BOT_DB* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_DB GetValInVec(std::vector<BOT_DB> *vec_, sint ele) { BOT_DB ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_DB> *vec_, sint ele, BOT_DB* val=0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		
		if (ele > -1 && ele < (sint)vec_->size())
		{
	...
	sint GetMembValInVec(std::vector<BOT_DB> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_DB_MAX && memb > -1)
		{
	...
	BOT_DB_M* GetValInVec(std::vector<BOT_DB_M*> *vec_, sint ele) { BOT_DB_M* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_DB_M GetValInVec(std::vector<BOT_DB_M> *vec_, sint ele) { BOT_DB_M ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_DB_M> *vec_, sint ele, BOT_DB_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_DB_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_DB_MAX && memb > -1)
	...
	BOT_CLIENT* GetValInVec(std::vector<BOT_CLIENT*> *vec_, sint ele) { BOT_CLIENT* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_CLIENT GetValInVec(std::vector<BOT_CLIENT> *vec_, sint ele) { BOT_CLIENT ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_CLIENT> *vec_, sint ele, BOT_CLIENT* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_CLIENT> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_C_MAX && memb > -1)
		{
	...
	BOT_CLIENT_M* GetValInVec(std::vector<BOT_CLIENT_M*> *vec_, sint ele) { BOT_CLIENT_M* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_CLIENT_M GetValInVec(std::vector<BOT_CLIENT_M> *vec_, sint ele) { BOT_CLIENT_M ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_CLIENT_M> *vec_, sint ele, BOT_CLIENT_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_CLIENT_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_C_MAX && memb > -1)
	...
	BOT_LOGDET* GetValInVec(std::vector<BOT_LOGDET*> *vec_, sint ele) { BOT_LOGDET* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_LOGDET GetValInVec(std::vector<BOT_LOGDET> *vec_, sint ele) { BOT_LOGDET ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_LOGDET> *vec_, sint ele, BOT_LOGDET* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_LOGDET> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_LOG_MAX && memb > -1)
		{
	...
	BOT_LOGDET_M* GetValInVec(std::vector<BOT_LOGDET_M*> *vec_, sint ele) { BOT_LOGDET_M* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_LOGDET_M GetValInVec(std::vector<BOT_LOGDET_M> *vec_, sint ele) { BOT_LOGDET_M ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_LOGDET_M> *vec_, sint ele, BOT_LOGDET_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_LOGDET_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_LOG_MAX && memb > -1)
	...
	BOT_FILE* GetValInVec(std::vector<BOT_FILE*> *vec_, sint ele) { BOT_FILE* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_FILE GetValInVec(std::vector<BOT_FILE> *vec_, sint ele) { BOT_FILE ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_FILE> *vec_, sint ele, BOT_FILE* val = 0)
	{
		if (!val) { return -1; }
	...
	sint GetMembValInVec(std::vector<BOT_FILE> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_FS_MAX && memb > -1)
		{
	...
	BOT_FILE_M* GetValInVec(std::vector<BOT_FILE_M*> *vec_, sint ele) { BOT_FILE_M* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	BOT_FILE_M GetValInVec(std::vector<BOT_FILE_M> *vec_, sint ele) { BOT_FILE_M ret; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_FILE_M> *vec_, sint ele, BOT_FILE_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<BOT_FILE_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_FS_MAX && memb > -1)
	...
	TINFO* GetValInVec(std::vector<TINFO*> *vec_, sint ele) { TINFO* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<TINFO> *vec_, sint ele, TINFO* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<TINFO> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_TTS_MAX && memb > -1)
		{
	...
	TINFO_M* GetValInVec(std::vector<TINFO_M*> *vec_, sint ele) { TINFO_M* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<TINFO_M> *vec_, sint ele, TINFO_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<TINFO_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_TTS_MAX && memb > -1)
	...
	HELD_MTX* GetValInVec(std::vector<HELD_MTX*> *vec_, sint ele) { HELD_MTX* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<HELD_MTX> *vec_, sint ele, HELD_MTX* val)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint GetMembValInVec(std::vector<HELD_MTX> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < HMTX_MAX && memb > -1)
		{
	...
	lok_req* GetValInVec(std::vector<lok_req*> *vec_, sint ele) { lok_req* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<lok_req> *vec_, sint ele, lok_req* val)
	{
		if (!val) { return -1; }
		sint ret = -1;
		...
		if (ele > -1 && ele < (sint)vec_->size())
		{
			...
			ret = GetMembValInVec(vec_, ele, BOT_REQ_HMTX, &val->hmtx);
		}
		return ret;
	}
	sint GetMembValInVec(std::vector<lok_req> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_REQ_MAX && memb > -1)
		{
			...
			else if (memb == BOT_REQ_HMTX) { std::vector<HELD_MTX>* vp = reinterpret_cast<std::vector<HELD_MTX>*>(val); size_t siz = 0; while (siz < vec_->at(ele).hmtx.size()) { vp->push_back(vec_->at(ele).hmtx[siz]); siz++; } }
			else {}
			ret = ele;
		}
		return ret;
	}
	BOT_STMT* GetValInVec(std::vector<BOT_STMT*> *vec_, sint ele) { BOT_STMT* ret = 0; if (ele > -1 && ele < (sint)vec_->size()) { ret = vec_->at(ele); } return ret; }
	sint GetValInVec(std::vector<BOT_STMT> *vec_, sint ele, BOT_STMT* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (ele > -1 && ele < (sint)vec_->size())
	...
	sint GetMembValInVec(std::vector<BOT_STMT> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		if (val && ele > -1 && ele < (sint)vec_->size() && memb < BOT_STMT_MAX && memb > -1)
		{
	...
	sint SetValInVec(std::vector<carr_2> *vec_, sint ele, char val[2]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_4> *vec_, sint ele, char val[4]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_8> *vec_, sint ele, char val[8]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_16> *vec_, sint ele, char val[16]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_32> *vec_, sint ele, char val[32]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_64> *vec_, sint ele, char val[64]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_128> *vec_, sint ele, char val[128]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_256> *vec_, sint ele, char val[256]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_512> *vec_, sint ele, char val[512]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_1024> *vec_, sint ele, char val[1024]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_2048> *vec_, sint ele, char val[2048]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<carr_4096> *vec_, sint ele, char val[4096]) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { strcpy_s(vec_->at((size_t)ele).carr, val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<uchar> *vec_, sint ele, uchar val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<void*> *vec_, sint ele, void* val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<sint> *vec_, sint ele, uint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<slint> *vec_, sint ele, ulint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<sllint> *vec_, sint ele, ullint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<uint> *vec_, sint ele, uint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<ulint> *vec_, sint ele, ulint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<ullint> *vec_, sint ele, ullint val) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<float> *vec_, sint ele, float* val = 0) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = *val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<cchar*> *vec_, sint ele, cchar* val = 0) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<std::string> *vec_, sint ele, std::string* val = 0) { sint ret = -1; if (val && ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele).clear(); vec_->at((size_t)ele).append(*val); ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_DB*> *vec_, sint ele, BOT_DB* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_DB> *vec_, sint ele, BOT_DB* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_DB> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
		
		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_DB_MAX) 
		{
	...
	sint SetValInVec(std::vector<BOT_DB_M> *vec_, sint ele, BOT_DB_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_DB_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
	...
	sint SetValInVec(std::vector<BOT_CLIENT*> *vec_, sint ele, BOT_CLIENT* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_CLIENT> *vec_, sint ele, BOT_CLIENT* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_CLIENT> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_C_MAX)
		{
	...
	sint SetValInVec(std::vector<BOT_CLIENT_M> *vec_, sint ele, BOT_CLIENT_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_CLIENT_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
	...
	sint SetValInVec(std::vector<BOT_LOGDET*> *vec_, sint ele, BOT_LOGDET* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_LOGDET> *vec_, sint ele, BOT_LOGDET* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_LOGDET> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_LOG_MAX)
		{
	...
	sint SetValInVec(std::vector<BOT_LOGDET_M> *vec_, sint ele, BOT_LOGDET_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_LOGDET_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
	...
	sint SetValInVec(std::vector<BOT_FILE*> *vec_, sint ele, BOT_FILE* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_FILE> *vec_, sint ele, BOT_FILE* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_FILE> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_FS_MAX)
		{
	...
	sint SetValInVec(std::vector<BOT_FILE_M> *vec_, sint ele, BOT_FILE_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_FILE_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
	...
	sint SetValInVec(std::vector<TINFO*> *vec_, sint ele, TINFO* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<TINFO> *vec_, sint ele, TINFO* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<TINFO> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_TTS_MAX)
		{
	...
	sint SetValInVec(std::vector<TINFO_M> *vec_, sint ele, TINFO_M* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<TINFO_M> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;
	...
	sint SetValInVec(std::vector<HELD_MTX*> *vec_, sint ele, HELD_MTX* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<HELD_MTX> *vec_, sint ele, HELD_MTX* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<HELD_MTX> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < HMTX_MAX)
		{
	...
	sint SetValInVec(std::vector<lok_req*> *vec_, sint ele, lok_req* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<lok_req> *vec_, sint ele, lok_req* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
		...
		if (ele > -1 && ele < (sint)vec_->size())
		{
			...
			ret = SetMembValInVec(vec_, ele, BOT_REQ_HMTX, &val->hmtx);
		}
		return ret;
	}
	sint SetMembValInVec(std::vector<lok_req> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_REQ_MAX)
		{
			...
			else if (memb == BOT_REQ_HMTX) { std::vector<HELD_MTX>* val_ = reinterpret_cast<std::vector<HELD_MTX>*>(val); size_t siz = 0; vec_->at(ele).hmtx.clear(); while (siz < val_->size()) { vec_->at(ele).hmtx.push_back(val_->at(siz)); siz++; } }
			else {}
			ret = ele;
		}
		return ret;
	}
	sint SetValInVec(std::vector<BOT_STMT*> *vec_, sint ele, BOT_STMT* val = 0) { if (!val) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at((size_t)ele) = val; ret = ele; } return ret; }
	sint SetValInVec(std::vector<BOT_STMT> *vec_, sint ele, BOT_STMT* val = 0)
	{
		if (!val) { return -1; }
		sint ret = -1;
	...
	sint SetMembValInVec(std::vector<BOT_STMT> *vec_, sint ele = -1, sint memb = -1, void* val = 0)
	{
		sint ret = -1;

		if (val && ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_STMT_MAX)
		{
	...
	sint PushToMembVal(std::vector<BOT_DB> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_DB_MAX)
		{
			if (memb == BOT_DB_NAME)
			{
				if (opt == 1)
	...
	sint PushToMembVal(std::vector<BOT_DB_M> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
	...
	sint PushToMembVal(std::vector<BOT_CLIENT> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_C_MAX)
		{
	...
	sint PushToMembVal(std::vector<BOT_CLIENT_M> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
	...
	sint PushToMembVal(std::vector<BOT_LOGDET> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_LOG_MAX)
		{
			if (memb == BOT_LOG_T)
			{
				if (opt == 1)
	...
	sint PushToMembVal(std::vector<BOT_LOGDET_M> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
	...
	sint PushToMembVal(std::vector<BOT_FILE> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_FS_MAX)
		{
	...
	sint PushToMembVal(std::vector<BOT_FILE_M> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
	...
	sint PushToMembVal(std::vector<TINFO> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_TTS_MAX)
		{
	...
	sint PushToMembVal(std::vector<TINFO_M> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
	...
	sint PushToMembVal(std::vector<HELD_MTX> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < HMTX_MAX)
		{
			if (memb == HMTX_MEMS)
			{
				if (!opt)
	...
	sint PushToMembVal(std::vector<lok_req> *vec_, sint ele, sint memb, sint opt = -1, void* val = 0)
	{
		if (!vec_ || ele < 0 || opt < 0 || !val)
		{
			return -1;
		...
		if (ele > -1 && ele < (sint)vec_->size() && memb > -1 && memb < BOT_REQ_MAX)
		{
			if (memb == BOT_REQ_HMTX)
			{
				if (!opt)
				{
					HELD_MTX* val_ = (HELD_MTX*)val;
					vec_->at(ele).hmtx.push_back(*val_);
				}
				else if (opt == 3)
				{
					std::vector<HELD_MTX>* val_ = reinterpret_cast<std::vector<HELD_MTX>*>(val);
					size_t siz = 0;

					while (siz < val_->size())
					{
						vec_->at(ele).hmtx.push_back(val_->at(siz));
						siz++;
					}
				}
				else {}
	...
	sint ClearValFromVec(std::vector<carr_2> *vec_, char val[2], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_4> *vec_, char val[4], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_8> *vec_, char val[8], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_16> *vec_, char val[16], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_32> *vec_, char val[32], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_64> *vec_, char val[64], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_128> *vec_, char val[128], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_256> *vec_, char val[256], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_512> *vec_, char val[512], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_1024> *vec_, char val[1024], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_2048> *vec_, char val[2048], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<carr_4096> *vec_, char val[4096], bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz).carr, (void*)val, sizeof(vec_->at(siz).carr)) && sizeof(vec_->at(siz).carr)) { vec_->at(siz).carr[0] = '\0'; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<uchar> *vec_, uchar val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp((void*)vec_->at(siz), (void*)val, sizeof(vec_->at(siz)))) { vec_->at(siz) = UCHAR_MAX; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<void*> *vec_, void* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!memcmp(vec_->at(siz),val,sizeof(vec_->at(siz)))) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<sint> *vec_, sint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = INT_MIN; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<slint> *vec_, slint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = INT32_MIN; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<sllint> *vec_, sllint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = INT64_MIN; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<uint> *vec_, uint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = UINT_MAX; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<ulint> *vec_, ulint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = UINT32_MAX; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<ullint> *vec_, ullint val, bool clear_all) { sint ret = -1; ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == val) { vec_->at(siz) = UINT64_MAX; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<float> *vec_, float* fp, bool clear_all) { sint ret = 0; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz) == *fp) { vec_->at(siz) = 0.0f; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<cchar*> *vec_, cchar* mid, bool clear_all) { sint ret = 0; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz), mid)) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<std::string> *vec_, std::string* mid, bool clear_all) { sint ret = 0; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz).c_str(), mid->c_str())) { vec_->at(siz).clear(); ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_DB> *vec_, BOT_DB* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_DB ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<DB_CONN*> *vec_, DB_CONN* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz)) { if (vec_->at(siz)->lid == val->lid) { vec_->at(siz)->Clear(); ret++; if (!clear_all) { siz = vec_->size(); } } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOTF_CONN*> *vec_, BOTF_CONN* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz)) { if (vec_->at(siz)->lid == val->lid) { vec_->at(siz)->Clear(); ret++; if (!clear_all) { siz = vec_->size(); } } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_DB_M> *vec_, BOT_DB_M* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_DB_M ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_CLIENT> *vec_, BOT_CLIENT* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_CLIENT ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_CLIENT_M> *vec_, BOT_CLIENT_M* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_CLIENT_M ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_LOGDET> *vec_, BOT_LOGDET* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_LOGDET ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_LOGDET_M> *vec_, BOT_LOGDET_M* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_LOGDET_M ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_FILE> *vec_, BOT_FILE* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { vec_->at(siz).Clear(); ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_FILE_M> *vec_, BOT_FILE_M* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { BOT_FILE_M ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<TINFO> *vec_, TINFO* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { TINFO ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<TINFO_M> *vec_, TINFO_M* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { TINFO_M ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<HELD_MTX> *vec_, HELD_MTX* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { HELD_MTX ns; vec_->at(siz) = ns; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<lok_req> *vec_, lok_req* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (vec_->at(siz).lid == val->lid) { vec_->at(siz).Clear(); ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_DB*> *vec_, BOT_DB* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz)->name.c_str(), val->name.c_str())) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_CLIENT*> *vec_, BOT_CLIENT* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz)->name.c_str(), val->name.c_str())) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_LOGDET*> *vec_, BOT_LOGDET* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz)->logtitle.c_str(), val->logtitle.c_str())) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<BOT_FILE*> *vec_, BOT_FILE* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz)->name.c_str(), val->name.c_str())) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }
	sint ClearValFromVec(std::vector<TINFO*> *vec_, TINFO* val, bool clear_all) { sint ret = -1; if (!val) { return ret; } ret++; size_t siz = 0; while (siz < vec_->size()) { if (!strcmp(vec_->at(siz)->thread_name.c_str(), val->thread_name.c_str())) { vec_->at(siz) = 0; ret++; if (!clear_all) { siz = vec_->size(); } } siz++; } return ret; }

	sint ClearEleFromVec(std::vector<carr_2> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_4> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_8> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_16> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_32> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_64> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_128> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_256> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_512> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_1024> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_2048> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<carr_4096> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { if (strlen(vec_->at(ele).carr)) { vec_->at(ele).carr[0] = '\0'; ret = ele; } }return ret; }
	sint ClearEleFromVec(std::vector<uchar> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = UCHAR_MAX; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<void*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<sint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = INT_MIN; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<slint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = INT32_MIN; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<sllint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = INT64_MIN; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<uint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = UINT_MAX; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<ulint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = UINT32_MAX; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<ullint> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = UINT64_MAX; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<float> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0.0f; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<cchar*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = ""; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<std::string> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).clear(); ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOT_STMT> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).Clear(); ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<DB_CONN*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele)->Clear(); ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOTF_CONN*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele)->Clear(); ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOT_DB> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_DB ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_DB> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_DB_M> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_DB_M ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_DB_M> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_CLIENT> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_CLIENT ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_CLIENT> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_CLIENT_M> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_CLIENT_M ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_CLIENT_M> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_LOGDET> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_LOGDET ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_LOGDET> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_LOGDET_M> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_LOGDET_M ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_LOGDET_M> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_FILE> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).Clear(); ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_FILE> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_FILE_M> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { BOT_FILE_M ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<BOT_FILE_M> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<TINFO> *vec_ = 0, sint ele = -1) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).Clear(); ret = ele; } return ret; }
	...
	sint ClearEleFromVec(std::vector<TINFO_M> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { TINFO_M ns; vec_->at(ele) = ns; ret = ele; } return ret; }
	sint ClearMembVal(std::vector<TINFO_M> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<HELD_MTX> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).lid = -1; vec_->at(ele).w_mtx = -1; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<lok_req> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele).Clear(); ret = ele; } return ret; }
	sint ClearMembVal(std::vector<lok_req> *vec_, sint ele, sint memb)
	{
		if (!vec_ || ele < 0)
		{
			return -1;
	...
	sint ClearEleFromVec(std::vector<BOT_DB*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOT_CLIENT*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOT_LOGDET*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<BOT_FILE*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<TINFO*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }
	sint ClearEleFromVec(std::vector<HELD_MTX*> *vec_, sint ele) { if (ele < 0) { return -1; } sint ret = -1; if (ele > -1 && ele < (sint)vec_->size()) { vec_->at(ele) = 0; ret = ele; } return ret; }

	void CleanVec(std::vector<carr_2> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_4> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_8> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_16> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_32> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_64> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_128> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_256> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_512> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_1024> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_2048> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<carr_4096> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!strlen(vec_->at(siz).carr)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<uchar> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==UCHAR_MAX) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<void*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<sint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==INT_MIN) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<slint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==INT32_MIN) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<sllint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==INT64_MIN) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<uint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==UINT_MAX) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<ulint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==UINT32_MAX) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<ullint> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)==UINT64_MAX) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<float> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz) == DBL_MIN) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<cchar*> *vec_) { sint siz = vec_->size() - 1; while (siz > -1) { if (!strcmp(vec_->at(siz), "") || !strlen(vec_->at(siz))) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<std::string> *vec_) { sint siz = vec_->size() - 1; while (siz > -1) { if (!strcmp(vec_->at(siz).c_str(), "") || vec_->at(siz).empty()) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_DB> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<DB_CONN*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)) { if (vec_->at(siz)->lid < 0) { vec_->pop_back(); } else { siz = -1; } } else { vec_->pop_back(); } siz--; } }
	void CleanVec(std::vector<BOTF_CONN*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz)) { if (vec_->at(siz)->lid < 0) { vec_->pop_back(); } else { siz = -1; } } else { vec_->pop_back(); } siz--; } }
	void CleanVec(std::vector<BOT_CLIENT> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_LOGDET> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_FILE> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<TINFO> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<HELD_MTX> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<lok_req> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_DB*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_CLIENT*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_LOGDET*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_FILE*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<TINFO*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<HELD_MTX*> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (!vec_->at(siz)) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_DB_M> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_CLIENT_M> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_LOGDET_M> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_FILE_M> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<TINFO_M> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }
	void CleanVec(std::vector<BOT_STMT> *vec_) { sint siz = (sint)vec_->size() - 1; while (siz > -1) { if (vec_->at(siz).lid < 0) { vec_->pop_back(); } else { siz = -1; } siz--; } }

	void ClearVec(std::vector<carr_2> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_4> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_8> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_16> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_32> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_64> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_128> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_256> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_512> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_1024> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_2048> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<carr_4096> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<uchar> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<void*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<sint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<slint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<sllint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<uint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<ulint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<ullint> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<float> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<cchar*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<std::string> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_DB> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<DB_CONN> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOTF_CONN> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_CLIENT> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_LOGDET> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_FILE> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<TINFO> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<HELD_MTX> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<lok_req> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_DB*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_CLIENT*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_LOGDET*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_FILE*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<TINFO*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<HELD_MTX*> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_DB_M> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_CLIENT_M> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_LOGDET_M> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_FILE_M> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<TINFO_M> *vec_) { vec_->clear(); }
	void ClearVec(std::vector<BOT_STMT> *vec_) { vec_->clear(); }
...
public:
	...
	machine(sint i_debug_lvl = BOT_DEBUG_LVL, sint i_debug_m = BOT_DEBUG_M, sint nlid = -1, cchar* i_name = "")
	{
		quit = false;
		Client_ = 0;
		d_mtxs.clear();
...
private:
	...
	sint HasGMutexLocked(sint g_opt)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::HasGMutexLocked(sint g_opt(%i))", g_opt);
		...
		for (size_t siz = 0; siz < d_mtxs.size(); siz++)
		{
			...
			{
				if (d_mtxs[siz].locked)
				{
					if (debug_lvl >= 900 && debug_m)
					{
						carr_128 outp;
						sint op = BStrP(outp.carr, outp.siz, "HasGMutexLocked_d(g_opt:%i) = %u locked.", g_opt, siz);
						op = Output(outp.carr, 2);
					}
					return (sint)siz;
				}
		...
		{
			carr_64 outp;
			sint op = BStrP(outp.carr, outp.siz, "HasGMutexLocked(g_opt:%i) = -1", g_opt);
			op = Output(outp.carr, 2);
		}
		return -1;
	}
	sint HasGMutex(sint g_opt)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::HasGMutex(sint g_opt(%i))", g_opt);
			op = Output(outp.carr, 2);
		}
		...
		for (size_t siz = 0; siz < d_mtxs.size(); siz++)
		{
			if (d_mtxs[siz].w_mtx == g_opt)
			{
				...
				{
					carr_128 outp;
					sint op = BStrP(outp.carr, outp.siz, "HasGMutex(%i) at d_mtxs[%u]", g_opt, siz);
					op = Output(outp.carr, 2);
				}
				return (sint)siz;
			}
		...
		{
			carr_64 outp;
			sint op = BStrP(outp.carr, outp.siz, "HasGMutex(g_opt:%i) not found", g_opt);
			op = Output(outp.carr, 2);
		}
		return -1;
	}
	sint LockGMutex(sint g_opt, bool sleep_wait = true, sint ele = -1)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::LockGMutex(sint g_opt(%i), bool sleep_wait(%u), sint ele(%i))", g_opt, (uint)sleep_wait, ele);
			op = Output(outp.carr, 2);
		}
		...
		if (ele < 0)
		{
			HELD_MTX nmtx(g_opt, lid, false);
			ele = vtool.AddValToVec(&d_mtxs, &nmtx, true, true);
		}
		...
		else
		{
			...
			while (!done)
			{
				...
				case MTX_DBC:
				{
					ret = bot_dbc_mtx.Lock();
					break;
				}
				case MTX_PEND:
				{
					ret = bot_pend_mtx.Lock();
					break;
				}
				case MTX_TTSV:
				{
					ret = bot_tts_mtx.Lock();
					break;
				}
				case MTX_DBL:
				{
				...
				if (!done)
				{
					if (sleep_wait)
					{
						...
						{
							carr_64 outp;
							sint op = BStrP(outp.carr, outp.siz, "sleeping 100000ns, Global Mutex(%i) is locked.", g_opt);
							op = Output(outp.carr, 2);
						}
			...
			if (ret)
			{
				...
				{
					carr_64 outp;
					sint op = BStrP(outp.carr, outp.siz, "unable to lock Global Mutex(opt:%i sleep count(%i).", g_opt, ct);
					op = Output(outp.carr, 2);
				}
				return -1;
			}
			else
			{
				...
				{
					carr_64 outp;
					sint op = BStrP(outp.carr, outp.siz, "Locked Global Mutex(opt:%i).", g_opt);
					op = Output(outp.carr, 2);
				}
				if (!d_mtxs[ele].locked)
				{
					d_mtxs[ele].locked = true;
				}
			}
		}
		return ele;
	}
	sint UnlockGMutex(sint g_opt, sint ele = -1)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "UnlockGMutex(sint g_opt(%i), sint ele(%i))", g_opt, ele);
			op = Output(outp.carr, 2);
		}

		ele = HasGMutexLocked(g_opt);
		sint lox = -1;
		...
		if (ele > -1)
		{
			...
			case MTX_CV:
			{
				lox = bot_cv_mtx.Unlock();
				break;
			}
			case MTX_LOG:
			{
				lox = bot_log_mtx.Unlock();
				break;
			}
			case MTX_O:
			{
				lox = bot_co_mtx.Unlock();
				break;
			}
			case MTX_DBC:
			{
				lox = bot_dbc_mtx.Unlock();
				break;
			}
			case MTX_PEND:
			{
				lox = bot_pend_mtx.Unlock();
				break;
			}
			case MTX_TTSV:
			{
				lox = bot_tts_mtx.Unlock();
				break;
			}
			case MTX_DBL:
			{
				lox = bot_db_mtx.Unlock();
				break;
			}
			case MTX_FO:
			{
				lox = bot_fo_mtx.Unlock();
				break;
			}
			case MTX_TRAT:
			{
				lox = thr_t_mtx.Unlock();
				break;
			}
			case MTX_STRT:
			{
				lox = bot_strt_mtx.Unlock();
				break;
			}
			case MTX_FCON:
			{
		...
		if (lox)
		{
			if (debug_lvl >= 1 && debug_m)
			{
				...
				if (ele < 0)
				{
					sint op = BStrP(outp.carr, outp.siz, "unable to find Global Mutex(opt:%i).", g_opt);
				}
				else
				{
					sint op = BStrP(outp.carr, outp.siz, "unable to unlock Global Mutex(opt:%i).", g_opt);
				}
		...
		else
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "Unlocked Global Mutex(opt:%i).", g_opt);
				op = Output(outp.carr, 2);
			}
			return ele;
		}
	...
	sint HasEleMtx(sint g_opt, sint ele, sint siz = -1, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "HasEleMtx(sint g_opt(%i), sint ele(%i), sint siz(%i), bool is_meta(%u))", g_opt, ele, siz, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "HasEleMtx(g:%i e:%i is_meta:%u) = %i", g_opt, ele, (uint)is_meta, ret);
			op = Output(outp.carr, 2);
		}
		return ret;
	}
	sint LockElement(sint g_opt = -1, sint ele = -1, bool from_rest = false, bool sleep_wait = true, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "LockElement(sint g_opt(%i), sint ele(%i), bool from_rest(%u), bool sleep_wait(%u), bool is_meta(%u))", g_opt, ele, (uint)from_rest, (uint)sleep_wait, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		while (!done)
		{
			sint h = HasGMutexLocked(g_opt);
			sint p = -1;

			if (h < 0)
			{
				p = LockGMutex(g_opt, true);
				h = p;
			}

			if (h > -1)
			{
				sint x = HasEleMtx(g_opt, ele, h, is_meta);

				if (x < 0)
				{
					lok_req nreq(ele, g_opt, lid, (sint)is_meta, &d_mtxs);
					sint hx = HasGMutexLocked(MTX_REQ);
					sint px = -1;

					if (hx < 0)
					{
						px = LockGMutex(MTX_REQ, true);
						hx = px;
					}
					if (hx > -1)
					{
						...
						case MTX_CV:
						{
							if ((!is_meta && ele < (sint)c_vec.d_vec.size()) || (is_meta && ele < (sint)c_vec.m_vec.size()))
							{
								ret = lok_d.Req(&nreq, sleep_wait);

								if (!ret)
								{
									if (!is_meta)
									{
										if (c_vec.d_vec[ele].lok < 0)
										{
											c_vec.d_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
									...
									{
										if (c_vec.m_vec[ele].lok < 0)
										{
											c_vec.m_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
						...
						case MTX_LOG:
						{
							if ((!is_meta && ele < (sint)bot_log_vec.d_vec.size()) || (is_meta && ele < (sint)bot_log_vec.m_vec.size()))
							{
								ret = lok_d.Req(&nreq, sleep_wait);
								...
								if (!ret)
								{
									if (!is_meta)
									{
										if (bot_log_vec.d_vec[ele].lok < 0)
										{
									...
									{
										if (bot_log_vec.m_vec[ele].lok < 0)
										{
						...
						case MTX_TTSV:
						{
							if ((!is_meta && ele < (sint)tts_vec.d_vec.size()) || (is_meta && ele < (sint)tts_vec.m_vec.size()))
							{
								ret = lok_d.Req(&nreq, sleep_wait);
						...
						case MTX_DBL:
						{
							if ((!is_meta && ele < (sint)db_vec.d_vec.size()) || (is_meta && ele < (sint)db_vec.m_vec.size()))
							{
								ret = lok_d.Req(&nreq, sleep_wait);
								...
								if (!ret)
								{
									if (!is_meta)
									{
										if (db_vec.d_vec[ele].lok < 0)
										{
											db_vec.d_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
									...
									{
										if (db_vec.m_vec[ele].lok < 0)
										{
											db_vec.m_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
						...
						case MTX_DBC:
						{
							if (ele < (sint)sizeof(DCON))
							{
								ret = lok_d.Req(&nreq, sleep_wait);

								if (!ret)
								{
									ret = DCON[ele].mtx.Lock();
								}
							}
							break;
						}
						case MTX_FO:
						{
							if ((!is_meta && ele < (sint)fo_vec.d_vec.size()) || (is_meta && ele < (sint)fo_vec.m_vec.size()))
							{
								ret = lok_d.Req(&nreq, sleep_wait);
								...
								if (!ret)
								{
									if (!is_meta)
									{
										if (fo_vec.d_vec[ele].lok < 0)
										{
											fo_vec.d_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
									...
									{
										if (fo_vec.m_vec[ele].lok < 0)
										{
											fo_vec.m_vec[ele].lok = lid;
										}
										else
										{
											ret = 1;
						...
						case MTX_FCON:
						{
							if (ele < (sint)sizeof(FCON))
							{
								ret = lok_d.Req(&nreq, sleep_wait);

								if (!ret)
								{
									ret = FCON[ele].mtx.Lock();
								}
							}
							break;
						}
						...
						if (ret)
						{
							if (sleep_wait)
							{
								if (ret < 0) // another thread has the mutex we want and wants a mutex this thread holds
								{
									if (ct == 100)
									{
										sint w = lok_d.Withd(ele, g_opt, lid, (sint)is_meta);
										done = true;
									}
								}
								else
								{
									ct = ret; //when Req() returns a 1 another thread is about to unlock our requested element
								}
							}
							else
							{
						...
						else
						{
							if (!is_meta)
							{
								x = vtool.AddValToVec(&d_mtxs[h].h_mems, ele, true, true);
							}
							else
							{
								x = vtool.AddValToVec(&d_mtxs[h].m_mems, ele, true, true);
							}
							done = true;
						}
						if (px > -1)
						{
							px = UnlockGMutex(MTX_REQ, hx);
						}
					}
				}
				else
			...
			if (ret == 9999)
			{
				...
				{
					carr_128 outp;
					sint op = BStrP(outp.carr, outp.siz, "element(g:%i ele:%i is_meta:%u) already locked.", g_opt, ele, (uint)is_meta);
					op = Output(outp.carr, 2);
				}
				ret = 0;
			}
			...
			if (!done)
			{
				...
				{
					carr_128 outp;
					sint op = BStrP(outp.carr, outp.siz, "sleeping 100ns, element(g:%i ele:%i is_meta:%u) is locked.", g_opt, ele, (uint)is_meta);
					op = Output(outp.carr, 2);
				}
				ct++;
				sint xc = nsRest(BOT_NANO_REST);
			}
		}
		...
		if (ret)
		{
			...
			{
				carr_128 outp;
				sint op = BStrP(outp.carr, outp.siz, "unable to lock element(g:%i ele:%i is_meta:%u).", g_opt, ele, (uint)is_meta);
				op = Output(outp.carr, 2);
			}
			return -1;
		}
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "locked element(g:%i ele:%i is_meta:%u).", g_opt, ele, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		return ret;
	}
	sint LockElementByName(sint g_opt, cchar* name, bool from_rest = false, bool sleep_wait = true, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "LockElementByName(sint g_opt(%i), cchar* name(\"%s\"), bool from_rest(%u), bool sleep_wait(%u), bool is_meta(%u))", g_opt, name, (uint)from_rest, (uint)sleep_wait, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint xc = GetVecEleByName(g_opt, name, is_meta);
		sint ret = LockElement(g_opt, xc, from_rest, sleep_wait, is_meta);
		return ret;
	}
	sint UnlockElement(sint g_opt, sint ele, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "UnlockElement(sint g_opt(%i), sint ele(%i), bool is_meta(%u))", g_opt, ele, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		if (h > -1)
		{
			ret = HasEleMtx(g_opt, ele, h, is_meta);
			...
			if (ret > -1)
			{
				...
				case MTX_CV:
				{
					if (!is_meta)
					{
						if (c_vec.d_vec[ele].lok == lid)
						{
							c_vec.d_vec[ele].lok = -1;
							ret = 0;
						}
					}
					else
					{
						if (c_vec.m_vec[ele].lok == lid)
						{
							c_vec.m_vec[ele].lok = -1;
							ret = 0;
						}
					}
					break;
				...
				case MTX_LOG:
				{
					if (!is_meta)
					{
						if (bot_log_vec.d_vec[ele].lok == lid)
						{
					...
					{
						if (bot_log_vec.m_vec[ele].lok == lid)
						{
				...
				case MTX_DBL:
				{
					if (!is_meta)
					{
						if (db_vec.d_vec[ele].lok == lid)
						{
							db_vec.d_vec[ele].lok = -1;
							ret = 0;
						}
					}
					else
					{
						if (db_vec.m_vec[ele].lok == lid)
						{
							db_vec.m_vec[ele].lok = -1;
							ret = 0;
						}
					}
					break;
				...
				case MTX_FO:
				{
					if (!is_meta)
					{
						if (fo_vec.d_vec[ele].lok == lid)
						{
							fo_vec.d_vec[ele].lok = -1;
							ret = 0;
						}
					}
					else
					{
						if (fo_vec.m_vec[ele].lok == lid)
						{
							fo_vec.m_vec[ele].lok = -1;
							ret = 0;
						}
					}
					break;
			...
			if (!ret)
			{
				...
				if (hx > -1)
				{
					retx = lok_d.Withd(ele, g_opt, lid, (sint)is_meta);
				...
				if (!retx)
				{
					if (!is_meta)
					{
						sint xc = vtool.ClearValFromVec(&d_mtxs[h].h_mems, ele, false);
						vtool.CleanVec(&d_mtxs[h].h_mems);
					}
					else
					{
						sint xc = vtool.ClearValFromVec(&d_mtxs[h].m_mems, ele, false);
						vtool.CleanVec(&d_mtxs[h].m_mems);
					}
				}
			}
		}
		...
		if (ret)
		{
			...
			{
				carr_128 outp;
				sint op = BStrP(outp.carr, outp.siz, "unable to unlock element(g:%i ele:%i is_meta:%u).", g_opt, ele, (uint)is_meta);
				op = Output(outp.carr, 2);
			}
		}
		else
		{
			...
			{
				carr_128 outp;
				sint op = BStrP(outp.carr, outp.siz, "unlocked element(g:%i ele:%i is_meta:%u).", g_opt, ele, (uint)is_meta);
				op = Output(outp.carr, 2);
			}
		}
		return ret;
	}
	sint UnlockElementByName(sint g_opt, cchar* name, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "UnlockElementByName(size_t g_opt(%i), cchar* name(\"%s\"), bool is_meta(%u))", g_opt, name, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint xc = GetVecEleByName(g_opt, name, is_meta);
	...
	sint OpenObject(sint nobj = -1, sint nele = -1, sint* r_opt = 0, ...)
	{
		...
		else
		{
			...
			case MTX_FO:
			{
				...
				if (ret > -1)
				{
					ret = OpenFile(&xfile, opts[0], r_opt);
				}
				break;
			}
			case MTX_DBL:
			...
			default:
			{
				ret = LockElement(nobj, nele, false, true, (bool)opts[0]);
				break;
			}
			}
		}
	...
	sint CloseObject(sint nobj = -1, sint nele = -1, sint is_meta = -1, sint* opt = 0)
	{
		...
		if (nele < 0)
		{
			sint h = HasGMutexLocked(nobj);

			if (h > -1)
			{
				ret = UnlockGMutex(nobj, h);
			}
		}
		else
		{
	...
	sint PushToVec(void *val_, sint g_opt, bool check = true, bool use_empty = true, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "PushToVec(void *val_(%i), sint g_opt(%i), bool is_meta(%u))", (sint)val_, g_opt, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			if (!is_meta)
			{
				...
				case MTX_CV:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&c_vec.d_vec, vp, check, use_empty);
				...
				case MTX_DBL:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&db_vec.d_vec, vp, check, use_empty);
				...
				case MTX_FO:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&fo_vec.d_vec, vp, check, use_empty);
			...
			else
			{
				...
				case MTX_CV:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&c_vec.m_vec, vp, check, use_empty);
					}
					break;
				}
				case MTX_LOG:
				...
				case MTX_DBL:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&db_vec.m_vec, vp, check, use_empty);
					}
					break;
				}
				case MTX_FO:
				{
					...
					{
						vp->c_by = lid;
						ret = vtool.AddValToVec(&fo_vec.m_vec, vp, check, use_empty);
					}
					break;
				}
				default:
	...
	sint PushToVecEleMem(void *val_, sint g_opt, sint ele, sint memb, sint v_opt = -1, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "PushToVecEleMem(void *val_(%i), sint g_opt(%i), sint ele(%i), sint memb(%i), sint g_opt(%i), bool is_meta(%u))", (sint)val_, g_opt, ele, memb, v_opt, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			sint x = LockElement(g_opt, ele, false, true, is_meta);
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.PushToMembVal(&c_vec.d_vec, ele, memb, v_opt, val_);
						if (ret > -1)
						{
					...
					case MTX_DBL:
					{
						ret = vtool.PushToMembVal(&db_vec.d_vec, ele, memb, v_opt, val_);
						if (ret > -1)
						{
							sint iret = -1;
					...
					case MTX_FO:
					{
						ret = vtool.PushToMembVal(&fo_vec.d_vec, ele, memb, v_opt, val_);
						if (ret > -1)
						{
							sint iret = -1;
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.PushToMembVal(&c_vec.m_vec, ele, memb, v_opt, val_);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.PushToMembVal(&db_vec.m_vec, ele, memb, v_opt, val_);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.PushToMembVal(&fo_vec.m_vec, ele, memb, v_opt, val_);
						break;
					}
					default:
					{
				...
				if (!x)
				{
					x = UnlockElement(g_opt, ele, is_meta);
				}
			}
	...
	sint PushToVecEleMemByName(cchar* name, void *val_, sint g_opt, sint memb, sint v_opt = -1, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "PushToVecEleMemByName(cchar* name(%s), void *val_(%i), sint g_opt(%i), sint memb(%i), sint g_opt(%i), bool is_meta(%u))", name, (sint)val_, g_opt, memb, v_opt, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint xc = GetVecEleByName(g_opt, name, is_meta);
	...
	sint GetVecEle(void* val_, sint g_opt, sint ele = -1, bool cine = false, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetVecEle(void* val_(%i), sint g_opt(%i), size_t ele(%u), bool cine(%u), bool is_meta(%u))", (sint)val_, g_opt, ele, (uint)cine, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.GetValInVec(&c_vec.d_vec, ele, (BOT_CLIENT*)val_);
						if (ret < 0 && cine)
						{
					...
					case MTX_DBL:
					{
						ret = vtool.GetValInVec(&db_vec.d_vec, ele, (BOT_DB*)val_);
						if (ret < 0 && cine)
						{
					...
					case MTX_FO:
					{
						ret = vtool.GetValInVec(&fo_vec.d_vec, ele, (BOT_FILE*)val_);
						if (ret < 0 && cine)
						{
							ret = PushToVec(val_, g_opt, true, true, false);
						}
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.GetValInVec(&c_vec.m_vec, ele, (BOT_CLIENT_M*)val_);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.GetValInVec(&db_vec.m_vec, ele, (BOT_DB_M*)val_);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.GetValInVec(&fo_vec.m_vec, ele, (BOT_FILE_M*)val_);
						break;
					}
					default:
					{
	...
	sint GetVecEleByName(void* val_, sint g_opt, cchar* name, bool cine = false, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_512 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetVecEleByName(void* val_(%i), sint g_opt(%i), cchar* name(\"%s\"), bool cine(%u), bool is_meta(%u))", (sint)val_, g_opt, name, (uint)cine, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		if (ret > -1)
		{
			for (uint c = 0; c < (sint)svec.size(); c++)
			{
				...
				{
					ret = GetVecEle(val_, g_opt, c, cine, is_meta);
					return ret;
				}
			}
		}
	...
	sint GetVecEleByName(sint g_opt, cchar* name, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_512 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetVecEleByName(sint g_opt(%i), cchar* name(\"%s\"), bool is_meta(%u))", g_opt, name, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		std::vector<std::string> svec;
		sint ret = GetElementNames(g_opt, &svec, is_meta);
	...
	sint GetEmptyVecEle(sint g_opt, bool is_meta = true, sint mode = -1)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetEmptyVecEle(sint g_opt(%i), bool is_meta(%u), sint mode(%i))", g_opt, (uint)is_meta, mode);
			op = Output(outp.carr, 2);
		}
	...
	sint GetVecEleMem(void* val_, sint g_opt, sint ele = -1, sint memb = -1, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetVecEleMem(void* val_(%i), sint g_opt(%i), size_t ele(%u), sint memb(%i), bool is_meta(%u))", (sint)val_, g_opt, ele, memb, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.GetMembValInVec(&c_vec.d_vec, ele, memb, val_);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.GetMembValInVec(&db_vec.d_vec, ele, memb, val_);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.GetMembValInVec(&fo_vec.d_vec, ele, memb, val_);
						break;
					}
					default:
					{
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.GetMembValInVec(&c_vec.m_vec, ele, memb, val_);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.GetMembValInVec(&db_vec.m_vec, ele, memb, val_);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.GetMembValInVec(&fo_vec.m_vec, ele, memb, val_);
						break;
					}
					default:
					{
	...
	sint SetVecEle(void* val_, sint g_opt, sint ele = -1, bool cine = false, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "SetVecEle(void* val_(%i), sint g_opt(%i), sint ele(%i), bool cine(%u), bool is_meta(%u))", (sint)val_, g_opt, ele, (uint)cine, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			if (cine)
			{
				...
				while (vsiz < ele)
				{
					...
					case MTX_CV:
					{
						...
						{
							BOT_CLIENT_M nval;
							vsiz = vtool.AddValToVec(&c_vec.m_vec, &nval, false, false);
						}
						else
						{
							BOT_CLIENT nval;
							vsiz = vtool.AddValToVec(&c_vec.d_vec, &nval, false, false);
						}
						break;
					}
					case MTX_LOG:
					...
					case MTX_DBL:
					{
						...
						{
							BOT_DB_M nval;
							vsiz = vtool.AddValToVec(&db_vec.m_vec, &nval, false, false);
						}
						else
						{
							BOT_DB nval;
							vsiz = vtool.AddValToVec(&db_vec.d_vec, &nval, false, false);
						}
						break;
					}
					case MTX_FO:
					{
						...
						{
							BOT_FILE_M nval;
							vsiz = vtool.AddValToVec(&fo_vec.m_vec, &nval, false, false);
						}
						else
						{
							BOT_FILE nval;
							vsiz = vtool.AddValToVec(&fo_vec.d_vec, &nval, false, false);
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						BOT_CLIENT* vp = (BOT_CLIENT*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&c_vec.d_vec, ele, vp);
					...
					case MTX_LOG:
					{
						BOT_LOGDET* vp = (BOT_LOGDET*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&bot_log_vec.d_vec, ele, vp);
						if (ret > -1)
						{
							BOT_LOGDET_M ns;
							ns.Update(vp);
					...
					case MTX_TTSV:
					{
						TINFO* vp = (TINFO*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&tts_vec.d_vec, ele, vp);
						if (ret > -1)
						{
							TINFO_M ns;
							ns.Update(vp);
					...
					case MTX_DBL:
					{
						BOT_DB* vp = (BOT_DB*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&db_vec.d_vec, ele, vp);
						if (ret > -1)
						{
							BOT_DB_M ns;
							ns.Update(vp);
					...
					case MTX_FO:
					{
						BOT_FILE* vp = (BOT_FILE*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&fo_vec.d_vec, ele, vp);
						if (ret > -1)
						{
							BOT_FILE_M ns;
							ns.Update(vp);
				...
				else
				{
					...
					case MTX_CV:
					{
						BOT_CLIENT_M* vp = (BOT_CLIENT_M*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&c_vec.m_vec, ele, vp);
						break;
					}
					case MTX_LOG:
					{
						BOT_LOGDET_M* vp = (BOT_LOGDET_M*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&bot_log_vec.m_vec, ele, vp);
						break;
					}
					case MTX_TTSV:
					{
						TINFO_M* vp = (TINFO_M*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&tts_vec.m_vec, ele, vp);
						break;
					}
					case MTX_DBL:
					{
						BOT_DB_M* vp = (BOT_DB_M*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&db_vec.m_vec, ele, vp);
						break;
					}
					case MTX_FO:
					{
						BOT_FILE_M* vp = (BOT_FILE_M*)val_;
						vp->c_by = lid;
						ret = vtool.SetValInVec(&fo_vec.m_vec, ele, vp);
						break;
					}
					default:
					{
	...
	sint SetVecEleMem(void* val_, sint g_opt, sint ele, sint memb, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "SetVecEleMem(void* val_(%i), sint g_opt(%i), sint ele(%i), sint memb(%i), bool is_meta(%u))", (sint)val_, g_opt, ele, memb, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.SetMembValInVec(&c_vec.d_vec, ele, memb, val_);
						if (ret > -1)
						{
					...
					case MTX_DBL:
					{
						ret = vtool.SetMembValInVec(&db_vec.d_vec, ele, memb, val_);
						if (ret > -1)
						{
							sint iret = -1;
							if (memb == BOT_DB_TABLES)
					...
					case MTX_FO:
					{
						ret = vtool.SetMembValInVec(&fo_vec.d_vec, ele, memb, val_);
						if (ret > -1)
						{
							sint iret = -1;
							if (memb == BOT_FS_STRVEC)
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.SetMembValInVec(&c_vec.m_vec, ele, memb, val_);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.SetMembValInVec(&db_vec.m_vec, ele, memb, val_);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.SetMembValInVec(&fo_vec.m_vec, ele, memb, val_);
						break;
					}
					default:
					{
	...
	sint ClearVecEle(sint g_opt, sint ele, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "ClearVecEle(sint g_opt(%i), sint ele(%i), bool is_meta(%u))", g_opt, ele, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.ClearEleFromVec(&c_vec.d_vec, ele);
						vtool.CleanVec(&c_vec.d_vec);
					...
					case MTX_LOG:
					{
						...
						vtool.CleanVec(&bot_log_vec.d_vec);
					...
					case MTX_DBL:
					{
						ret = vtool.ClearEleFromVec(&db_vec.d_vec, ele);
						vtool.CleanVec(&db_vec.d_vec);
					...
					case MTX_FO:
					{
						ret = vtool.ClearEleFromVec(&fo_vec.d_vec, ele);
						vtool.CleanVec(&fo_vec.d_vec);
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.ClearEleFromVec(&c_vec.m_vec, ele);
						vtool.CleanVec(&c_vec.m_vec);
						break;
					}
					case MTX_LOG:
					{
						ret = vtool.ClearEleFromVec(&bot_log_vec.m_vec, ele);
						vtool.CleanVec(&bot_log_vec.m_vec);
						break;
					}
					case MTX_TTSV:
					{
						ret = vtool.ClearEleFromVec(&tts_vec.m_vec, ele);
						vtool.CleanVec(&tts_vec.m_vec);
						break;
					}
					case MTX_DBL:
					{
						ret = vtool.ClearEleFromVec(&db_vec.m_vec, ele);
						vtool.CleanVec(&db_vec.m_vec);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.ClearEleFromVec(&fo_vec.m_vec, ele);
						vtool.CleanVec(&fo_vec.m_vec);
						break;
					}
					default:
					{
	...
	sint ClearVecEleMem(sint g_opt, sint ele, sint memb, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "ClearVecEle(sint g_opt(%i), sint ele(%i), sint memb(%i), bool is_meta(%u))", g_opt, ele, memb, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						ret = vtool.ClearMembVal(&c_vec.d_vec, ele, memb);
						if (ret > -1)
						{
							sint iret = ClearVecEle(g_opt, ele);
						}
					...
					case MTX_DBL:
					{
						ret = vtool.ClearMembVal(&db_vec.d_vec, ele, memb);
						if (ret > -1)
						{
							sint iret = ClearVecEle(g_opt, ele);
						}
					...
					case MTX_FO:
					{
						ret = vtool.ClearMembVal(&fo_vec.d_vec, ele, memb);
						if (ret > -1)
						{
							sint iret = ClearVecEle(g_opt, ele);
						}
				...
				else
				{
					...
					case MTX_CV:
					{
						ret = vtool.ClearMembVal(&c_vec.m_vec, ele, memb);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						ret = vtool.ClearMembVal(&db_vec.m_vec, ele, memb);
						break;
					}
					case MTX_FO:
					{
						ret = vtool.ClearMembVal(&fo_vec.m_vec, ele, memb);
						break;
					}
					default:
					{
	...
	sint VecSize(sint g_opt, bool is_meta = true)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "VecSize(sint g_opt(%i), bool is_meta(%u))", g_opt, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
		...
		if (h > -1)
		{
			if (!is_meta)
			{
				...
				case MTX_DBL:
				{
					ret = (sint)db_vec.d_vec.size();
					break;
				}
				case MTX_FO:
				{
					ret = (sint)fo_vec.d_vec.size();
					break;
				}
				default:
				{
			...
			else
			{
				...
				case MTX_DBL:
				{
					ret = (sint)db_vec.m_vec.size();
					break;
				}
				case MTX_FO:
				{
					ret = (sint)fo_vec.m_vec.size();
					break;
				}
				default:
				{
	...
	sint GetElementNames(sint g_opt, std::vector<std::string>* nvec, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetElementNames(sint g_opt(%i), std::vector<std::string>* nvec(%i), is_meta(%u))", g_opt, (sint)nvec, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
			...
			if (!is_meta)
			{
				...
				case MTX_CV:
				{
					...
					while (ele < c_vec.d_vec.size())
					{
						...
						{
							std::string str;
							str.append(c_vec.d_vec[ele].name);
							nvec->push_back(str);
				...
				case MTX_LOG:
				{
					...
					while (ele < bot_log_vec.d_vec.size())
					{
						sint x = LockElement(g_opt, ele, false, true, is_meta);
						...
						{
							std::string str;
							str.append(bot_log_vec.d_vec[ele].logtitle);
							nvec->push_back(str);
				...
				case MTX_TTSV:
				{
					...
					while (ele < tts_vec.d_vec.size())
					{
						...
						{
							std::string str;
							str.append(tts_vec.d_vec[ele].thread_name);
							nvec->push_back(str);
				...
				case MTX_DBL:
				{
					...
					while (ele < db_vec.d_vec.size())
					{
						...
						{
							std::string str;
							str.append(db_vec.d_vec[ele].name);
							nvec->push_back(str);
				...
				case MTX_FO:
				{
					...
					while (ele < fo_vec.d_vec.size())
					{
						...
						{
							std::string str;
							str.append(fo_vec.d_vec[ele].name);
							nvec->push_back(str);
			...
			else
			{
				...
				case MTX_CV:
				{
					...
					while (ele < c_vec.m_vec.size())
					{
						...
						{
							std::string str;
							str.append(c_vec.m_vec[ele].name);
							nvec->push_back(str);
				...
				case MTX_LOG:
				{
					...
					while (ele < bot_log_vec.m_vec.size())
					{
						sint x = LockElement(g_opt, ele, false, true, is_meta);
						...
						{
							std::string str;
							str.append(bot_log_vec.m_vec[ele].logtitle);
							nvec->push_back(str);
				...
				case MTX_TTSV:
				{
					...
					while (ele < tts_vec.m_vec.size())
					{
						...
						{
							std::string str;
							str.append(tts_vec.m_vec[ele].thread_name);
							nvec->push_back(str);
				...
				case MTX_DBL:
				{
					...
					while (ele < db_vec.m_vec.size())
					{
						...
						{
							std::string str;
							str.append(db_vec.m_vec[ele].name);
							nvec->push_back(str);
				...
				case MTX_FO:
				{
					...
					while (ele < fo_vec.m_vec.size())
					{
						...
						{
							std::string str;
							str.append(fo_vec.m_vec[ele].name);
							nvec->push_back(str);
	...
	sint GetElementIDs(sint g_opt, std::vector<sint> * nvec, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetElementIDs(sint g_opt(%i), std::vector<sint> * nvec(%i), is_meta(%u))", g_opt, (sint)nvec, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(g_opt);
			...
			if (!is_meta)
			{
				...
				case MTX_CV:
				{
					...
					while (ele < c_vec.d_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(c_vec.d_vec[ele].lid);
				...
				case MTX_LOG:
				{
					...
					while (ele < bot_log_vec.d_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(bot_log_vec.d_vec[ele].lid);
				...
				case MTX_TTSV:
				{
					...
					while (ele < tts_vec.d_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(tts_vec.d_vec[ele].lid);
				...
				case MTX_DBL:
				{
					...
					while (ele < db_vec.d_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(db_vec.d_vec[ele].lid);
				...
				case MTX_FO:
				{
					...
					while (ele < fo_vec.d_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(fo_vec.d_vec[ele].lid);
			...
			else
			{
				...
				case MTX_CV:
				{
					...
					while (ele < c_vec.m_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(c_vec.m_vec[ele].lid);
				...
				case MTX_LOG:
				{
					...
					while (ele < bot_log_vec.m_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(bot_log_vec.m_vec[ele].lid);
				...
				case MTX_TTSV:
				{
					...
					while (ele < tts_vec.m_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(tts_vec.m_vec[ele].lid);
				...
				case MTX_DBL:
				{
					...
					while (ele < db_vec.m_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(db_vec.m_vec[ele].lid);
				...
				case MTX_FO:
				{
					...
					while (ele < fo_vec.m_vec.size())
					{
						...
						if (LockRet(x) > -1)
						{
							nvec->push_back(fo_vec.m_vec[ele].lid);
	...
	sint GetDateTimeNS(char str_[], bool form = true)
	{
#ifdef _WIN32
		SYSTEMTIME tim;
		GetSystemTime(&tim);
		...
		if (form)
		{
			sint xc = BStrP(str_, 128, "%.2d / %.2d / %.4d %.2d : %.2d : %.2d : %.3d", tim.wMonth, tim.wDay, tim.wYear, tim.wHour, tim.wMinute, tim.wSecond, tim.wMilliseconds);
		}
		else
		{
			sint xc = BStrP(str_, 128, "%.2d %.2d %.4d %.2d %.2d %.2d %.3d", tim.wMonth, tim.wDay, tim.wYear, tim.wHour, tim.wMinute, tim.wSecond, tim.wMilliseconds);
	...
	sint GetDateTime(char str_[], bool form = true)
	{
#ifdef _WIN32
		SYSTEMTIME tim;
		GetSystemTime(&tim);
	...
	sint GetTime(char str_[], bool forms = true)
	{
		time_t tstamp;
		...
		if (forms)
		{
			sint xc = BStrP(str_, 64, "%.2u : %.2u : %.2u : %.3u", tim.wHour, tim.wMinute, tim.wSecond, tim.wMilliseconds);
		}
		else
		{
			sint xc = BStrP(str_, 64, "%.2d %.2d %.2d %.3d", tim.wHour, tim.wMinute, tim.wSecond, tim.wMilliseconds);
		}
#else
		struct tm t;
		struct timespec ti;
	...
	sint GetDate(char str_[], bool form = true)
	{
		time_t tstamp;
		struct tm t;
		sint xc = (sint)time(&tstamp);
	...
	sint LogName(sint opt, char nm[], char dat[], bool tomine = false)
	{
		switch (opt)
		{
		case 0:
		{
			if (debug_lvl > 0 && debug_m)
			{
				if (!tomine)
				{
					sint xc = BStrP(nm, 255, "Activity_Log_%s", dat);
				}
				else
				{
					sint xc = BStrP(nm, 255, "Activity_Log_%s_%s", dat, ptn.c_str());
				}
			}
			break;
		}
		case 1:
		{
			if (!tomine)
			{
				sint xc = BStrP(nm, 255, "Error_Log_%s", dat);
			}
			else
			{
				sint xc = BStrP(nm, 255, "Error_Log_%s_%s", dat, ptn.c_str());
			}
			break;
		}
		case 2:
		...
		case 3:
		{
			if (!tomine)
			{
				sint xc = BStrP(nm, 256, "Maintenance_Log_%s", dat);
			}
			else
			{
				sint xc = BStrP(nm, 255, "Maintenance_Log_%s_%s", dat, ptn.c_str());
			}
			break;
		}
		default:
	...
	sint LogPut(cchar* msg_, sint option, bool tomain = true, bool tomine = true)
	{
		char dat[32] = { 0 };
		...
		if (tomain)
		{
			...
			if (xc > -1)
			{
				...
				else
				{
					...
					sint of = -1;
					sint sx = OpenFile(&nfile, BOT_FILE_APPEND, &of, true);

					if (sx > -1)
					{
						sx = FileOut(&nfile, 3, &u_logs[xc].log_q);
	...
	sint GetFConn(BOT_FILE* val_ = 0, sint o_mode = -1)
	{
		...
		sint ret = -1;
		sint h = HasGMutexLocked(MTX_FCON);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_FCON);
			h = p;
		}
		if (h > -1)
		{
			...
			{
				sint xc = LockElement(MTX_FCON, siz, false, false, false);
			...
			if (p > -1)
			{
				p = UnlockGMutex(MTX_FCON, h);
			}
		}
		return ret;
	}
	sint GetOFConn(BOT_FILE* val_ = 0, sint o_mode = -1)
	{
		if (!val_)
		{
			return -1;
	...
	sint CloseFConn(BOT_FILE* val_ = 0, sint o_mode = -1)
	{
		if (!val_ || o_mode < 0)
		{
			return -1;
		...
		sint ret = -1;
		sint xc = GetOFConn(val_, o_mode);

		if (xc > -1)
		{
			f_con[xc]->Clear();
			f_con[xc] = 0;
			vtool.CleanVec(&f_con);
			ret = UnlockElement(MTX_FCON, xc, false);
		}
		return ret;
	}
	sint FileExists(BOT_FILE *file_, bool cine = false)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			char instr[128] = { 0 };
			sint oc = BStrP(instr, 128, "::FileExists(BOT_FILE *file_(%i), bool cine(%u))", (sint)file_, (uint)cine);
			oc = Output(instr, 2);

			if (debug_lvl >= 10)
			{
				char xstr[256] = { 0 };
				oc = BStrP(xstr, 256, "%s%s%s", file_->path.c_str(), file_->name.c_str(), file_->type.c_str());
				oc = Output(xstr, 2);
			}
		}
		if (!file_)
		...
		file_->lid = GetVecEleByName(MTX_FO, file_->name.c_str());

		if (file_->lid < 0 && cine)
		{
			sint xc = PushToVec(file_, MTX_FO, true, true, false);
...
#ifdef _WIN32
		rc = _stat(str.c_str(), &file_->filestats);
#else
		rc = stat(str.c_str(), &file_->filestats);
#endif
	...
	sint OpenFile(BOT_FILE* file_, sint o_mode = -1, sint *was_open = 0, bool cine = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			char instr[2048] = { 0 };
			sint oc = BStrP(instr, 2048, "::OpenFile(BOT_FILE* file_(\"%s\"), sint o_mode(%i), sint *was_open(%i), bool cine(%u))", file_->name.c_str(), o_mode, (sint)was_open, (uint)cine);
			oc = Output(instr, 2);
		}

		if (!file_ || !was_open || o_mode < 0)
		{
			return -10;
		}
		
		sint fc = GetOFConn(file_, o_mode);

		if (fc < 0)
		{
			*was_open = GetFConn(file_, o_mode);

			if (*was_open < 0 || *was_open >= (sint)sizeof(f_con))
			{
				return -1;
			}
			else
			{
				f_con[*was_open]->Renew(file_, lid, o_mode);
			}
		}
		else
		{
		...
		if (fc < 0)
		{
			...
			if (xc < 0)
			{
				if (cine)
				{
					...
					f_con[*was_open]->fstrm.open(instr.c_str(), o_mode);

					if (f_con[*was_open]->fstrm.is_open())
					{
						f_con[*was_open]->fstrm.close();
						xc = FileExists(file_, cine);
			...
			lx = LockElement(MTX_FO, file_->lid, false, true, false);
		...
		{
			ofst.omode = f_con[fc]->omode;
			ofst.ia_loc = f_con[fc]->fstrm.tellg();
			ofst.oa_loc = f_con[fc]->fstrm.tellp();
			f_con[fc]->fstrm.seekg(f_con[fc]->fstrm.end);
			ofst.e_loc = f_con[fc]->fstrm.tellg();
			f_con[fc]->fstrm.seekg(ofst.ia_loc);
			f_con[fc]->fstrm.close();
		}
		...
		if (S_ISREG(file_->filestats.st_mode))
		{
			...
			instr.append(file_->type.c_str());
			//char instr[2048] = { 0 };
			//sint oc = BStrP(instr, 2048, "%s%s%s", file_->path.c_str(), file_->name.c_str(), file_->type.c_str());
		...
		if (fc < 0)
		{
			if ( !lx)
			{
				lx = UnlockElement(MTX_FO, file_->lid, false);
			}
			sint oc = ClearVecEle(MTX_FO, file_->lid, false);
		}
		return *was_open;
	}
	sint FileOut(BOT_FILE* file_ = 0, sint as_opt = 0, ...)
	{
		if (!file_)
		{
			return -1;
	...
	sint CloseFile(BOT_FILE* file_, sint o_mode = -1, bool del = false, bool encr = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_2048 instr;
			sint oc = BStrP(instr.carr, instr.siz, "::CloseFile(BOT_FILE* file_(\"%s\"), sint o_mode(%i), bool del(%i), bool encr(%u))", file_->name.c_str(), o_mode, (uint)del, (uint)encr);
		...
		sint xc = FileExists(file_);
		xc = CloseFConn(file_, o_mode);
		sint p = UnlockElement(MTX_FO, file_->lid, false);
		return p;
	}
	...
	sint InitReadFile(cchar* filename = "", std::vector<std::string>* str = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			char outp[1200] = { 0 };
			sint oc = BStrP(outp, 1200, "InitReadFile(cchar* filename(%\"%s\"), std::vector<std::string> str(%u))", filename, str->size());
			oc = Output(outp, 2);
		}

		if (!strlen(filename) || !str)
		{
			return -1;
		}

		BOT_FILE FileStruct;
		FileStruct.name.append(filename);
		FileStruct.strvec = *str;

#ifdef _WIN32
		struct _stat filestats;
		sint xc = _stat(filename, &filestats);
#else
		struct stat filestats;
		sint xc = stat(filename, &filestats);
#endif
		...
		if (!xc)
		{
			...
			{
				char outp[1200] = { 0 };
				sint oc = BStrP(outp, 1200, "got stats for %s", filename);
				oc = Output(outp, 2);
			}
			...
			if (S_ISREG(filestats.st_mode))
			{
				...
				{
					char outp[1200] = { 0 };
					sint oc = BStrP(outp, 1200, "%s is a regular file", filename);
					oc = Output(outp, 2);
				}
				char fn_[256] = { 0 };
				sprintf_s(fn_, "ReadFile: %s", filename);
				TINFO tts(1, fn_, ReadFile);
				xc = InitThread(&tts);

				//BOT_FILE *FileStruct_ = &FileStruct;
				//pthread_t bot_thread;
				//tothread_struct tts;
				//tts.Info = Info
				/*
				tts.thread_name = "ReadFile : ";
				tts.thread_name.append( filename );
				tts.lid = ttsv_->size();
				tts.thread = thread;
				tothread_struct *tts_;
				ttsv_->push_back( tts );
				tts_ = &ttsv_->back();
				xc = pthread_create( &tts_->thread, NULL, ReadFile, tts_ );

				if ( !xc )
				{
				if ( debug_lvl >= 1 && debug )
				{
				output = "thread created, xc is ";
				output.append( ITOA( xc ) );
				Output( Info_, output.c_str(), 2 );
				output.clear();
				}
				}
				*/
		...
		else
		{
			...
			{
				char outp[1200] = { 0 };
				sint oc = BStrP(outp, 1200, "Unable to open: %s", filename);
				oc = Output(outp, 2);
			}
		}
		return xc;
	...
	// Basic Idents
	sint GetIdents(cchar* str_, std::vector<std::string> *strvec_, sint idents, sint opt);
	sint PushToIdents(std::vector<PARSS> *vec_, cchar *str_ );
	sint PushToIdentsV(PARSS *Sym_, sint x );
	sint ClearIdents();
	sint ClearIdentsV(sint x );
	sint ClearIdentsVE(sint x, sint e );
	sint SetIdentsV(std::vector<PARSS> *vec_, sint x );
	sint SetIdentsVE(PARSS *Sym_, sint x, sint e );
	sint GetIdentsV(std::vector<PARSS> *vec_, sint x );
	sint GetIdentsVE(PARSS *vec_, sint x, sint e );
	sint GetIdentsSize();
	sint GetIdentsSizeV(sint x );
	sint GetIdentsSpec(std::string *str_, sint x );
	sint GetBasicIdents(cchar* str_, std::vector<std::string> *strvec_, std::vector<PARSS> *rvec_, sint opt, sint ident);
	sint VerifyBasicIdents(BOT_DB *dbs_, std::vector<BOT_STMT> *tvec_, sint table);
	sint BuildBasicIdents(BOT_DB *dbs_, std::vector<BOT_STMT> *tvec_, std::vector<sint> *opts_);
	sint AddBasicIdents(std::vector<PARSS> *vec_);
	sint OutputBasicIdents();
	...
	slint nsRest(slint i, bool keep_mtx = false)
	{
		...
		{
			carr_128 instr;
			sint oc = BStrP(instr.carr, instr.siz, "nsRest(slint i(%li), bool keep_mtx(%u))", i, (uint)keep_mtx);
			oc = Output(instr.carr, 2);
		}
		...
		for (slint slpct = 0; slpct < i; slpct += BOT_NANO_REST)
		{
			if (!keep_mtx)
			{
				sint h = HasGMutexLocked(MTX_REQ);
				sint p = -1;

				if (h < 0)
				{
					p = LockGMutex(MTX_REQ);
					h = p;
				}

				if (h > -1)
				{
					sint cheq = lok_d.CheqReqs(lid, &relocks, &d_mtxs);

					if (p > -1)
					{
						p = UnlockGMutex(MTX_REQ, h);
					}
		...
		if (debug_lvl >= 9 && debug_m)
		{
			...
			{
				carr_128 outp;
				i = BStrP(outp.carr, outp.siz, "attempted to rest:%lins, rested for:%lins", i, xc);
	...
	sint Rest(sint i, bool keep_mtx = false)
	{
		...
		{
			carr_128 instr;
			sint oc = BStrP(instr.carr, instr.siz, "Rest(sint i(%i), bool keep_mtx(%u))", i, (uint)keep_mtx);
			oc = Output(instr.carr, 2);
		}
		...
		if (debug_lvl >= 9 && debug_m)
		{
			...
			{
				carr_128 outp;
				sint op = BStrP(outp.carr, outp.siz, "attempted to rest:%lims, rested for:%lims", i, xc);
				op = Output(outp.carr, 2);
			}
		}
	...
	// Client Functions
	sint ClearClient(BOT_CLIENT *c_);
	...
	sint Output(cchar* output_, sint option, bool logmain = true, bool logmine = true)
	{
		carr_64 tdata;
		sint xc = GetTime(tdata.carr);
		carr_256 instr;
		xc = BStrP(instr.carr, instr.siz, "%s [%i][%s] ", tdata.carr, lid, ptn.c_str());
		std::string str;
		...
		if (logmain || logmine)
		{
			xc = LogPut(str.c_str(), option, logmain, logmine);
		}

		if (!option)
		{
			sint h = HasGMutexLocked(MTX_O);
			sint p = -1;
			
			if (h < 0)
			{
				p = LockGMutex(MTX_O, true);
				h = p;
			}
			...
			if (h > -1)
			{
				...
				if (p > -1)
				{
					p = UnlockGMutex(MTX_O, h);
				}
			}
		}
		return 0;
	}
	sint Input(bool logmain=true, bool logmine=true)
	{
		std::string input;
		std::getline(std::cin, input);
		sint xc = LogPut(input.c_str(), 0, logmain, logmine);

		if (!input.empty())
		{
			std::vector<std::string> vec;
			xc = GetIdents("", &vec, -1, -1);
	...
	const size_t ustrlen(uchar* val) { if (sizeof(*val) < BOT_STRLEN_MAX) { size_t siz = 0; while (siz < BOT_STRLEN_MAX && siz < sizeof(*val)) { if (val[siz] == '\0') { const size_t csiz = siz; return csiz; } siz++; } } return 0; }
	sint StrSizTests()
	{
		std::cout << "... ... ... ... ... ... ..." << std::endl;
		std::cout << "... ... ... ... ... ... ..." << std::endl;
		...
		std::cout << "BStrP(ncar, 15, \"test string...\");" << std::endl;
		xt = BStrP(ncar, 15, "test string...");
		...
		std::cout << "BStrP(ecar, 15, \"_test_ _string_...\");" << std::endl;
		xt = BStrP(ecar, 15, "_test_ _string_...");
		std::cout << "returned: " << xt << std::endl;
		...
		std::cout << "BStrP(scar, 64, \"print this char array as a test for getting and printing sizes.\");" << std::endl;
		xt = BStrP(scar, 64, "print this char array as a test for getting and printing sizes.");
		std::cout << "returned: " << xt << std::endl;
	...
	sint BStrP(char inchr[], size_t in_siz, cchar* fstr, ...)
	{
		if (in_siz > BOT_STRLEN_MAX)
		{
			return -1;
		}
	...
	const size_t BSiz(const char tchar[])
	{
		return sizeof(tchar);
	}
	sint BotP_Error(sint err_no, cchar* msg)
	{
		switch (err_no)
		{
		case 0:
	...
	sint BStrChk(char inchr[], size_t in_siz)
	{
		if (!in_siz)
		{
			return -1;
		...
		{
			inchr[in_siz - 1] = '\0';
			std::cout << "*BOTERROR*char[]: " << inchr << " was without a terminating NULL byte.*BOTERROR*" << std::endl;
		}
	...
	sint LEncStr(std::string *str_, sint opt)
	{
		if (debug_lvl >= 100 && debug_m)
		{
			char instr[1024]{ 0 };
			sint oc = BStrP(instr, 1024, "LEncStrstring *str_(\"%s\"), sint opt(%i))", str_->c_str(), opt);
			oc = Output(instr, 2);
		}
		...
		if (opt > -1)
		{
			if (opt < GetIdentsSizeV(4))
			{
				PARSS Sym;
				sint xc = GetIdentsVE(&Sym, 4, opt);
				s.append(Sym.sym);
				s.append(*str_);
				s.append(Sym.closer);
			}
		...
		else
		{
			...
			while (x < GetIdentsSizeV(4))
			{
				PARSS Sym;
				sint f = GetIdentsVE(&Sym, 4, x);
				f = str_->find(Sym.sym, 0);
				...
				else
				{
					...
					else if (!Sym.enc.empty())
					{
						f = str_->find(Sym.enc, f);
			...
			if (x == GetIdentsSizeV(4))
			{
				...
				{
					char instr[1024]{ 0 };
					sint oc = BStrP(instr, 1024, "enclosed string: %s %i", str_->c_str(), opt);
					oc = Output(instr, 2);
				}
				return 1;
			}
	...
	std::string LEncStrI(std::string *str_, sint opt)
	{
		if (debug_lvl >= 100 && debug_m)
		{
			char instr[1024]{ 0 };
			sint oc = BStrP(instr, 1024, "LEncStrI(string *str_(\"%s\"), sint opt(%i))", str_->c_str(), opt);
			oc = Output(instr, 2);
		}

		std::string s;

		if (opt > -1 && opt < GetIdentsSizeV(4))
		{
			PARSS Sym;
			sint xc = GetIdentsVE(&Sym, 4, opt);
			s.append(Sym.sym);
		...
		else
		{
			...
			if (x == GetIdentsSizeV(4) && x > -1)
			{
				...
				{
					char instr[1024]{ 0 };
					sint oc = BStrP(instr, 1024, "enclosed string: %s %i", s.c_str(), opt);
					oc = Output(instr, 2);
				}
				return s;
			}
		...
		{
			char instr[1024]{ 0 };
			sint oc = BStrP(instr, 1024, "enclosed string: %s", s.c_str(), opt);
			oc = Output(instr, 2);
		}
		return s;
	}
	std::string LEncStrI(cchar* str_ = "", sint opt = -1)
	{
		...
		{
			char instr[1024]{ 0 };
			sint oc = BStrP(instr, 1024, "LEncStrI(string *str_(%i), sint opt(%i))", (sint)str_, opt);
			oc = Output(instr, 2);
		}
		...
		else
		{
			...
			if (x == GetIdentsSizeV(4) && x > -1)
			{
				for (x = nstr.find("\"", 0); x > -1; )
				{
					nstr.at(x) = '\'';

					if (x < (sint)nstr.length() - 2)
					{
						x = nstr.find("\"", x + 2);
					}
					else
	...
	sint VToStr(std::vector<ullint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			{
				char icar[64]{ 0 };
				sint xc = BStrP(icar, 64, "%s%lli", sep, vec_->at(siz));
				curlen += strlen(icar);

				if (curlen < i_siz)
				{
					sint xc = BStrP(instr_, i_siz, "%s%s", instr_, vec_->at(siz));
				}
				else
				{
					siz = vec_->size();
	...
	sint VToStr(std::vector<ulint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			{
				char icar[64]{ 0 };
				sint xc = BStrP(icar, 64, "%s%lu", sep, vec_->at(siz));
				curlen += strlen(icar);
	...
	sint VToStr(std::vector<uint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			{
				char icar[64]{ 0 };
				sint xc = BStrP(icar, 64, "%s%u", sep, vec_->at(siz));
				curlen += strlen(icar);
	...
	sint VToStr(std::vector<slint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			{
				char icar[64]{ 0 };
				sint xc = BStrP(icar, 64, "%s%li", sep, vec_->at(siz));
				curlen += strlen(icar);
	...
	sint VToStr(std::vector<sint>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			{
				char icar[64]{ 0 };
				sint xc = BStrP(icar, 64, "%s%i", sep, vec_->at(siz));
				curlen += strlen(icar);
	...
	sint VToStr(std::vector<std::string>* vec_ = 0, char instr_[] = 0, size_t i_siz = 0, cchar* sep = " ")
	{
		...
		if (i_siz < BOT_STRLEN_MAX)
		{
			...
			while (siz < vec_->size())
			{
				...
				if (curlen < i_siz)
				{
					sint xc = BStrP(instr_, i_siz, "%s%s", instr_, vec_->at(siz).c_str());
				}
				else
				{
					siz = vec_->size();
	...
	std::string ITOA(sint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%i", i); if (xc > -1) { str.append(ci); }	return str;	}
	std::string LITOA(slint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%li", i); if (xc > -1) { str.append(ci); } return str; }
	std::string LLITOA(sllint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%lli", i); if (xc > -1) { str.append(ci);	} return str; }
	std::string UTOA(uint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%u", i); if (xc > -1) { str.append(ci); } return str; }
	std::string LUTOA(ulint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%lu", i); if (xc > -1) { str.append(ci); } return str; }
	std::string LLUTOA(ullint i) { std::string str = ""; char ci[21]{ 0 }; sint xc = BStrP(ci, 21, "%llu", i); if (xc > -1) { str.append(ci); } return str; }
	std::string DTOA(double i) { std::string str = ""; char ci[64]{ 0 }; sint xc = BStrP(ci, 64, "%f", i); if (xc < 64 && xc > -1) { str.append(ci); } return str; }
	std::string LDTOA(long double i) { std::string str = ""; char ci[64]{ 0 }; sint xc = BStrP(ci, 64, "%f", i); if (xc < 64 && xc > -1) { str.append(ci); } return str; }
	...
	sint FindInStr(uchar f, const uchar* str, sint at, sint lim, bool fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindInStr(uchar f(%02X), const uchar* str(%i), sint at(%i), sint lim(%i) bool fwd(%u))", f, (sint)str, at, lim, (uint)fwd);
			op = Output(outp.carr, 2);
			...
			while (at < slen && at <= lim)
			{
				...
				while (siz < flen && at + siz <= lim)
				{
					if (f == str[at + siz])
					{
						...
						{
							carr_64 outp;
							sint op = BStrP(outp.carr, outp.siz, "found at:%i", at + siz);
							op = Output(outp.carr, 2);
						}
					}
					else
	...
	sint FindInStr(uchar f, uchar* str, sint at, sint lim, bool fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindInStr(uchar f(%02X), uchar* str(%i), sint at(%i), sint lim(%i) bool fwd(%u))", f, (sint)str, at, lim, (uint)fwd);
			op = Output(outp.carr, 2);
	...
	sint FindInStr(char f, cchar* str, sint at, sint lim, bool fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindInStr(char f(%c), cchar* str(%i), sint at(%i), sint lim(%i) bool fwd(%u))", f, (sint)str, at, lim, (uint)fwd);
			op = Output(outp.carr, 2);
		...
		if (fwd)
		{
			while (at < (sint)strlen(str) && at <= lim)
			{
				if (f == str[at])
				{
					...
					{
						carr_64 outp;
						sint op = BStrP(outp.carr, outp.siz, "found at:%i", at);
						op = Output(outp.carr, 2);
					}
					return at;
				}
	...
	sint FindInStr(cchar* f, cchar* str, sint at, sint lim, bool fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindInStr(cchar* f(%i), cchar* str(%i), sint at(%i), sint lim(%i) bool fwd(%u))", (sint)f, (sint)str, at, lim, (uint)fwd);
			op = Output(outp.carr, 2);
	...
	sint FindVInStr(std::vector<std::string>* strvec_, std::string* str_, sint* loc, sint lim, sint* ele, sint ele_lim, bool fwd = true, bool v_fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindVInStr(vector<string>* strvec_(%i), string* str_(%i), sint* loc(%i), sint lim(%i), sint* ele(%i), sint ele_lim(%i), bool fwd(%u), bool v_fwd(%u))", (sint)strvec_, (sint)str_, (sint)loc, lim, (sint)ele, ele_lim, (uint)fwd, (uint)v_fwd);
			op = Output(outp.carr, 2);
		...
		if (lim < 0)
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "adjusting lim:%i to %i", lim, nlim);
				op = Output(outp.carr, 2);
			}
			lim = nlim;
		}
		if (ele_lim < 0)
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "adjusting ele_lim:%i to %i", ele_lim, nlim);
				op = Output(outp.carr, 2);
			}
			ele_lim = nlim;
		}
		...
		if (*loc >= (sint)str_->length())
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "%i >= (sint)strlen(str_)", *loc);
				op = Output(outp.carr, 2);
			}
			*loc = -1;
			return -1;
		...
		else if ((fwd && *loc > lim))
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "fwd(%u) && *loc(%i) > lim(%i)", (uint)fwd, *loc, lim);
				op = Output(outp.carr, 2);
			}
			*loc = -1;
			return -1;
		...
		else if ((v_fwd && *ele > ele_lim))
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "v_fwd(%u) && *loc(%i) > ele_lim)(%i)", (uint)v_fwd, *loc, lim);
				op = Output(outp.carr, 2);
			}
			*loc = -1;
			return -1;
		...
		{
			carr_64 outp;
			sint op = BStrP(outp.carr, outp.siz, "returning %i", ret);
			op = Output(outp.carr, 2);
		}
		*loc = ret;
		return ret;
	...
	sint FindVInStr(std::vector<char>* cvec_, std::string* str_, sint* loc, sint lim, sint* ele, sint ele_lim, bool fwd = true, bool v_fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindVInStr(vector<char>* cvec_(%i), string* str_(%i), sint* loc(%i), sint lim(%i), sint* ele(%i), sint ele_lim(%i), bool fwd(%u), bool v_fwd(%u))", (sint)cvec_, (sint)str_, (sint)loc, lim, (sint)ele, ele_lim, (uint)fwd, (uint)v_fwd);
			op = Output(outp.carr, 2);
		...
		if (*loc >= (sint)str_->length())
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "%i >= (sint)str_->length())", *loc);
				op = Output(outp.carr, 2);
			}
			*loc = -1;
			return -1;
	...
	sint FindVInStr(std::vector<cchar*>* cvec_, cchar* str_, sint* loc, sint lim, sint* ele, sint ele_lim, bool fwd = true, bool v_fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindVInStr(vector<cchar*>* strvec_(%i), cchar* str_(%i), sint* loc(%i), sint lim(%i), sint* ele(%i), sint ele_lim(%i), bool fwd(%u), bool v_fwd(%u))", (sint)cvec_, (sint)str_, (sint)loc, lim, (sint)ele, ele_lim, (uint)fwd, (uint)v_fwd);
			op = Output(outp.carr, 2);
	...
	sint FindVInStr(std::vector<char>*cvec_, cchar* str_, sint* loc, sint lim, sint* ele, sint ele_lim, bool fwd = true, bool v_fwd = true)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FindVInStr(vector<char>* strvec_(%i), cchar* str_(%i), sint* loc(%i), sint lim(%i), sint* ele(%i), sint ele_lim(%i), bool fwd(%u), bool v_fwd(%u))", (sint)cvec_, (sint)str_, (sint)loc, lim, (sint)ele, ele_lim, (uint)fwd, (uint)v_fwd);
			op = Output(outp.carr, 2);
	...
	sint OrdLocs(std::vector<size_t>* litlocs_ = 0, std::vector<size_t>* locs_ = 0, cchar* str_ = 0)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::OrdLocs(vector<size_t>* litlocs_(%i), vector<size_t>* locs_(%i), cchar* str_(%u))", (sint)litlocs_, (sint)locs_, (sint)str_);
			op = Output(outp.carr, 2);
		...
		if (!locs_->empty())
		{
			...
			if (debug_lvl >= 700 && debug_m)
			{
				...
				carr_2048 outp;
				sint op = BStrP(outp.carr, outp.siz, "OrdLocs: %s", nump.carr);
	...
	sint OrdEnc(size_t at_loc = 0, std::vector<size_t>* litlocs_ = 0, std::vector<size_t>* locs_ = 0, cchar* str_ = 0)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::OrdEnc(size_t at_loc(%u), vector<size_t>* litlocs_(%i), vector<size_t>* locs_(%i), cchar* str_(%i))", at_loc, (sint)litlocs_, (sint)locs_, (sint)str_);
			op = Output(outp.carr, 2);
		...
		for (size_t siz = 1; siz < locs_->size(); siz+=2)
		{
			if (at_loc > locs_->at(siz - 1) && at_loc < locs_->at(siz))
			{
				...
				{
					carr_128 outp;
					sint op = BStrP(outp.carr, outp.siz, " \"%c\" enclosed between: %u \"%c\" and %u \"%c\"", str_[at_loc], locs_->at(siz - 1), str_[locs_->at(siz - 1)], locs_->at(siz), str_[locs_->at(siz - 1)]);
					op = Output(outp.carr, 2);
				}
				ret = 0;
				siz = locs_->size();
		...
		{
			carr_32 outp;
			sint op = BStrP(outp.carr, outp.siz, "OrdEnc: %i", ret);
			op = Output(outp.carr, 2);
		}
		return ret;
	}
	sint LitLocs(std::vector<size_t>* locs_, cchar* str_)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::LitLocs(vector<size_t>* locs_(%i), cchar* str_(%i))", (sint)locs_, (sint)str_);
			op = Output(outp.carr, 2);
		...
		if (!locs_->empty())
		{
			if (debug_lvl >= 700 && debug_m)
			{
				...
				sint oc = VToStr(locs_, nump.carr, nump.siz);
				carr_2048 outp;
				sint op = BStrP(outp.carr, outp.siz, "LitLocs: %s", nump.carr);
				oc = Output(outp.carr, 2);
			}
			return 0;
		}
	...
	sint LitEnc(size_t at_loc, std::vector<size_t>* locs_, cchar* str_)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::LitEnc(size_t at_loc(%u), vector<size_t>* locs_(%i), cchar* str_(%i))", at_loc, (sint)locs_, (sint)str_);
			op = Output(outp.carr, 2);
		...
		{
			carr_32 outp;
			sint op = BStrP(outp.carr, outp.siz, "LitEnc: %i", ret);
			op = Output(outp.carr, 2);
		}
		return ret;
	}
	bool XEnclosedInY(std::vector<size_t>* litlocs_, std::string* statements_, sint sc_loc, sint x_loc, std::string* fy_char)
	{
		sint opens = 0;
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::XenclosedInY(vector<size_t>* litlocs_(%i), string* statements(%i), sint sc_loc(%i), sint x_loc(%i), string* fy_char \"%s\")", (sint)litlocs_, (sint)statements_, sc_loc, x_loc, fy_char->c_str());
			op = Output(outp.carr, 2);
		...
		while (fy_loc > old_fy_loc && fy_loc < x_loc)
		{
			if (LitEnc(fy_loc, litlocs_, statements_->c_str()))
			{
				...
				{
					carr_256 outp;
					sint op = BStrP(outp.carr, outp.siz, "opener found at:%i opens:%i", fy_loc, opens);
					op = Output(outp.carr, 2);
				}
				...
				while (!done)
				{
					if (nloc > fy_loc && nloc < sc_loc)
					{
						if (nloc == x_loc)
						{
							...
							{
								carr_256 outp;
								sint op = BStrP(outp.carr, outp.siz, "closer is our char at:%i opens:%i", x_loc, opens);
								op = Output(outp.carr, 2);
							}
						...
						else
						{
							if (LitEnc(nloc, litlocs_, statements_->c_str()))
							{
								...
								if (nloc < x_loc)
								{
									...
									{
										carr_256 outp;
										sint op = BStrP(outp.carr, outp.siz, "closer is before our char at:%i opens:%i", x_loc, opens);
										op = Output(outp.carr, 2);
									}
								...
								else
								{
									...
									{
										carr_256 outp;
										sint op = BStrP(outp.carr, outp.siz, "closer is after our char at:%i opens:%i", nloc, opens);
										op = Output(outp.carr, 2);
									}
									done = true;
								}
							...
							else
							{
								...
								{
									carr_256 outp;
									sint op = BStrP(outp.carr, outp.siz, "looking for another closer after:%i opens:%i", nloc + 1, opens);
									op = Output(outp.carr, 2);
								}
								nloc = statements_->find(by_char, by_loc + 1);
							}
					...
					else
					{
						...
						{
							carr_256 outp;
							sint op = BStrP(outp.carr, outp.siz, "no closer was found, opens:%i", opens);
							op = Output(outp.carr, 2);
						}
						done = true;
					}
		...
		if (opens > 0 || y)
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "a \"%s\" before this \"%c\" was open, opens:%i", fy_char->c_str(), statements_->at(x_loc), opens);
				op = Output(outp.carr, 2);
			}
			y = true;
		}
		else
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "\"%c\" not enclosed by \"%s\" (2)", statements_->at(x_loc), fy_char->c_str());
				op = Output(outp.carr, 2);
			}
		}
		return y;
	...
	bool XEnclosedInY(std::vector<size_t>* litlocs_, cchar* statements_, sint sc_loc, sint x_loc, char fy_char)
	{
		sint opens = 0;
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::XenclosedInY(vector<size_t>* litlocs_(%i), string* statements(%i), sint sc_loc(%i), sint x_loc(%i), string* fy_char \"%c\")", (sint)litlocs_, (sint)statements_, sc_loc, x_loc, fy_char);
			op = Output(outp.carr, 2);
		...
		if (opens > 0 || y)
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "a \"%c\" before this \"%c\" was open", fy_char, statements_[x_loc]);
				op = Output(outp.carr, 2);
			}
		...
		else
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "\"%c\" not enclosed by \"%c\" (4)", statements_[x_loc], fy_char);
				op = Output(outp.carr, 2);
			}
		}
	...
	sint GetMSSV(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_);
	sint GetMSSVUsages(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_);
	sint BuildMSymVec(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_);
	//sint SSeparators(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_);
	//sint SEnclosers(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_, sint sep);
	//sint SWords(PINPUT_STRUCT* PIS_, std::vector<std::vector<MICSYM>> *msvec_, sint sep);
	sint SeparateSymbolsBySymbols(PINPUT_STRUCT* PIS_, std::vector<PARSS> *vec_, sint sort_opt);
	...
	sint SeparateSymbolsI(PINPUT_STRUCT* PIS_, sint sort_opt, sint add_opt);
	sint SeparateSymbolsII(PINPUT_STRUCT* PIS_, sint sort_opt);
	sint SeparateSymbolsIII(PINPUT_STRUCT* PIS_, sint sort_opt);
	sint SeparateSymbolsIV(PINPUT_STRUCT* PIS_, sint sort_opt);
	std::vector<std::string> SeparateCharGroups(cchar* clientinput_ );
	
	sint PushToPSymVecAdd(PINPUT_STRUCT* PIS_, PARSS *Sym_, std::vector<CODE_STRUCT > *Codes_ );
	sint PushToPSymVec(PINPUT_STRUCT* PIS_, PARSS *Sym_ );
	sint PISO(PINPUT_STRUCT* PIS_, sint opt);
	...
	sint SeparateSQL(std::string* statements_ = 0, std::vector<std::string>* vec_ = 0)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::SeparateSQL(string* statements_(%i), vec_(%i))", (sint)statements_, (sint)vec_);
			op = Output(outp.carr, 2);
	...
	sint AnalyzeStmt(BOT_STMT *t_ = 0)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::AnalyzeStmt(BOT_STMT *t_(%i))", (sint)t_);
			op = Output(outp.carr, 2);

			if (debug_lvl >= 120 && t_)
			{
				carr_4096 xutp;
				op = BStrP(xutp.carr, xutp.siz, "stmt(\"%s\")", t_->stmt.c_str());
		...
		while (!snip.empty())
		{
				...
				for (altx = snip.find('.', altx + 1); altx > -1 && altx < (sint)snip.length(); altx = snip.find('.', altx + 1))
				{
					if (LitEnc(altx, &litlocs, snip.c_str()) && !XEnclosedInY(&litlocs, snip.c_str(), (sint)snip.length() - 1, altx, sql_opers_keywords[6]))
					{
						...
						while (ix > -1)
						{
							...
							{
								carr_256 outp;
								sint op = BStrP(outp.carr, outp.siz, "searching for:\"%s\" from:%i x:%i", nvec[ix].c_str(), loc, x);
								op = Output(outp.carr, 2);
							}
			...
			else
			{
				...
				else if (snip[x] == '(')
				{
					...
					else
					{
						...
						{
							carr_64 outp;
							sint op = BStrP(outp.carr, outp.siz, "%c at 0, %c at end", snip[0], snip[snip.length() - 1]);
							op = Output(outp.carr, 2);
						}
					}
							...
							while (altx > -1)
							{
								if (LitEnc(altx, &litlocs, snip.c_str()) && !XEnclosedInY(&litlocs, snip.c_str(), (sint)snip.length() - 1, altx, sql_opers_keywords[6]))
								{
									if (at_loc < spec_loc || spec_loc == -1)
									{
										...
										{
											carr_256 outp;
											sint op = BStrP(outp.carr, outp.siz, "pushing to cols:\"%s\" strlen()=%u sizeof()=%u", snip.substr(x, altx - x).c_str(), strlen(snip.substr(x, altx - x).c_str()), sizeof(snip.substr(x, altx - x).c_str()));
											op = Output(outp.carr, 2);
										}
									...
									else
									{
										...
										{
											carr_256 outp;
											sint op = BStrP(outp.carr, outp.siz, "pushing to vals:\"%s\" strlen()=%u sizeof()=%u", snip.substr(x, altx - x).c_str(), strlen(snip.substr(x, altx - x).c_str()), sizeof(snip.substr(x, altx - x).c_str()));
											op = Output(outp.carr, 2);
										}
										t_->vals.push_back(snip.substr(x, altx - x));
									}
			...
			if (yv < (sint)o_locs.size())
			{
				...
				{
					carr_256 outp;
					sint op = BStrP(outp.carr, outp.siz, "snipping from:%u to:%u len=%u at_loc=%i spec_loc=%i open(%u)", o_locs[xv], o_locs[yv], o_locs[yv] - o_locs[xv], at_loc, spec_loc, (uint)open);
					op = Output(outp.carr, 2);
				}
	...
	sint BQS(BOT_STMT *t_ = 0)
	{
		...
		{
			carr_64 outp;
			sint oc = BStrP(outp.carr, outp.siz, "::BQS(BOT_STMT *t_(%i))");
			oc = Output(outp.carr, 2);
		}
	...
	sint BTS(BOT_STMT *t_ = 0)
	{
		...
		{
			carr_64 outp;
			sint oc = BStrP(outp.carr, outp.siz, "::BTS(BOT_STMT *t_(%i))", (sint)t_);
			oc = Output(outp.carr, 2);
		}
		...
		if (t_->tables.empty())
		{
			if (!t_->dbname.empty() && t_->it_type == 9)
			{
				statement.clear();
				carr_512 outp;
				sint op = BStrP(outp.carr, outp.siz, "%s %s AS %s;", sql_trans_keywords[t_->it_type], t_->dbname.c_str(), t_->dbname.c_str());
		...
		else
		{
			...
			while (tx < t_->tables.size())
			{
				...
				if (t_->ic_types[tx] > 0)
				{
					...
					{
						carr_256 outp;
						sint op = BStrP(outp.carr, outp.siz, "Could not find tablename \"%s\" in vectors.", t_->tables[0].c_str());
			...
			if (t_->opts[9].size())
			{
				...
				case 0:
				{
					if (t_->vals.size() && t_->cols.size())
					{
						...
						{
							carr_256 outp;
							sint op = BStrP(outp.carr, outp.siz, " %s = %s, ", t_->cols[x].c_str(), t_->vals[x].c_str());
							statement.append(outp.carr);
							x++;
						}
						statement.pop_back();
			...
			if (t_->opts[0].size())
			{
				...
				else if (t_->opts[0][0] == 2)
				{
					...
					{
						carr_64 outp;
						sint op = BStrP(outp.carr, outp.siz, "columns(%u) vals(%u) mismatch.", t_->cols.size(), t_->vals.size());
						op = Output(outp.carr, 2);
						return 1;
					}
				}
	...
	sint BuildStatements(std::vector<BOT_STMT>* stmts_ = 0, BOT_COMMIT *com_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::BuildStatements((std::vector<BOT_STMT>* stmts_(%i), BOT_COMMIT *com_)(%i))", (sint)stmts_, (sint)com_);
			op = Output(outp.carr, 2);
		}
		...
		for (uint i = 0; i < stmts_->size(); i++)
		{
			...
			if (!xc)
			{
				...
				if (dx < 0)
				{
						...
						{
							dx = vtool.AddValToVec(&cont_names[cont_names.size() - 1], &stmts_->at(i).tables[tsiz], true, false);
						}
						com_->t.push_back(stmts_->at(i));
						stmts_->at(i).Clear();
				}
				else
				{
					...
					for (size_t tsiz = 0; tsiz < stmts_->at(i).tables.size(); tsiz++)
					{
						xc = vtool.AddValToVec(&cont_names[dx], &stmts_->at(i).tables[tsiz], true, false);
					}
					com_->t.push_back(stmts_->at(i));
					stmts_->at(i).Clear();
				}
		...
		vtool.CleanVec(stmts_);

		sint hx = HasGMutexLocked(MTX_PEND);
		sint ox = -1;

		if (hx < 0)
		{
			ox = LockGMutex(MTX_PEND, true);
			hx = ox;
		}

		if (hx > -1)
		{
			for (size_t csiz = 0; csiz < bot_t_pend.size(); csiz++)
			{
				sint ax = vtool.AddValToVec(&com_->t, &bot_t_pend[csiz], true, true);
			}
		...
		for (sint i = (sint)com_->db_names.size() - 1; i > -1; i--)
		{
			...
			{
				carr_256 ncar;
				sint xc = BStrP(ncar.carr, ncar.siz, "building necessary statements for %s", com_->db_names[i].c_str());
	...
	sint OutpuTINFO(BOT_STMT *t_ = 0)
	{
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, " it_type:%i dbname:\"%s\"\ntables:", t_->it_type, t_->dbname.c_str());
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, " %i:%s", o, t_->tables[o].c_str());
			output.append(ncar.carr);
		}
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, " %i:%i %s", o, t_->ic_types[o], sql_obj_keywords[t_->ic_types[o]]);
			output.append(ncar.carr);
		}
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, "\nt_ has %u vals eles: ", t_->vals.size());
			output.append(ncar.carr);
		}
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, "| %s ", t_->vals[o].c_str());
			output.append(ncar.carr);
		}
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, "| %s ", t_->cols[o].c_str());
			output.append(ncar.carr);
		}
		...
		{
			carr_256 ncar;
			sint xp = BStrP(ncar.carr, ncar.siz, "| %s ", t_->conds[o].c_str());
			output.append(ncar.carr);
		}
		...
		for (uint o = 0; o < t_->opts.size(); o++)
		{
			if (!t_->opts[o].empty())
			{
				...
				{
					carr_256 ncar;
					sint xp = BStrP(ncar.carr, ncar.siz, ", %i ", t_->opts[o][xc]);
					output.append(ncar.carr);
				}
			}
		}
	...
	sint GetDBConn(BOT_DB_M* val_ = 0, sint opt = -1)
	{
		if (opt < 0 || !val_)
		{
			return -1;
		...
		sint ret = -1;
		sint h = HasGMutexLocked(MTX_DBC);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_DBC);
			h = p;
		}
		...
		if (h > -1)
		{
			if (olim < BOT_ATT_LIM)
			{
				...
				{
					sint xc = LockElement(MTX_DBC, siz, false, false, false);
					...
					if (!xc)
					{
						...
						{
							xc = UnlockElement(MTX_DBC, siz, false);
						}
					}
				}
			}
	...
	sint OpenDB(cchar *db_name_, sint x = -1, sint* was_open = 0)
	{
		...
		{
			carr_256 outp;
			sint oc = BStrP(outp.carr, outp.siz, "::OpenDB(cchar* db_name_ \"%s\", sint x(%i), sint* was_open(%i))", db_name_, x, (sint)was_open);
			oc = Output(outp.carr, 2);
		}
		...
		BOT_DB_M dbg;
		sint gc = GetVecEleByName((void*)&dbg, MTX_DBL, db_name_);
		...
		if (gc < 0)
		{
			...
			{
				carr_256 outp;
				sint oc = BStrP(outp.carr, outp.siz, "DB \"%s\" not found in list.", db_name_);
				oc = Output(outp.carr, 2);
			}
		}
		...
		if (gc > -1)
		{
			lx = LockElement(MTX_DBL, dbg.lid, false, true, false);
			...
			if (LockRet(lx) < 0)
			{
				...
				{
					carr_256 outp;
					sint oc = BStrP(outp.carr, outp.siz, "Failed to acquire mtx for DB \"%s\"(%i)", db_name_, dbg.lid);
					oc = Output(outp.carr, 2);
				}
				return -2;
			}
		...
		{
			carr_256 outp;
			sint oc = BStrP(outp.carr, outp.siz, "Opening DB \"%s\"(%i) olim(%i) exists(%i)", db_name_, dbg.lid, olim, dbg.exists);
			oc = Output(outp.carr, 2);
		}
		...
		if (!d_con_)
		{
			...
			for (dbc = GetDBConn(&dbg, x); dbc < 0; dbc = GetDBConn(&dbg, x))
			{
				slint rst = nsRest(BOT_NANO_REST);
			}
			...
			if (dbc > -1)
			{
				...
				else
				{
					...
					{
						carr_256 outp;
						sint op = BStrP(outp.carr, outp.siz, "invalid open opt:%i for %s", x, db_name_);
						op = Output(outp.carr, 2);
					}
					if (!lx)
					{
						sint oc = UnlockElement(MTX_DBL, dbg.lid, false);
					}
					if (dbc > -1)
					{
				...
				if (rc)
				{
					...
					{
						carr_128 outp;
						sint op = BStrP(outp.carr, outp.siz, "%s", GetError().c_str());
						op = Output(outp.carr, 1);
						op = Output(outp.carr, 2);
					}
				...
				d_con_->Renew(db_name_, x, lid);
			}
		}
		else
		{
			if ((dbg.exists > -1) && !d_con_->o_com && strcmp(d_con_->name.c_str(), db_name_))
			{
				carr_256 bstmt;
				sint op = BStrP(bstmt.carr, bstmt.siz, "ATTACH %s AS %s;", db_name_, db_name_);
				cchar* tail;
				sqlite3_stmt* ppstmt;
				...
				if (!rc)
				{
					...
					while (!done)
					{
						...
						case SQLITE_BUSY:
						{
							...
							slint op = nsRest(BOT_NANO_REST, true);
							break;
						}
						default:
						{
				...
				else
				{
					...
					if (!lx)
					{
						op = UnlockElement(MTX_DBL, dbg.lid, false);
					}
					return -5;
				}
			}
		...
		uint nulim = UINT_MAX;
		sint hlim = HasGMutexLocked(MTX_DBC);
		sint plim = -1;

		if (hlim < 0)
		{
			plim = LockGMutex(MTX_DBC, true);
			hlim = plim;
		}
		...
		if (hlim > -1)
		{
			...
			if (plim > -1)
			{
				plim = UnlockGMutex(MTX_DBC, hlim);
			}
		}
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "DB \"%s\" opened, olim is:%i", dbg.name.c_str(), nulim);
			op = Output(outp.carr, 2);
		}
		return 0;
	}
	sint CloseDB(cchar* db_name_ = 0)
	{
		...
		{
			carr_256 outp;
			sint oc = BStrP(outp.carr, outp.siz, "::CloseDB(sint ele(%i))", (sint)db_name_);
			oc = Output(outp.carr, 2);
		}
		...
		sint rc = HasDBOpen(db_name_, d_con_->omode);
		...
		if (rc < 0)
		{
			...
			{
				carr_256 outp;
				sint oc = BStrP(outp.carr, outp.siz, "DB \"%s\" connection(%i) not found.", db_name_, d_con_->omode);
				oc = Output(outp.carr, 2);
			}
			return rc;
		}
		...
		{
			sint ex = vtool.ValInVec(&d_con_->att, db_name_);
			...
			carr_256 bstmt;
			sint xc = BStrP(bstmt.carr, bstmt.siz, "DETACH %s;", d_con_->att[ex].c_str());
			cchar* tail;
			sqlite3_stmt* ppstmt;

			if (debug_lvl >= 9 && debug_m)
			{
				sint oc = Output("preparing statement:", 2);

				if (debug_lvl >= 9 && debug_m)
				{
					oc = Output(bstmt.carr, 2);
				}
			}
			...
			{
				std::string output;
				sint oc = IGetError(&output, 0);
				oc = Output(output.c_str(), 1);
			...
			else
			{
				if (debug_lvl >= 9 && debug_m)
				{
					sint oc = Output("statement prepared.", 2);
				...
				while (!done)
				{
					...
					case SQLITE_DONE:
					{
						if (debug_lvl >= 9 && debug_m)
						{
							xc = Output("SQLITE_DONE/OK", 2);
						}

						vtool.ClearEleFromVec(&d_con_->att, ex);
						vtool.CleanVec(&d_con_->att);
						uint nulim = UINT_MAX;
						sint hlim = HasGMutexLocked(MTX_DBC);
						sint plim = -1;
						...
						xc = UnlockElementByName(MTX_DBL, db_name_, false);
						...
						{
							carr_256 outp;
							sint oc = BStrP(outp.carr, outp.siz, "unlocked DB \"%s\" olim:%i.", db_name_, nulim);
							oc = Output(outp.carr, 2);
						}
						done = true;
						break;
		...
		else
		{
			...
			if (d_con_->att.empty())
			{
				...
				else
				{
					...
					rc = UnlockElementByName(MTX_DBL, db_name_, false);
	...
	sint OpenDBs(std::vector <std::string> *db_names_, sint opt = -1, std::vector <sint> *wos_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			...
			{
				carr_256 outp;
				sint oc = BStrP(outp.carr, outp.siz, "::OpenDBs(std::vector <std::string> *db_names_(%i), sint opt(%i), std::vector <sint> *wos_(%i))", (sint)db_names_, opt, (sint)wos_);
				oc = Output(outp.carr, 2);
			}
		}
	...
	sint CloseDBs(std::vector<std::string> *names_ = 0)
	{
		...
		{
			carr_256 outp;
			sint oc = BStrP(outp.carr, outp.siz, "::CloseDBs(std::vector<sint>* names_(%i))", (sint)names_);
			oc = Output(outp.carr, 2);
		}
	...
	sint Commit(std::vector<BOT_STMT>* stmts_ = 0)
	{
		...
		{
			carr_64 outp;
			sint op = BStrP(outp.carr, outp.siz, "::Commit(BOT_COMMIT* com_(%i))", (sint)stmts_);
			op = Output(outp.carr, 2);
		}
		...
		while (!done)
		{
			...
			if (!done)
			{
				sint xc = BuildStatements(stmts_, &Comm);
					...
					if (vv > -1)
					{
						...
						if (Comm.t[nsiz].it_type > 0 && !began || Comm.t[nsiz].it_type < 1 && began)
						{
							...
							else
							{
								...
								if (rc)
								{
									sint xp = Output(GetError().c_str(), 1);
								}
							}
						}
						if (!rc)
				...
				if (!Comm.t.empty())
				{
					...
					if (hx > -1)
					{
						for (size_t csiz = 0; csiz < Comm.t.size(); csiz++)
						{
							...
							if (rc < 0)
							{
								bot_t_pend.push_back(Comm.t[csiz]);
							}
							else
							{
	...
	sint VerifyDatabase(BOT_DB *dbs_)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::VerifyDatabase(BOT_DB *dbs_(%i, \"%s\"))", dbs_->lid, dbs_->name.c_str());
			op = Output(outp.carr, 2);
		}
		...
		if (odb > -1)
		{
			...
			for (size_t cc = 0; cc < dbs_->tables.size(); cc++)
			{
				...
				else
				{
					if (!dbs_->type)
					{
						/*if (!strcmp("SYMS", dbs_->tables[cc].c_str()))
						{
						v = VerifyBasicIdents(dbs_, &tvec, cc, -1);

						if (v)
						{
						xc++;
						string output;
						output.append("error verifying basic_idents");
						v = Output(output.c_str(), 1);
						v = Output(output.c_str(), 2);
						}
						}*/
					}
				}
				...
				if (cc == dbs_->tables.size())
				{
					if (xc)
					{
						...
						{
							carr_64 outp;
							sint op = BStrP(outp.carr, outp.siz, "%i tables failed.", xc);
							op = Output(outp.carr, 2);
						}
						if (tvec.size())
						{
	...
	sint VerifyTable(BOT_DB *dbs_, std::vector<BOT_STMT> *tvec_, sint x, sint odb = -1)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_512 outp;
			sint op = BStrP(outp.carr, outp.siz, "::VerifyTable(BOT_DB *dbs_(%i, %s.%s), std::vector<BOT_STMT> *tvec_(%i), sint x(%i), sint odb(%i))", dbs_->lid, dbs_->name.c_str(), dbs_->tables[x].c_str(), (sint)tvec_, x, odb);
			op = Output(outp.carr, 2);
		}
		...
		while (!xxc)
		{
			...
			if (rc)
			{
				...
				if (!dbs_->type && x == 4)
				{
					sint h = HasGMutexLocked(MTX_STRT);
					sint p = -1;

					if (h < 0)
					{
						p = LockGMutex(MTX_STRT, true);
						h = p;
					}
						...
						for (sint stcount = (sint)dbs_->columns[x][0].size() - 1; stcount > -1; stcount--)
						{
							...
							if (rc)
							{
								...
								else if (rc == -3)
								{
									...
									{
										carr_512 outp;
										sint op = BStrP(outp.carr, outp.siz, "%s found but of type %s not found in table %s.%s", dbs_->columns[x][0][stcount].c_str(), dbs_->columns[x][1][stcount].c_str(), dbs_->name.c_str(), dbs_->tables[x].c_str());
										op = Output(outp.carr, 2);
									}
								}
								else {}
						...
						else
						{
							if (!m_cols.empty())
							{
								...
								for (uint m = 0; m < (sint)m_cols.size(); m += 2)
								{
									...
									{
										carr_512 outp;
										sint op = BStrP(outp.carr, outp.siz, "%s %s not found in table %s.%s", m_cols[m].c_str(), m_cols[m + 1].c_str(), dbs_->name.c_str(), dbs_->tables[x].c_str());
										op = Output(outp.carr, 2);
										op = Output(outp.carr, 1);
									}
						...
						for (sint stcount = (sint)dbs_->columns[x][0].size() - 1; stcount > -1; stcount--)
						{
							...
							if (rc)
							{
								...
								else if (rc == -3)
								{
									...
									{
										carr_512 outp;
										sint op = BStrP(outp.carr, outp.siz, "%s %s not found in table %s.%s", dbs_->columns[x][0][stcount].c_str(), dbs_->columns[x][1][stcount].c_str(), dbs_->name.c_str(), dbs_->tables[x].c_str());
										op = Output(outp.carr, 2);
										op = Output(outp.carr, 1);
									}
								}
	...
	sint CreateDatabase(BOT_DB *dbs_ = 0)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::CreateDatabase(BOT_DB *dbs_(%i))", (sint)dbs_);
			op = Output(outp.carr, 2);
		}
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "creating db: \"%s\"", dbs_->name.c_str());
			op = Output(outp.carr, 2);
		}
	...
	sint GetChars(std::vector<PARSS> *pvec_, std::string* lang_, sint opt);
	...
	sint GetSymbols(std::vector <MICSYM> *syms_ = 0, char x_cond_[] = 0, bool maint = false)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::GetSymbols(std::vector <MICSYM> *syms_(%i), cchar* x_cond_ \"%s\", bool maint(%u))", (sint)syms_, x_cond_, (uint)maint);
			op = Output(outp.carr, 2);
		}
			...
			for (size_t nsiz = 0; nsiz < syms_->size(); nsiz++)
			{
				if (syms_->at(nsiz).id > -2)
				{
					...
					if (!maint)
					{
						...
						xc = BStrP(dval, sizeof(dval), "SYMBOL = X'%02X'", syms_->at(nsiz).sym);
					...
					if (x_cond_)
					{
						if (strlen(x_cond_))
						{
							...
							xc = BStrP(outc, sizeof(outc), "%s", x_cond_);
					...
					{
						char outc[32]{ 0 };
						xc = BStrP(outc, 32, "ID = %i", syms_->at(nsiz).id);
			...
			for (size_t siz = 0; siz < stmts.size(); siz++)
			{
				...
				else
				{
					...
					while (!done)
					{
						...
						{
							carr_128 ncar;
							sint xp = BStrP(ncar.carr, ncar.siz, "%s", GetError().c_str());
	...
	sint AddSymbols(std::vector <MICSYM> *syms_ = 0)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::AddSymbols(std::vector <MICSYM> *syms_(%i))", (sint)syms_);
			op = Output(outp.carr, 2);
		}
		...
		for (size_t x = 0; x < eles.size(); x++)
		{
			...
			if (syms_->at(eles[x]).id < 0)
			{
				...
				if (sizeof(syms_->at(eles[x]).sym))
				{
					...
					sint xp = BStrP(xstr, sizeof(xstr), "X'%02X'", syms_->at(eles[x]).sym);
				...
				if (!syms_->at(eles[x]).encodings.empty())
				{
					...
					for (size_t xs = 0; xs < syms_->at(eles[x]).encodings.size(); xs++)
					{
						...
						sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)syms_->at(eles[x]).encodings[xs]);
						str.append(ci);
					}
					if (!str.empty())
					{
					...
					for (size_t xs = 0; xs < syms_->at(eles[x]).constituent_to.size(); xs++)
					{
						for (size_t nsiz = 0; nsiz < syms_->at(eles[x]).constituent_to[xs].size(); nsiz++)
						{
							for (size_t xsiz = 0; xsiz < syms_->at(eles[x]).constituent_to[xs][nsiz].size(); xsiz++)
							{
								...
								sint xp = BStrP(ci, sizeof(ci), "%02X", syms_->at(eles[x]).constituent_to[xs][nsiz][xsiz]);
								str.append(ci);
							}
							char xi[(sizeof(sep) * 2) + 1]{ 0 };
							sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
							str.append(xi);
						}
						char xi[(sizeof(lsep) * 2) + 1]{ 0 };
						sint xp = BStrP(xi, sizeof(xi), "%02X", lsep);
						str.append(xi);
					}
					if (!str.empty())
					{
				...
				if (!syms_->at(eles[x]).history_following.empty())
				{
					...
					for (size_t xx = 0; xx < syms_->at(eles[x]).history_following.size(); xx++)
					{
						for (size_t xs = 0; xs < syms_->at(eles[x]).history_following[xx].size(); xs++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", syms_->at(eles[x]).history_following[xx][xs].sym);
							str.append(ci);
							char xi[(sizeof(syms_->at(eles[x]).history_following[xx][xs].ct) * 2) + 1]{ 0 };
							xp = BStrP(xi, sizeof(xi), "%02X", (uchar)syms_->at(eles[x]).history_following[xx][xs].ct);
							str.append(xi);
						}
						char xi[(sizeof(sep) * 2) + 1]{ 0 };
						sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
				...
				if (!syms_->at(eles[x]).history_preceeding.empty())
				{
					...
					for (size_t xx = 0; xx < syms_->at(eles[x]).history_preceeding.size(); xx++)
					{
						for (size_t xs = 0; xs < syms_->at(eles[x]).history_preceeding[xx].size(); xs++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", syms_->at(eles[x]).history_preceeding[xx][xs].sym);
							str.append(ci);
							char xi[(sizeof(syms_->at(eles[x]).history_preceeding[xx][xs].ct) * 2) + 1]{ 0 };
							xp = BStrP(xi, sizeof(xi), "%02X", (uchar)syms_->at(eles[x]).history_preceeding[xx][xs].ct);
							str.append(xi);
						}
						char xi[(sizeof(sep) * 2) + 1]{ 0 };
						sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
			...
			else
			{
				...
				if (syms_->at(eles[x]).ud_encodings)
				{
					...
					for (size_t a = 0; a < syms_->at(eles[x]).encodings.size(); a++)
					{
						...
						sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)syms_->at(eles[x]).encodings[a]);
						str.append(ci);
					}
					if (!str.empty())
					{
				...
				if (syms_->at(eles[x]).ud_history_following)
				{
					...
					for (size_t a = 0; a < syms_->at(eles[x]).history_following.size(); a++)
					{
						for (size_t b = 0; b < syms_->at(eles[x]).history_following[a].size(); b++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", syms_->at(eles[x]).history_following[a][b].sym);
							str.append(ci);
							char xi[(sizeof(syms_->at(eles[x]).history_following[a][b].ct) * 2) + 1]{ 0 };
							xp = BStrP(xi, sizeof(xi), "%02X", (uchar)syms_->at(eles[x]).history_following[a][b].ct);
							str.append(xi);
						}
						char xi[(sizeof(sep) * 2) + 1]{ 0 };
						sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
				...
				if (syms_->at(eles[x]).ud_history_preceeding)
				{
					...
					for (size_t a = 0; a < syms_->at(eles[x]).history_preceeding.size(); a++)
					{
						for (size_t b = 0; b < syms_->at(eles[x]).history_preceeding[a].size(); b++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", syms_->at(eles[x]).history_preceeding[a][b].sym);
							str.append(ci);
							char xi[(sizeof(syms_->at(eles[x]).history_preceeding[a][b].ct) * 2) + 1]{ 0 };
							xp = BStrP(xi, sizeof(xi), "%02X", (uchar)syms_->at(eles[x]).history_preceeding[a][b].ct);
							str.append(xi);
						}
						char xi[(sizeof(sep) * 2) + 1]{ 0 };
						sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
				...
				else
				{
					...
					xc = BStrP(str_, sizeof(str_), "SYMBOL = X'%02X'", syms_->at(eles[x]).sym);
	...
	sint GetCode(std::vector<MACSYM>* codes_ = 0, char x_cond_[] = 0, bool maint = false)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::GetCode(std::vector <MACSYM> *codes_(%i), cchar* x_cond_ \"%s\", bool maint(%u))", (sint)codes_, x_cond_, (uint)maint);
			op = Output(outp.carr, 2);
		}
		...
		if (odb > -1)
		{
					...
					for (size_t siz = 1; siz < cdb.tables.size(); siz++)
					{
						...
						if (!maint)
						{
							...
							for (size_t siz = 0; siz < codes_->at(xsiz).cons.size(); siz++)
							{
								...
								xc = BStrP(nchr, sizeof(nchr), "%02X", codes_->at(xsiz).cons[siz].sym);
								str.append(nchr);
							}
							if (!str.empty())
							{
						...
						{
							char outc[32]{ 0 };
							xc = BStrP(outc, 32, "ID = %i", codes_->at(xsiz).id);
			...
			if (!rc)
			{
				...
				{
					rc = GetSymbols(&codes_->at(siz).cons, "", maint);
				}
			}
	...
	sint AddCode(std::vector<MACSYM > *codes_ = 0)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::AddCode(std::vector <MACSYM> *codes_(%i))", (sint)codes_->size());
			op = Output(outp.carr, 2);
		}
		...
		for (size_t x = 0; x < eles.size(); x++)
		{
			...
			if (!codes_->at(eles[x]).cons.empty())
			{
				for (size_t a = 0; a < codes_->at(eles[x]).cons.size(); a++)
				{
					...
					sint xp = BStrP(ci, sizeof(ci), "%02X", codes_->at(eles[x]).cons[a].sym);
					con_str.append(ci);
				}
			}
				...
				if (codes_->at(eles[x]).id < 0)
				{
					...
					if (!codes_->at(eles[x]).usages.empty())
					{
						...
						for (size_t xc = 0; xc < codes_->at(eles[x]).usages.size(); xc++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)codes_->at(eles[x]).usages[xc]);
							str.append(ci);
						}
						if (!str.empty())
						{
					...
					if (!codes_->at(eles[x]).used.empty())
					{
						...
						for (size_t xc = 0; xc < codes_->at(eles[x]).used.size(); xc++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)codes_->at(eles[x]).used[xc]);
							str.append(ci);
						}
						if (!str.empty())
						{
						...
						for (size_t a = 0; a < codes_->at(eles[x]).history_following.size(); a++)
						{
							for (size_t b = 0; b < codes_->at(eles[x]).history_following[a].size(); b++)
							{
								for (size_t c = 0; c < codes_->at(eles[x]).history_following[a][b].cons.size(); c++)
								{
									...
									sint xp = BStrP(ci, sizeof(ci), "%02X", codes_->at(eles[x]).history_following[a][b].cons[c].sym);
									str.append(ci);
								}
								char xi[(sizeof(codes_->at(eles[x]).history_following[a][b].ct) * 2) + 1]{ 0 };
								sint xp = BStrP(xi, sizeof(xi), "%02X", (uchar)codes_->at(eles[x]).history_following[a][b].ct);
								str.append(xi);
								char ci[(sizeof(sep) * 2) + 1]{ 0 };
								xp = BStrP(xi, sizeof(ci), "%02X", sep);
								str.append(ci);
							}
							char xi[(sizeof(lsep) * 2) + 1]{ 0 };
							sint xp = BStrP(xi, sizeof(xi), "%02X", lsep);
						...
						for (size_t a = 0; a < codes_->at(eles[x]).history_preceeding.size(); a++)
						{
							for (size_t b = 0; b < codes_->at(eles[x]).history_preceeding[a].size(); b++)
							{
								for (size_t c = 0; c < codes_->at(eles[x]).history_preceeding[a][b].cons.size(); c++)
								{
									...
									sint xp = BStrP(ci, sizeof(ci), "%02X", codes_->at(eles[x]).history_preceeding[a][b].cons[c].sym);
									str.append(ci);
								}
								char xi[(sizeof(codes_->at(eles[x]).history_preceeding[a][b].ct) * 2) + 1]{ 0 };
								sint xp = BStrP(xi, sizeof(xi), "%02X", (uchar)codes_->at(eles[x]).history_preceeding[a][b].ct);
								str.append(xi);
								char ci[(sizeof(sep) * 2) + 1]{ 0 };
								xp = BStrP(xi, sizeof(ci), "%02X", sep);
								str.append(ci);
					...
					if (!codes_->at(eles[x]).alters.empty())
					{
						...
						for (size_t xc = 0; xc < codes_->at(eles[x]).alters.size(); xc++)
						{
							for (size_t nsiz = 0; nsiz < codes_->at(eles[x]).alters[xc].size(); nsiz++)
							{
								...
								sint xp = BStrP(ci, sizeof(ci), "%02X", codes_->at(eles[x]).alters[xc]);
								str.append(ci);
							}
							char xi[(sizeof(sep) * 2) + 1]{ 0 };
							sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
				...
				else
				{
					...
					if (codes_->at(eles[x]).ud_usages)
					{
						...
						for (size_t a = 0; a < codes_->at(eles[x]).usages.size(); a++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)codes_->at(eles[x]).usages[a]);
							str.append(ci);
						}
						if (!str.empty())
						{
					...
					if (codes_->at(eles[x]).ud_used)
					{
						...
						for (size_t a = 0; a < codes_->at(eles[x]).used.size(); a++)
						{
							...
							sint xp = BStrP(ci, sizeof(ci), "%02X", (uchar)codes_->at(eles[x]).used[a]);
							str.append(ci);
						}
						if (!str.empty())
						{
					...
					if (codes_->at(eles[x]).ud_alters)
					{
						...
						for (size_t a = 0; a < codes_->at(eles[x]).alters.size(); a++)
						{
							for (size_t b = 0; b < codes_->at(eles[x]).alters[a].size(); b++)
							{
								...
								sint xp = BStrP(ci, sizeof(ci), "%02X", codes_->at(eles[x]).alters[a]);
								str.append(ci);
							}
							char xi[(sizeof(sep) * 2) + 1]{ 0 };
							sint xp = BStrP(xi, sizeof(xi), "%02X", sep);
	...
	sint GetWord(cchar *str_ = 0, std::vector<WORD_STRUCT > *Words_ = 0, char x_cond_[] = 0, sint row = -1);
	sint AddWords(std::vector<WORD_STRUCT > *words_ = 0);
	sint GetCommand(BOTCOMMAND *Command_ = 0, std::vector<BOTCOMMAND > *Cmds_ = 0, std::vector<std::string> *conds_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetCommand(BOTCOMMAND* Command_ \"%s\", vector<BOTCOMMAND>* Cmds_(%i), vector<string>* conds_(%i))", Command_->cmd.c_str(), (sint)Cmds_->size(), (sint)conds_->size());
			op = Output(outp.carr, 2);
		}
	...
	sint GetLogin(cchar* str_ = "")
	{
		...
		{
			carr_256 ncar;
			sint xc = BStrP(ncar.carr, ncar.siz, "::GetLogin(cchar* str_(%i))", (sint)str_);
			xc = Output(ncar.carr, 2);
		}
		...
		if (odb > -1)
		{
			...
			if (!xc)
			{
				...
				if (!xc)
				{
					...
					if (xc)
					{
						sint op = Output(GetError().c_str(), 1);
					}
				}
				else
				{
	...
	sint UpdateAccount(BOT_CLIENT *Client_ = 0)
	{
		...
		{
			carr_128 ncar;
			sint xc = BStrP(ncar.carr, ncar.siz, "::UpdateAccount(BOT_CLIENT* Client_(%i))", (sint)Client_);
			xc = Output(ncar.carr, 2);
		}
		if (!Client_)
		{
		...
		condstr.append(LEncStrI(&Client_->login_name, -1));
		...
		if (Client_->loggedin)
		{
			if (Client_->changename)
			{
				...
				xc = t.AddCol("LOGIN_NAME", LEncStrI(&str, -1).c_str());
			}
			...
			{
				xc = t.AddCol("LAST_LOGIN", LEncStrI(&Client_->logindate, -1).c_str());
		...
		else
		{
			if (Client_->updatelogin)
			{
				xc = t.AddCol("LAST_FAILED_LOGIN", LEncStrI(&Client_->last_failed_login, -1).c_str());
	...
	sint GetLastAccess(BOT_DB *dbs_ = 0, sint *last_modi_ = 0, sint *last_modi_ns_ = 0, sint *last_maint_ = 0, sint *last_maint_ns_ = 0)
	{
		...
		{
			carr_512 outp;
			sint op = BStrP(outp.carr, outp.siz, "GetLastAccess(BOT_DB* dbs_ \"%s\", sint *last_modi_(%i), sint *last_modi_ns_(%i), sint *last_maint_(%i), sint *last_maint_ns_(%i))(", dbs_->name.c_str(), *last_modi_, *last_modi_ns_, *last_maint_, *last_maint_ns_);
			op = Output(outp.carr, 2);
		}
	...
	sint LastAccessMaintenance(BOT_DB *dbs_ = 0)
	{
		...
		{
			carr_256 outp;
			sint xc = BStrP(outp.carr, outp.siz, "::LastAccessMaintenance(BOT_DB *dbs_(%i)", (sint)dbs_);
	...
	sint PushToDBList(BOT_DB *dbs_ = 0)
	{
		...
		{
			carr_64 outp;
			BStrP(outp.carr, outp.siz, "PushToDBList(BOT_DB *dbs_(%i))", (sint)dbs_);
			sint op = Output(outp.carr, 2);
		}
		...
		if (h > -1)
		{
			...
			case 3:
			{
				AddStrToVec(&dbs_->tables, "LAST_ACCESS", false, false);
				// +1 so we can create a single letter table like A.[A]

				for (sint t = 0; t < GetIdentsSizeV(1) + GetIdentsSizeV(2) + 1; t++)
				{
					std::string letter;
					letter.append(dbs_->name.c_str());

					if (t)
					{
						if (t - 1 < GetIdentsSizeV(2))
						{
							PARSS Sym;
							sint xc = GetIdentsVE(&Sym, 2, t - 1);
							letter.append(Uppercase(Sym.sym.c_str()));
						}
						else
						{
							PARSS Sym;
							sint xc = GetIdentsVE(&Sym, 1, t - 1 - GetIdentsSizeV(2));
							letter.append(Uppercase(Sym.sym.c_str()));
						}
					}
					uint xt = 0;

					while (xt < dbs_->tables.size())
					{
						if (!strcmp(dbs_->tables[xt].c_str(), letter.c_str()))
						{
							xt = dbs_->tables.size();
						}
						xt++;
					...
					if (xt == dbs_->tables.size())
					{
						AddStrToVec(&dbs_->tables, letter.c_str(), false, false);
					}
				}

				for (uint t = 0; t < dbs_->tables.size(); t++)
				{
					std::vector<std::vector<std::string>> tcols;
					dbs_->columns.push_back(tcols);
					std::vector<std::string> cols, colss;
					...
					if (t < bot_strt.word_columns.size())
					{
						std::string str;
						str.append(bot_strt.word_columns[t]);
						sint xp = SeparateStrByChar(&str, &c, &cols, 1);
					}
					else
					{
						std::string str;
						str.append(bot_strt.word_columns[(sint)bot_strt.word_columns.size() - 1]);
						sint xp = SeparateStrByChar(&str, &c, &cols, 1);
					}
	...
	sint SetTTSVEleM(sint e, sint m, void* vp_ = 0, bool is_meta = true)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::SetTTSVEleM(sint e(%i), sint m(%i), void* vp_(%i), bool is_meta(%u))", e, m, (sint)vp_, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = -1;
		sint h = HasGMutexLocked(MTX_TTSV);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_TTSV, true);
			h = p;
		}

		if (h > -1)
		{
			sint x = LockElement(MTX_TTSV, e, false, true, is_meta);
			...
			if (LockRet(x) > -1)
			{
				...
				if (!x)
				{
					x = UnlockElement(MTX_TTSV, e, is_meta);
				}
			}
			if (p > -1)
			{
				p = UnlockGMutex(MTX_TTSV, h);
			}
		}
		return ret;
	}
	sint GetTTSVEleM(sint e, sint m, void* vp_, bool is_meta = true)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::GetTTSVEleM(sint e(%i), sint m(%i), void* vp_(%i), bool is_meta(%u))", e, m, (sint)vp_, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
		...
		if (h > -1)
		{
			...
			if (LockRet(x) > -1)
			{
				...
				if (!x)
				{
					p = UnlockElement(MTX_TTSV, e, is_meta);
				}
			}
			if (p > -1)
			{
				x = UnlockGMutex(MTX_TTSV, h);
			}
		}
		return ret;
	}
	sint PThreadErr(char in_chr[], sint err)
	{
		...
		case EINVAL:
		{
			sint x = BStrP(in_chr, 256, "The mutex was created with the protocol attribute having the value PTHREAD_PRIO_PROTECT and the calling thread's priority is higher than the mutex's current priority ceiling(%i)", err);
			break;
		}
		case EAGAIN:
		{
			sint x = BStrP(in_chr, 256, "The mutex could not be acquired because the maximum number of recusive locks for mutex has been exceeded(%i)", err);
			break;
		}
		case ENOTRECOVERABLE:
		{
			sint x = BStrP(in_chr, 256, "The state protected by the mutex is not recoverable(%i)", err);
			break;
		}
		case EOWNERDEAD:
		{
			sint x = BStrP(in_chr, 256, "The mutex is a robust mutex and the process containing the previous owning thread terminated while holding the mutex lock. The mutex shall be acquired by the calling thread and it is up to the new owner to make the state consistent(%i)", err);
			break;
		}
		case EDEADLK:
		{
			sint x = BStrP(in_chr, 256, "A deadlock condition was or the mutex is type PTHREAD_MUTEX_ERRORCHECK and the current thread already owns the mutex(%i)", err);
			break;
		}
		case EBUSY:
		{
			sint x = BStrP(in_chr, 256, "The mutex could not be acquired because it was already locked(%i)", err);
			break;
		}
		case EPERM:
		{
			sint x = BStrP(in_chr, 256, "The mutex type is PTHREAD_MUTEX_ERRORCHECK or PTHREAD_MUTEX_RECURSIVE, or the mutex is a robust mutex, and the current thread does not own the mutex(%i)", err);
			break;
		}
		default:
		{
			sint x = BStrP(in_chr, 256, "The mutex err code is unknown(%i)", err);
			return -1;
			break;
		}
		}
	...
	sint InitThread(TINFO *tts_ = 0)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::InitThread(TINFO* tts_(%i))", (sint)tts_);
			op = Output(outp.carr, 2);
		}
		if (!tts_)
		{
	...
	sint InitThreads(std::vector<TINFO > *vec_, std::vector<sint>* rets_)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::InitThreads(std::vector<TINFO> *vec_(%i), std::vector<sint> *rets_(%i))", (sint)vec_, (sint)rets_);
			op = Output(outp.carr, 2);
		}
		sint x = -1;
		if (!vec_->empty())
	...
	sint CreateThread(TINFO* tts_ = 0, sint* ret_ = 0)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::CreateThread(TINFO* tts_(%i), sint* ret_(%i))", (sint)tts_, (sint)ret_);
			op = Output(outp.carr, 2);
		}
		...
		{
			tts_->fin = -2;
			sint xp = PushToVec(tts_, MTX_TTSV, true, true, false);
			...
			if (debug_lvl >= 10 && debug_m)
			{
				...
				if (xc)
				{
					na = BStrP(outp, 1200, "Error(%i) initializing thread:%s", xc, tts_->thread_name.c_str());
				}
				else
				{
					na = BStrP(outp, 1200, "thread created, tts_->lid=%i, initializing thread:%s", tts_->lid, tts_->thread_name.c_str());
				}
				na = Output(outp, 2);
			}
	...
	sint CreateThreads(std::vector<TINFO>* tts_, std::vector<sint>* rets_)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::CreateThreads(vector<TINFO>* tts_(%i), vector<sint>* rets_(%i))", (sint)tts_, (sint)rets_);
			op = Output(outp.carr, 2);
		}
		sint ret = -1;
		sint h = HasGMutexLocked(MTX_TTSV);
		...
		if (h > -1)
		{
			...
			{
				tts_->at(siz).fin = -2;
				sint xc = PushToVec(&tts_->at(siz), MTX_TTSV, true, true, false);
				...
				if (VecSize(MTX_TTSV, false) + 1 > BOTTHREAD_MAX || siz == tts_->size())
				{
					if (p > -1)
					{
						sint xp = UnlockGMutex(MTX_TTSV, h);
					}
					xc = FinishThreads(&nvec, rets_);
					nvec.clear();

					if (siz < tts_->size() && p > -1)
					{
						sint xp = LockGMutex(MTX_TTSV, true, h);
					}
				}
			}
			...
			while (siz < tts_->size())
			{
				if (create_rets[siz])
				{
					...
					{
						carr_128 outp;
						sint op = BStrP(outp.carr, outp.siz, "Error(%i) initializing thread [%i]", create_rets[siz], tts_->at(siz).lid);
						op = Output(outp.carr, 2);
					}
					ret++;
				}
				else
				{
					...
					{
						char outp[1200]{ 0 };
						sint oc = BStrP(outp, 1200, "thread created, ttsv_->at(%i) for thread: \"%s\"", tts_->at(siz).lid, tts_->at(siz).thread_name.c_str());
						oc = Output(outp, 2);
					}
				}
				siz++;
	...
	sint FinishThread(TINFO* tts_, sint* ret_)
	{
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FinishThread(TINFO* tts_(%i %i), sint* ret_(%i))", (sint)tts_, tts_->opt, (sint)ret_);
			op = Output(outp.carr, 2);
		}
		...
		case 1:
		{
			...
			{
				TINFO tts;
				rc = GetVecEle(&tts, MTX_TTSV, tts_->lid, false, false);
				...
				if ((tts.fin == 1 || tts.fin == -1))
				{
					...
					if (debug_lvl >= 1 && debug_m)
					{
						...
						if (xc)
						{
							rc = BStrP(outp.carr, outp.siz, "Error(%i) initializing thread:%i", xc, tts_->lid);
							xc = -1;
						}
						else
						{
							xc = 0;
							*ret_ = (sint)vp_;
							rc = BStrP(outp.carr, outp.siz, "thread joined, tts_->lid=%i xc==%i", tts_->lid, *ret_);
						}
						rc = Output(outp.carr, 2);
					}
					rc = ClearVecEle(MTX_TTSV, tts_->lid, false);
				}
				else
				{
					rc = nsRest(BOT_NANO_REST);
				}
			}
		...
		default:
		{
			...
			{
				carr_128 outp;
				sint op = BStrP(outp.carr, outp.siz, "Error Invalid join/detach opt(%i) for thread(%i)", xc, tts_->lid);
				op = Output(outp.carr, 2);
			}
			break;
		}
	...
	sint FinishThreads(std::vector<TINFO>* tts_, std::vector<sint>* rets_)
	{
		...
		{
			carr_128 outp;
			sint op = BStrP(outp.carr, outp.siz, "::FinishThreads(vector<TINFO>* tts_(%i), vector<sint>* rets_(%i))", (sint)tts_, (sint)rets_);
			op = Output(outp.carr, 2);
		}
		...
		while (!done)
		{
			...
			while (siz < tts_->size())
			{
				...
				TINFO tts;
				p = GetVecEle(&tts, MTX_TTSV, tts_->at(siz).lid, false, false);
				...
				if (tts.opt)
				{
					if ((tts.fin == 1 || tts.fin == -1))
					{
						...
						if (debug_lvl >= 1 && debug_m)
						{
							...
							{
								tts.opt = xc;
								xc = BStrP(outp.carr, outp.siz, "Error(%i) joining thread(%s)", xc, tts.thread_name.c_str());
							}
							else
							{
								sint uret = (sint)ret_;
								...
								tts.opt = 0;
								xc = BStrP(outp.carr, outp.siz, "thread(%i %s) joined", tts.lid, tts.thread_name.c_str());
							}
							sint oc = Output(outp.carr, 2);
						}
						xc = ClearVecEle(MTX_TTSV, tts_->at(siz).lid, false);
					}
					else
					{
						siz = tts_->size();
	...
	sint TM(sint opt)
	{
		...
		{
			carr_64 outp;
			sint op = BStrP(outp.carr, outp.siz, "::TM(sint opt(%i))", opt);
			op = Output(outp.carr, 2);
		}
		...
		for (sint p = 0; p < VecSize(MTX_TTSV); p++)
		{
			TINFO_M ntts;
			sint sp = GetVecEle(&ntts, MTX_TTSV, p);
			...
			if ((ntts.fin == 1 || ntts.fin == -1) && !ntts.opt)
			{
				...
				{
					carr_64 outp;
					sint op = BStrP(outp.carr, outp.siz, "clearing ele(%i)", p);
					op = Output(outp.carr, 2);
				}
				ClearVecEle(MTX_TTSV, ntts.lid, false);
			}
			else if (ntts.fin == -10 && strlen(ntts.thread_name.c_str()))
			{
				if (debug_lvl >= 1 && debug_m)
				{
					...
					{
						carr_64 outp;
						sint op = BStrP(outp.carr, outp.siz, "found unstarted thread (%i)", p);
						op = Output(outp.carr, 2);
					}
				}
		...
		if (!create_vec.empty())
		{
			...
			{
				carr_64 outp;
				sint op = BStrP(outp.carr, outp.siz, "found (%u) threads to create", create_vec.size());
				op = Output(outp.carr, 2);
			}
		}
		if (!has_self && !lid)
		...
		{
		char tn[32];
		sint ntn = BStrP(tn, 32, "TestThread(%i)", p);
	...
	sllint TestThreadIter(sllint to_ = BOT_ITER_TO, bool tomain = true, bool tomine = true)
	{
		...
		for (sllint xc = 0; xc < to_; xc++)
		{
			val /= 1.333f;
			carr_32 outp;
			sint op = BStrP(outp.carr, outp.siz, "val=%.3f", val);
		...
		{
			carr_512 outp;
			sint op = BStrP(outp.carr, outp.siz, "%s: iter_to:%lli span.count()=%llu (slint)span.count=%lli", ptn.c_str(), to_, span.count(), (sllint)span.count());
			op = Output(outp.carr, 2, tomain, tomine);
		}
		return lc;
	}
	sllint TestThreadTRat(sllint to_ = BOT_ITER_TO, bool tomain = true, bool tomine = true)
	{
		...
		sllint xc = rc;
		sint h = HasGMutexLocked(MTX_TRAT);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_TRAT, true);
			h = p;
		}
		...
		{
			carr_256 outp;
			sint op = BStrP(outp.carr, outp.siz, "this thread's time was:%lli compared to fastest time:%lli setting this ratio:%.9f", rc, xc, thr_exe_mod);
			op = Output(outp.carr, 2, tomain, tomine);
		}
		return rc;
	}
	...
	static void* WordMaintenance(void * vp);
	static void* AddThisCode(void * vp);
	static void* AddTheseWords(void * vp);
	static void *ReadFile(void * vp);
	...
	sint CheckPrintFs()
	{
		...
		sint of = -1;
		sint lx = OpenFile(&nfile, BOT_FILE_READ, &of, true);
		...
		if (lx > -1 && lx < (sint)sizeof(f_con))
		{
			...
			while (!done)
			{
				...
				if (f_con[lx]->fstrm.getline(ncar.carr, 2048))
				{
					...
					sint f = str.find("BStrP", 0);

					if (f > -1)
					{
						std::cout << "BStrP found at litebot.h line:" << lct << std::endl;
		...
		if (lx > -1)
		{
			...
			sint of = -1;
			lx = OpenFile(&nfile, BOT_FILE_READ, &of, true);
			...
			if (lx > -1 && lx < (sint)sizeof(f_con))
			{
				...
				while (!done)
				{
					...
					if (f_con[lx]->fstrm.getline(ncar.carr, 2048))
					{
						...
						if (f > -1)
						{
							std::cout << "BStrP found at litebot.cpp line:" << lct << std::endl;
							std::vector<uint> litv, ordv;
							sint xc = LitLocs(&litv, ncar.carr);
							uint fct = 0;
	...
	sint ProcVins()
	{
		...
		sint of = -1;
		sint lx = OpenFile(&nfile, BOT_FILE_READ, &of);
		...
		if (lx > -1 && lx < (sint)sizeof(f_con))
		{
			...
			lx = OpenFile(&nufile, BOT_FILE_APPEND, &of, true);
	...
	sint AddDlrCode(cchar* to_vin = "")
	{
		...
		sint of = -1;
		sint lx = OpenFile(&nfile, BOT_FILE_OVERW, &of);
