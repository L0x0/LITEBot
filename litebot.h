...
public:
	...
	void Clear() { t_I = std::chrono::steady_clock::now(); opt = lok = lid = c_by = -1; fin = -10; if (!thread_name.empty()) { thread_name.clear(); } func = NULL; attr = NULL; }
	...
	TINFO(sint nopt = -1, cchar* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1) { attr = NULL; t_I = std::chrono::steady_clock::now(); func = nfunc; c_by = nc_by; opt = nopt; lok = lid = -1; fin = -10; size_t x = strlen(ntn); if (x && x < 256) { thread_name.append(ntn); } };
...
public:
	...
	void Clear() { attr = thread = 0; t_I = std::chrono::steady_clock::now(); opt = lok = lid = c_by = -1; fin = -10; if (!thread_name.empty()) { thread_name.clear(); } func = NULL; }
	...
	TINFO_M(sint nopt = -1, cchar* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1) { t_I = std::chrono::steady_clock::now(); opt = lid = lok = -1; fin = -10; if (nopt > -1) { opt = nopt; } if (strlen(ntn) && strlen(ntn) < 256) { thread_name.append(ntn); } func = nfunc; c_by = nc_by; };
...
private:
	...
	sint InitThreads(std::vector<TINFO_M> *vec_ = 0, std::vector<sint>* rets_ = 0)
	{
		if (debug_lvl >= 10 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "::InitThreads(std::vector<TINFO_M> *vec_(%i), std::vector<sint> *rets_(%i))", (sint)vec_, (sint)rets_);
			op = Output(outp.carr, 2);
		}
		...
		if (!vec_->empty())
		{
			x = CreateThreads(vec_);

			if (x < (sint)vec_->size())
			{
				x = FinishThreads(vec_, rets_);
			}
		}
		return x;
	}
	sint CreateThread(TINFO_M* tts_ = 0)
	{
		...
		TINFO tts(tts_->opt, tts_->thread_name.c_str(), tts_->func, tts_->c_by);
		tts.fin = -2;
		sint xc = PushToVec(&tts, MTX_TTS, true, true, false);
	...
	sint CreateThreads(std::vector<TINFO_M>* tts_ = 0)
	{
		if (debug_lvl >= 10 && debug_m)
		{
			carr_128 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "::CreateThreads(vector<TINFO_M>* tts_(%i))", (sint)tts_);
			op = Output(outp.carr, 2);
		}
	...
	sint FinishThreads(std::vector<TINFO_M>* tts_, std::vector<sint>* rets_)
	{
		if (debug_lvl >= 10 && debug_m)
		{
			carr_128 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "::FinishThreads(vector<TINFO_M>* tts_(%i), vector<sint>* rets_(%i))", (sint)tts_, (sint)rets_);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = (sint)tts_->size();

		while (ret)
		{
			for (uint siz = 0; siz < tts_->size(); siz++)
			{
				if (!tts_->at(siz).thread_name.empty())
				{
					sint xret = -1;
					sint tret = FinishThread(&tts_->at(siz), &xret);
	...
	sint TM(sint opt)
	{
		...
		if (!has_log)
		{
			...
			TINFO_M tts(0, "LogMaintenance", LogMaintenance);
		...
		if (!has_dm)
		{
			...
			TINFO_M tts(0, "DBMaintenance", DBMaintenance);
			sint xc = InitThread(&tts);
		}
		if (!has_tm)
		{
			...
			TINFO_M tts(0, "ThreadMaintenance", ThreadMaintenance);
			sint xc = InitThread(&tts);
		}
