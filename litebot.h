...
public:
	...
	void Renew(cchar* nname = "", sint nmlid = -1, sint nomode = -1, sint nfid = -1)
	{
		Clear();
		mlid = nmlid;
		lid = nfid;
...
public:
	...
	void Clear() { min_ratv = 0.0f; t_I = std::chrono::steady_clock::now(); opt = lok = lid = c_by = -1; fin = -10; if (!thread_name.empty()) { thread_name.clear(); } func = NULL; }
	...
	TINFO(sint nopt = -1, cchar* ntn = "", void *(*nfunc)(void *) = NULL, sint nc_by = -1, float nmin_ratv = 0.0f) { min_ratv = nmin_ratv; t_I = std::chrono::steady_clock::now(); func = nfunc; c_by = nc_by; opt = nopt; lok = lid = -1; fin = -10; size_t x = strlen(ntn); if (x && x < 256) { thread_name.append(ntn); } };
...
private:
	...
	sint BOTFileOUT(sint fcon = -1, sint as_opt = 0, ...)
	{
		if (fcon < 0 || as_opt < 0)
		{
			return -1;
	...
	sint BOTFileIN(sint fcon = -1, sint as_opt = 0, ...)
	{
		if (as_opt < 0)
		{
			return -1;
		...
		sint was_open = -1;
		BOT_FILE_M nfile("BOTFile_", "", "", lid, BOT_FILE_APPEND);

		if (fcon < 0)
		{
			sint ret = BOTOpenFile(&nfile, &was_open, true);
		}
		else
		{
			nfile.Clear();
			...
			case 1:
			{
				if (nfile.cm == 2)
				{
					...
					while (op_ != NULL)
					{
						...
						op_ = NULL;
						op_ = va_arg(args, cchar*);
					}
					ret = 0;
				}
				else if (nfile.cm == 1)
				{
					std::vector<BOT_STMT>* op_ = va_arg(args, std::vector<BOT_STMT>*);

					while (op_ != NULL)
					{
						
						op_ = NULL;
						op_ = va_arg(args, std::vector<BOT_STMT>*);
					}
					ret = 0;
				}
				else {}
			...
			case 2:
			{
				if (nfile.cm == 2)
				{
					...
					while (str_ != NULL)
					{
						...
						str_ = NULL;
						str_ = va_arg(args, std::string*);
					}
					ret = 0;
				}
				else if (nfile.cm == 1)
				{
					std::vector<BOT_STMT>* vec_ = va_arg(args, std::vector<BOT_STMT>*);

					while (vec_ != NULL)
					{
						
						vec_ = NULL;
						vec_ = va_arg(args, std::vector<BOT_STMT>*);
					}
					ret = 0;
				}
				break;
			...
			case 3:
			{
				if (nfile.cm == 2)
				{
					...
					while (strvec_ != NULL)
					{
						...
						strvec_ = NULL;
						strvec_ = va_arg(args, std::vector<std::string>*);
					}
					ret = 0;
				}
				else if (nfile.cm == 1)
	...
	sint InterpretInput(std::string *input_ = 0)
	{
		...
		if (input_->length() > 2)
		{
			...
			if (!memcmp((void*)&input_->at(0), (void*)&com_c[0], 1) && !memcmp((void*)&input_->at(1), (void*)&com_c[1], 1))
			{
				...
				char c = ' ';
				ret = SeparateStrByChar(input_->substr(2, input_->length() - 2).c_str(), c, &vec);
				ret = Command(&vec);
			}
		}
