		...
		else if (S_ISREG(file_->fst.filestats.st_mode))
		{
			...
			if (ret > -1)
			{
				...
				if (f_con[file_->fcon]->fstrm.is_open())
				{
					...
					while (f_con[file_->fcon]->fstrm.get(*(char*)&nuchar) && loc < file_->fst.e_loc && loc < (ullint)BOT_FSCAN_LIM)
					{
		...
		else if (S_ISREG(file_->fst.filestats.st_mode))
		{
			...
			if (ret > -1)
			{
				...
				if (f_con[file_->fcon]->fstrm.is_open())
				{
					...
					while (f_con[file_->fcon]->fstrm.get(*(char*)&nuchar) && loc < siz)
					{
						uv.push_back(nuchar);
