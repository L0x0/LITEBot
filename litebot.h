...
public:
	...
	machine(sint i_debug_lvl = BOT_DEBUG_LVL, sint i_debug_m = BOT_DEBUG_M, sint nlid = -1, cchar* i_name = "")
	{
		...
		sint h = HasGMutexLocked(MTX_BTIM);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_BTIM, true);
			h = p;
		}
...
private:
						...
						if (yl > -1)
						{
							if (lok_d.held[xl][yl].locked)
							{
								if (lok_d.reqs[z].at_time < lok_d.held_t[xl])
								{
									...
									{
										lok_req oreq(-1, lok_d.reqs[z].gmtx, lok_d.reqs[z].fid, lok_d.reqs[z].is_meta);
	...
	sint ReqMTX(lok_req* nreq = 0, bool wait = false)
	{
		...
		if (h > -1)
		{
			...
			for (uint siz = 0; siz < lok_d.held_ids.size(); siz++)
			{
				while (siz >= lok_d.held.size())
				{
					std::vector<HELD_MTX> nmtx;
					lok_d.held.push_back(nmtx);
				}
				while (siz >= lok_d.held_t.size())
				{
					std::chrono::steady_clock::time_point nmtx;
					lok_d.held_t.push_back(nmtx);
				}

				if (lok_d.held_ids[siz] < 0)
				{
					if (f_ele == UINT_MAX)
					{
						f_ele = siz;
				...
				else if (lok_d.held_ids[siz] == lid)
				{
					i_ele = siz;

					if (wait)
					{
						if (nreq->at_time < lok_d.held_t[siz])
						{
							lok_d.held_t[siz] = nreq->at_time;
						}
						else if (nreq->at_time > lok_d.held_t[siz])
						{
							nreq->at_time = lok_d.held_t[siz];
						}
						else {}
					}
				}
			...
			else
			{
				if (f_ele < UINT_MAX)
				{
					if (f_ele < i_ele)
					{
						lok_d.held_ids[f_ele] = nreq->fid;
						lok_d.held_t[f_ele] = lok_d.held_t[i_ele];

						if (!lok_d.held[f_ele].empty())
						{
				...
				else
				{
					...
					else
					{
						...
						lok_d.held_ids.push_back(nreq->fid);
						std::chrono::steady_clock::time_point t = std::chrono::steady_clock::now();
						lok_d.held_t.push_back(t);
						std::vector<HELD_MTX> xvec;
						f_ele = lok_d.held.size();
						lok_d.held.push_back(xvec);
	...
	sint HasGMutexLocked(sint g_opt = -1)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_128 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "::HasGMutexLocked(sint g_opt(%i))", g_opt);
		...
		for (size_t siz = 0; siz < d_mtxs.size(); siz++)
		{
			if (d_mtxs[siz].w_mtx == g_opt)
			{
				if (d_mtxs[siz].locked)
				{
					...
					{
						carr_128 outp;
						sint op = bot_sprintf(outp.carr, outp.siz, "HasGMutexLocked_d(g_opt:%i) = %u locked.", g_opt, siz);
						op = Output(outp.carr, 2);
					}
					return (sint)siz;
				}
		...
		{
			carr_64 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "HasGMutexLocked(g_opt:%i) = -1", g_opt);
			op = Output(outp.carr, 2);
		}
		return -1;
	}
	...
	sint LockGMutex(sint g_opt = -1, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_128 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "::LockGMutex(sint g_opt(%i), bool sleep_wait(%u))", g_opt, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		else
		{
			...
			while (!done)
			{
				...
				{
					std::chrono::steady_clock::time_point t = std::chrono::steady_clock::now();
					ret = pthread_mutex_trylock(&req_mtx.mtx);

					if (!ret)
					{
						done = true;
						d_mtxs[ele].locked = true;
						sint x = vtool.ValInVec(&lok_d.held_ids, lid);

						if (x < 0)
						{
							x = vtool.AddValToVec(&lok_d.held_ids, lid, false, true);

							if (x > -1)
							{
								if (x >= (sint)lok_d.held_t.size())
								{
									while (x >= (sint)lok_d.held_t.size())
									{
										lok_d.held_t.push_back(t);
									}
								}
								...
								if (x >= (sint)lok_d.held.size())
								{
									while (x > (sint)lok_d.held.size())
									{
										std::vector<HELD_MTX> nmtx;
										lok_d.held.push_back(nmtx);
									}
									lok_d.held.push_back(d_mtxs);
								}
								else
								{
									lok_d.held[x].clear();

									for (uint y = 0; y < d_mtxs.size(); y++)
									{
										if (d_mtxs[y].locked || !d_mtxs[y].h_mems.empty() || !d_mtxs[y].m_mems.empty())
										{
											lok_d.held[x].push_back(d_mtxs[y]);
										}
									}
								}
							}
				...
				if (!done)
				{
					...
					else
					{
						...
						{
							carr_512 outp;
							sint op = bot_sprintf(outp.carr, outp.siz, "sleeping, Global Mutex(%i) is locked ret(%i)", g_opt, ret);
							op = Output(outp.carr, 2);
						}
						slint rst = nsRest(BOT_NANO_REST, (g_opt == MTX_REQ));
						ct++;
					}
				}
			}
			if (ret)
			{
				...
				{
					carr_64 outp;
					sint op = bot_sprintf(outp.carr, outp.siz, "unable to lock Global Mutex(opt:%i sleep count(%i).", g_opt, ct);
					op = Output(outp.carr, 2);
				}
				return -1;
			}
			else
			{
				...
				{
					carr_64 outp;
					sint op = bot_sprintf(outp.carr, outp.siz, "Locked Global Mutex(opt:%i).", g_opt);
					op = Output(outp.carr, 2);
				}
			}
		}
	...
	sint UnlockGMutex(sint g_opt = -1)
	{
		...
		sint ele = HasGMutexLocked(g_opt);
		sint lox = -1;
		...
		if (ele > -1)
		{
			if (g_opt == MTX_REQ)
			{
				lox = pthread_mutex_unlock(&req_mtx.mtx);
			}
			else
			{
				switch (g_opt)
	...
	sint LockElement(BOT_LOGDET* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_LOGDET* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_TTS, val->lid, from_rest, sleep_wait, false);
	...
	sint LockElement(BOT_LOGDET_M* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_LOGDET_M* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_TTS, val->lid, from_rest, sleep_wait, true);
	...
	sint LockElement(TINFO* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(TINFO* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
	...
	sint LockElement(TINFO_M* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(TINFO_M* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
	...
	sint LockElement(BOT_DB* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_DB* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBL, val->lid, from_rest, sleep_wait, false);
	...
	sint LockElement(BOT_DB_M* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_DB_M* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBL, val->lid, from_rest, sleep_wait, true);
	...
	sint LockElement(DB_CONN* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(DB_CONN* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBC, val->lid, from_rest, sleep_wait, false);
		return ret;
	}
	sint LockElement(BOT_FILE* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_FILE* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FO, val->lid, from_rest, sleep_wait, false);
	...
	sint LockElement(BOT_FILE_M* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_FILE_M* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FO, val->lid, from_rest, sleep_wait, true);
	...
	sint LockElement(BOTF_CONN* val = 0, bool from_rest = false, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOTF_CONN* val(%i), bool from_rest(%u), bool sleep_wait(%u))", (sint)val, (uint)from_rest, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FCON, val->lid, from_rest, sleep_wait, false);
		return ret;
	}
	sint LockElement(sint g_opt = -1, sint ele = -1, bool from_rest = false, bool sleep_wait = true, bool is_meta = true)
	{
		...
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(sint g_opt(%i), sint ele(%i), bool from_rest(%u), bool sleep_wait(%u), bool is_meta(%u))", g_opt, ele, (uint)from_rest, (uint)sleep_wait, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
			...
			while (!done)
			{
				...
				if (!ret)
				{
					...
					if (h > -1)
					{
...
#else
								ret = pthread_mutex_lock(&DCON[ele].mtx.mtx);
#endif
							}
							break;
						}
						case MTX_FO:
						{
							if ((!is_meta && ele < (sint)fileo_vec.d_vec.size()) || (is_meta && ele < (sint)fileo_vec.m_vec.size()))
							{
								if (!is_meta)
								{
...
#else
								ret = pthread_mutex_lock(&FCON[ele].mtx.mtx);
#endif
							}
							break;
						}
				...
				if (!done)
				{
					if (sleep_wait)
					{
						...
						else
						{
							...
							{
								carr_128 outp;
								sint op = bot_sprintf(outp.carr, outp.siz, "sleeping, element(g:%i ele:%i is_meta:%u) is locked.", g_opt, ele, (uint)is_meta);
								op = Output(outp.carr, 2);
							}
							ct++;
							slint rst = nsRest(BOT_NANO_REST, sleep_wait);
						}
					}
					else
					{
	...
	sint LockElementByName(sint g_opt = -1, cchar* name = "", bool from_rest = false, bool sleep_wait = true, bool is_meta = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElementByName(sint g_opt(%i), cchar* name(\"%s\"), bool from_rest(%u), bool sleep_wait(%u), bool is_meta(%u))", g_opt, name, (uint)from_rest, (uint)sleep_wait, (uint)is_meta);
			op = Output(outp.carr, 2);
		}
	...
	sint UnlockElement(sint g_opt = -1, sint ele = -1, bool is_meta = true)
	{
		...
		if (ret > -1)
		{
			...
			if (!is_meta)
			{
				...
				xc = vtool.CleanVec(&d_mtxs[h].h_mems);
			}
			else
			{
				sint xc = vtool.ClearValFromVec(&d_mtxs[h].m_mems, ele, false);
				xc = vtool.CleanVec(&d_mtxs[h].m_mems);
			}
			if (!d_mtxs[h].locked && d_mtxs[h].m_mems.empty() && d_mtxs[h].h_mems.empty())
			{
				d_mtxs[h].Clear();
			}
		...
		if (h > -1)
		{
			...
			case MTX_CV:
			{
				if (!is_meta)
				{
					...
					for (size_t siz = 0; siz < c_vec.d_vec.size(); siz++)
					{
						sint x = LockElement(g_opt, (sint)siz, false, true, false);

						if (!x)
						{
							if (!strcmp(c_vec.d_vec[siz].login_name.c_str(), vp->login_name.c_str()))
							{
								vp->Clear();
								vp->Renew(&c_vec.d_vec[siz]);

								if (!x)
								{
									x = UnlockElement(g_opt, (sint)siz, false);
								}
								if (p > -1)
								{
									p = UnlockGMutex(g_opt);
				...
				else
				{
					...
					for (size_t siz = 0; siz < c_vec.m_vec.size(); siz++)
					{
						sint x = LockElement(g_opt, (sint)siz, false, true, true);

						if (!x)
						{
							if (!strcmp(c_vec.m_vec[siz].login_name.c_str(), vp->login_name.c_str()))
							{
								vp->Clear();
								vp->Renew(&c_vec.m_vec[siz]);

								if (!x)
								{
									x = UnlockElement(g_opt, (sint)siz, true);
								}
								if (p > -1)
								{
									p = UnlockGMutex(g_opt);
			...
			case MTX_DBL:
			{
				if (!is_meta)
				{
					...
					for (size_t siz = 0; siz < db_vec.d_vec.size(); siz++)
					{
						sint x = LockElement(g_opt, (sint)siz, false, true, false);

						if (!x)
						{
							if (!strcmp(db_vec.d_vec[siz].name.c_str(), vp->name.c_str()))
							{
								vp->Clear();
								vp->Renew(&db_vec.d_vec[siz]);
				...
				else
				{
					...
					for (size_t siz = 0; siz < db_vec.m_vec.size(); siz++)
					{
						sint x = LockElement(g_opt, (sint)siz, false, true, true);

						if (!x)
						{
							if (!strcmp(db_vec.m_vec[siz].name.c_str(), vp->name.c_str()))
							{
								vp->Clear();
								vp->Renew(&db_vec.m_vec[siz]);
			...
			if (!x)
			{
				if (!is_meta)
				{
					...
					case MTX_FO:
					{
						...
						vp->c_by = lid;
						ret = vtool.SetValInVec(&fileo_vec.d_vec, ele, vp);
						if (ret > -1)
						{
							BOT_FILE_M ns;
							ns.Update(vp);
				...
				else
				{
					...
					case MTX_FO:
					{
						...
						vp->c_by = lid;
						ret = vtool.SetValInVec(&fileo_vec.m_vec, ele, vp);
						break;
					}
					default:
					{
			...
			if (!x)
			{
				if (!is_meta)
				{
					...
					case MTX_CV:
					{
						...
						sint cl = vtool.CleanVec(&c_vec.d_vec);
					...
					case MTX_DBL:
					{
						...
						sint cl = vtool.CleanVec(&db_vec.d_vec);
				...
				else
				{
					...
					case MTX_CV:
					{
						...
						sint cl = vtool.CleanVec(&c_vec.m_vec);
						break;
					}
					case MTX_LOG:
					{
					...
					case MTX_DBL:
					{
						...
						sint cl = vtool.CleanVec(&db_vec.m_vec);
						break;
					}
					case MTX_FO:
					{
		...
		if (h > -1)
		{
			if (!is_meta)
			{
				...
				case MTX_FO:
				{
					...
					while (ele < fileo_vec.d_vec.size())
					{
						sint x = LockElement(g_opt, ele, false, true, is_meta);
						...
						{
							std::string str;
							str.append(fileo_vec.d_vec[ele].name);
							nvec->push_back(str);
			...
			else
			{
				...
				case MTX_FO:
				{
					...
					while (ele < fileo_vec.m_vec.size())
					{
						sint x = LockElement(g_opt, ele, false, true, is_meta);
						...
						{
							std::string str;
							str.append(fileo_vec.m_vec[ele].name);
							nvec->push_back(str);
			...
			if (!is_meta)
			{
				...
				case MTX_FO:
				{
					...
					while (ele < fileo_vec.d_vec.size())
					{
						...
						if (!x)
						{
							nvec->push_back(fileo_vec.d_vec[ele].lid);
			...
			else
			{
				...
				case MTX_FO:
				{
					...
					while (ele < fileo_vec.m_vec.size())
					{
						...
						if (!x)
						{
							nvec->push_back(fileo_vec.m_vec[ele].lid);
	...
	sint BotTimeP(char str_[], bool form = true, std::chrono::system_clock::time_point* tim_ = 0)
	{
		std::time_t t = std::chrono::system_clock::to_time_t(*tim_);
	...
	sint OpenFile(BOT_FILE* file_ = 0, sint *was_open = 0, bool cine = false)
	{
		...
		sint lx = LockElement(file_, false, true);
	...
	sint Output(cchar* output_, sint option, bool logmain = true, bool logmine = true)
	{
		...
		carr_256 instr;
		xc = bot_sprintf(instr.carr, instr.siz, "%s [%i][%s] ", tdata.carr, lid, ptn.c_str());
		std::string str;
	...
	std::string LEncStrI(cchar* str_ = "", sint opt = -1)
	{
		...
		{
			char instr[1024]{ 0 };
			sint oc = bot_sprintf(instr, 1024, "LEncStrI(string *str_(%i), sint opt(%i))", (sint)str_, opt);
			oc = Output(instr, 2);
		}
		...
		{
			char instr[1024]{ 0 };
			sint oc = bot_sprintf(instr, 1024, "enclosed string: %s %i", s.c_str(), opt);
			oc = Output(instr, 2);
		}
		return s;
	}
	...
	sint OpenDB(cchar *db_name_ = "", sint x = -1, sint* was_open = 0)
	{
		...
		dbg.omode = x;
		sint lx = LockElement(&dbg, false, true);
		...
		{
			carr_256 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "Opening DB \"%s\"(%i) olim(%i) exists(%i)", db_name_, dbg.lid, olim, dbg.exists);
			oc = Output(outp.carr, 2);
		}
	...
	sint SetTTSVEleM(sint e, sint m, void* vp_ = 0, bool is_meta = true)
	{
		...
		sint ret = -1;
		sint h = HasGMutexLocked(MTX_TTS);
		sint p = -1;

		if (h < 0)
		{
			p = LockGMutex(MTX_TTS, true);
			h = p;
		}

		if (h > -1)
		{
			sint x = LockElement(MTX_TTS, e, false, true, is_meta);
			...
			if (!x)
			{
				...
				if (!x)
				{
					x = UnlockElement(MTX_TTS, e, is_meta);
				}
			}
			if (p > -1)
			{
				p = UnlockGMutex(MTX_TTS);
			}
		}
		return ret;
	}
	sint GetTTSVEleM(sint e, sint m, void* vp_, bool is_meta = true)
	{
		...
		if (h > -1)
		{
			...
			if (!x)
			{
				...
				if (!x)
				{
					p = UnlockElement(MTX_TTS, e, is_meta);
				}
			}
			if (p > -1)
			{
				x = UnlockGMutex(MTX_TTS);
			}
		}
		return ret;
	}
		...
		if (h > -1)
		{
			...
			while (siz < tts_->size())
			{
				...
				if (VecSize(MTX_TTS, false) + 1 > BOTTHREAD_MAX || siz == tts_->size())
				{
					...
					{
						sint xp = LockGMutex(MTX_TTS, true);
					}
				}
			}
