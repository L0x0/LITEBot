...
#endif
#ifndef BOT_DBREST
#define BOT_DBREST BOT_MILLI_REST*250
#endif
#ifndef BOT_THRREST
...
#ifndef BOT_REQREST
#define BOT_REQREST BOT_NANO_REST*3
#endif
...
public:
	...
	void Clear() { connected = false; file_ele = omode = lok = c_by = lid = exists = -1; if (!name.empty()) { name.clear(); } if (!tables.empty()) { tables.clear(); } if (!columns.empty()) { columns.clear(); } }
	void Renew(BOT_DB* val_ = 0) { Clear(); file_ele = val_->file_ele; lid = val_->lid; lok = val_->lok; type = val_->type; exists = val_->exists; c_by = val_->c_by; omode = val_->omode; uint len = (uint)val_->name.length(); if (len && len < 128) { name.append(val_->name.c_str()); } for (uint x = 0; x < val_->tables.size(); x++) { tables.push_back(val_->tables[x]); } for (uint x = 0; x < val_->columns.size(); x++) { columns.push_back(val_->columns[x]); } }

	BOT_DB(cchar* nstr = "", sint ntype = -1) { connected = false; file_ele = omode = lok = c_by = lid = exists = -1; type = ntype; uint len = strlen(nstr); if (len && len < 128) { name.append(nstr); } };
...
public:
	...
	void Clear() { connected = false; file_ele = omode = lok = c_by = lid = exists = -1; if (!name.empty()) { name.clear(); } tsiz = 0; csiz = 0; }
	void Renew(BOT_DB_M* val_ = 0) { Clear(); lid = val_->lid; connected = val_->connected; file_ele = val_->file_ele; lok = val_->lok; type = val_->type; exists = val_->exists; c_by = val_->c_by; omode = val_->omode; name.append(val_->name.c_str()); tsiz = val_->tsiz; csiz = val_->csiz; }
	void Renew(BOT_DB* val_ = 0) { Clear(); lid = val_->lid; connected = val_->connected; file_ele = val_->file_ele; lok = val_->lok; type = val_->type; exists = val_->exists; c_by = val_->c_by; omode = val_->omode; name.append(val_->name.c_str()); tsiz = val_->tables.size(); csiz = val_->columns.size(); }
	...
	BOT_DB_M(cchar* nstr = "", sint ntype = -1, sint nlid = -1, size_t ntsiz = 0, size_t ncsiz = 0, sint nexist = -1, sint nc_by = -1) { connected = false; if (strlen(nstr) < 128) { file_ele = lok = -1; name.append(nstr); } type = ntype; lid = nlid; tsiz = ntsiz; csiz = ncsiz; exists = nexist; c_by = nc_by; omode = -1; };
...
public:
	...
	void Clear() { cm = lid = exists = c_by = omode = lok = -1; if (!path.empty()) { path.clear(); } if (!type.empty()) { type.clear(); } if (!name.empty()) { name.clear(); } if (!strvec.empty()) { strvec.clear(); } }
...
public:
	...
	void Clear() { cm = lid = exists = c_by = omode = lok = -1; if (!path.empty()) { path.clear(); } if (!type.empty()) { type.clear(); } if (!name.empty()) { name.clear(); } strsiz = 0; }
	...
	BOT_FILE_M(cchar* nfn = "", cchar* nft = "", cchar* nfp = "", sint nc_by = -1, sint nomode = -1) { cm = omode = nomode; lok = -1; size_t siz = strlen(nfn); if (siz && siz < BOT_FILENAME_MAX) { name.append(nfn); } siz = strlen(nft); if (siz && siz < 128) { type.append(nft); } siz = strlen(nfp); if (siz && siz < UINT64_MAX) { path.append(nfp); } strsiz = 0; exists = -1; c_by = nc_by; };
...
public:
	...
	void Clear() { in_use = false; flid = omode = lid = -1; /*if (!name.empty()) { name.clear(); } if (!type.empty()) { type.clear(); } if (!path.empty()) { path.clear(); }*/ if (fstrm.is_open()) { fstrm.close(); } }
...
public:
	...
	sint ValInArr(BOTF_CONN* arr[BOT_FO_MAX], sint val = -1, sint e_from = 0, bool forw = true) { if (!arr || val < 0) { return -1; } sint msiz = (sint)sizeof(arr); sint vn = -1; sint siz = e_from; if (e_from < 0) { siz = (msiz + e_from); } if (forw) { while (siz < msiz) { if (arr[siz]->flid == val && arr[siz]->lid > -1) { vn = siz; siz = msiz; } siz++; } } else { while (siz > -1) { if (arr[siz]->flid == val && arr[siz]->lid > -1) { vn = siz; siz = -1; } siz--; } } return vn; }
...
private:
								...
								if (lok_d.reqs[z].ele < 0)
								{
									if (lok_d.held[xl][yl].locked)
									{
										...
										//{
											...
											//sint ox = vtool.AddValToVec(nreqs_, &oreq, true, true);
										//}
										//else
										//{
										...
										{
											lok_req oreq(-1, lok_d.reqs[z].gmtx, lok_d.reqs[z].fid, -1);
											yl = vtool.AddValToVec(nreqs_, &oreq, true, true);
										}
										//}
									}
								}
								else
								{
									if (!lok_d.reqs[z].is_meta)
									{
										...
										if (nl > -1)
										{
											if (lok_d.reqs[z].at_time < lok_d.held_t[xl])
											{
												yl = vtool.AddValToVec(nreqs_, &lok_d.reqs[z], true, true);
											}
										}
									}
									else
	...
	sint ReqMTX(lok_req* nreq = 0, bool wait = false)
	{
		...
		if (!p)
		{
			...
			{
				sint x = vtool.ValInVec(&d_mtxs, nreq->gmtx);

				if (x < 0)
				{
					HELD_MTX nmtx(nreq->gmtx, -1, true);
					x = vtool.AddValToVec(&d_mtxs, &nmtx, false, true);
				}
				else
				{
					if (nreq->ele < 0)
	...
	sint LockGMutex(sint g_opt = -1, sint* had_lock = 0, bool sleep_wait = true)
	{
		...
		sint ele = vtool.ValInVec(&d_mtxs, g_opt);
	...
	/*sint LockElement(BOT_LOGDET* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_LOGDET* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_TTS, val->lid, had_lock, sleep_wait, false);
	...
	sint LockElement(BOT_LOGDET_M* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_LOGDET_M* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_TTS, val->lid, had_lock, sleep_wait, true);
	...
	sint LockElement(TINFO* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(TINFO* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
	...
	sint LockElement(TINFO_M* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(TINFO_M* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
	...
	sint LockElement(BOT_DB* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_DB* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBL, val->lid, had_lock, sleep_wait, false);
	...
	sint LockElement(BOT_DB_M* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_DB_M* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBL, val->lid, had_lock, sleep_wait, true);
	...
	sint LockElement(DB_CONN* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(DB_CONN* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_DBC, val->lid, had_lock, sleep_wait, false);
		return ret;
	}
	sint LockElement(BOT_FILE* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_FILE* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FO, val->lid, had_lock, sleep_wait, false);
	...
	sint LockElement(BOT_FILE_M* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOT_FILE_M* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FO, val->lid, had_lock, sleep_wait, true);
	...
	sint LockElement(BOTF_CONN* val = 0, sint* had_lock = 0, bool sleep_wait = true)
	{
		if (debug_lvl >= 900 && debug_m)
		{
			carr_256 outp;
			sint op = bot_sprintf(outp.carr, outp.siz, "LockElement(BOTF_CONN* val(%i), sint* had_lock(%i), bool sleep_wait(%u))", (sint)val, (sint)had_lock, (uint)sleep_wait);
			op = Output(outp.carr, 2);
		}
		...
		sint ret = LockElement(MTX_FCON, val->lid, had_lock, sleep_wait, false);
		return ret;
	}*/
	sint LockElement(sint g_opt = -1, sint ele = -1, sint* had_lock = 0, bool sleep_wait = true, bool is_meta = true)
	{
		...
		sint p = vtool.ValInVec(&d_mtxs, g_opt);

		if (p < 0)
		{
			HELD_MTX nmtx(g_opt, -1, false);
			p = vtool.AddValToVec(&d_mtxs, &nmtx, true, true);
		...
		if (!is_meta)
		{
			ret = vtool.ValInVec(&d_mtxs[p].h_mems, ele);
		}
		else
		{
			ret = vtool.ValInVec(&d_mtxs[p].m_mems, ele);
		}
	...
	sint UnlockElement(sint g_opt = -1, sint ele = -1, bool is_meta = true)
	{
		...
		if (ret > -1)
		{
			...
			{
				sint xc = vtool.ClearValFromVec(&d_mtxs[p].m_mems, ele, false);
	...
	sint CloseObject(sint nobj = -1, sint nele = -1, sint is_meta = -1, sint* r_opt = 0, sint* n_opt = 0)
	{
		...
		else
		{
			...
			case MTX_FO:
			{
				sint ret = vtool.ValInVec(&d_mtxs, nobj);
				...
				if (ret > -1)
				{
					...
					{
						sint xret = ret;
						ret = vtool.ValInVec(&d_mtxs[xret].h_mems, nele);
					...
					{
						sint xret = ret;
						ret = vtool.ValInVec(&d_mtxs[xret].m_mems, nele);
					...
					if (!is_meta)
					{
						...
						if (ret > -1)
						{
							if (d_con_)
							{
								...
								return CloseDB(nele, n_opt, true);
							}
							else
							{
	...
	sint GetOFConn(BOT_FILE_M* val_ = 0)
	{
		...
		case 1:
		{
			return HasDBOpen(val_->name.c_str(), val_->omode);
			break;
		}
		case 2:
		{
	...
	sint CloseFConn(sint ele = -1, bool clear_conn = true)
	{
		if (ele < 0)
		{
			return -1;
		...
		if (ele < (sint)f_con.size())
		{
			...
			{
				f_con[ele]->Clear();
				ret = UnlockElement(MTX_FCON, ele, false);
				f_con[ele] = 0;
				ret = vtool.CleanVec(&f_con);
			}
			else
			{
				if (f_con[ele]->fstrm.is_open())
				{
					f_con[ele]->fstrm.close();
				}
			}
		}
		return ret;
	...
	sint BOTCreateFile(BOT_FILE_M* file_ = 0, sint* was_open = 0, sint ele = -1)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_128 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::BOTCreateFile(BOT_FILE_M *file_(%i), sint* was_open(%i), sint ele(%i))", (sint)file_, (sint)was_open, ele);
			oc = Output(ncar.carr, 2);
		}
		if (!file_ || !was_open)
		{
		...
		{
			sint xh = 1;
			ret = OpenDB(file_->name.c_str(), 3, was_open, &xh);
		...
		case 2:
		{
			if (ele > -1)
			{
				...
				f_con[ele]->fstrm.open(instr.c_str(), file_->omode);
	...
	sint OpenFile(sint flid = -1, sint omode = -1, sint *was_open = 0, bool cine = false, bool r_to_st = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_512 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::OpenFile(sint flid(%i), sint omode(%i), sint *was_open(%i), bool cine(%u), bool r_to_st(%u))", flid, omode, (sint)was_open, (uint)cine, (uint)r_to_st);
			oc = Output(ncar.carr, 2);
		}
		...
		return OpenFile(nf.path.c_str(), nf.name.c_str(), nf.type.c_str(), omode, was_open, cine, r_to_st);
	}
	sint OpenFile(cchar* fp_ = "", cchar* fn_ = "", cchar* ft_ = "", sint omode = -1, sint *was_open = 0, bool cine = false, bool r_to_st = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_512 ncar;
			sint oc = bot_sprintf(ncar.carr, ncar.siz, "::OpenFile(cchar* fp_(%s), cchar* fn_(%s), cchar* ft_(%s), sint omode(%i), sint *was_open(%i), bool cine(%u), bool r_to_st(%u))", fp_, fn_, ft_, omode, (sint)was_open, (uint)cine, (uint)r_to_st);
			oc = Output(ncar.carr, 2);
		}

		if (!fp_ || !fn_ || !ft_ || !was_open || omode < 0)
		{
			return -10;
		}
		
		BOT_FILE_M mfile(fn_, ft_, fp_, lid, omode);
		sint gc = GetInVec((void*)&mfile, MTX_FO);

		if (gc < 0)
		{
			BOT_FILE nfile(fn_, ft_, fp_, lid, omode);
			...
			gc = GetVecEle((void*)&mfile, MTX_FO, nfile.lid);
		...
		if (!mfile.path.empty())
		{
			if (mfile.path[mfile.path.length() - 1] != '\\')
			{
				mfile.path.push_back('\\');
			}
		}

		gc = mfile.name.find('.', 0);

		if (gc > 0)
		{
			if (mfile.type.empty())
			{
				mfile.type.append(mfile.name.substr(gc, mfile.name.length() - gc));
			}
			mfile.name.erase(gc, mfile.name.length() - gc);
		}
		...
		{
			MACSYM fc(5, (uchar*)ft_, 0);
			gc = AddCode(&fc);
		}

		gc = ConMethod(&mfile);
		gc = SetVecEleMem((void*)&mfile.cm, MTX_FO, mfile.lid, BOT_FS_CM, false);
		sint fc = GetOFConn(&mfile);
		...
		if (fc < 0)
		{
			...
			else
			{
				...
				for (fc = GetFConn(&mfile); fc < 0 && gc < INT32_MAX; fc = GetFConn(&mfile))
				{
					slint rst = nsRest((slint)BOT_NANO_REST);
					gc++;
				}
		...
		sint xh = -1;
		sint lx = LockElement(MTX_FO, mfile.lid, &xh, true, false);
		...
		if (gc)
		{
			if (cine)
			{
				gc = BOTCreateFile(&mfile, was_open, fc);
		...
		if (S_ISREG(mfile.filestats.st_mode))
		{
			if (strcmp(cur_path.c_str(), mfile.path.c_str()))
			{
				cur_path.clear();
				cur_path.append(mfile.path);
			}
			...
			{
				sint xh = 1;
				fc = OpenDB(mfile.name.c_str(), mfile.omode, was_open, &xh);
				break;
			}
			case 2:
			{
				std::string instr;
				instr.append(mfile.path.c_str());
				instr.append(mfile.name.c_str());
				instr.append(mfile.type.c_str());
				f_con[fc]->fstrm.open(instr.c_str(), mfile.omode);
		...
		else if (S_ISDIR(mfile.filestats.st_mode))
		{
			gc = -1;
			gc = mfile.name.find(cur_path, 0);

			if (!gc)
			{
				cur_path.append("\\");
				cur_path.append(mfile.name);
			}
			else
			{
				cur_path.clear();
		...
		else if (S_ISCHR(mfile.filestats.st_mode)) {}
		else if (S_ISBLK(mfile.filestats.st_mode)) {}
		else if (S_ISFIFO(mfile.filestats.st_mode)) {}
		else if (S_ISLNK(mfile.filestats.st_mode)) {}
		else if (S_ISSOCK(mfile.filestats.st_mode)) {}
		else {}
		...
		if (fc < 0)
		{
			if (!xh)
			{
				xh = UnlockElement(MTX_FO, mfile.lid, false);
			}
			gc = ClearVecEle(MTX_FO, mfile.lid, false);
	...
	sint CloseFile(sint flid = -1, sint cm = -1, bool clear_conn = true, bool del = false, bool encr = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_2048 instr;
			sint oc = bot_sprintf(instr.carr, instr.siz, "::CloseFile(sint flid(%i), sint cm(%i), bool clear_conn(%u), bool del(%u), bool encr(%u))", flid, cm, (uint)clear_conn, (uint)del, (uint)encr);
		...
		{
			BOT_DB_M xdb;
			sint fc = GetVecEle((void*)&xdb, MTX_DBL, flid);

			if (fc > -1)
			{
				fc = GetVecEle((void*)&xfile, MTX_FO, xdb.file_ele);

				if (fc > -1)
				{
					sint xh = 1;
					fc = CloseDB(flid, &xh, clear_conn);
		...
		case 2:
		{
			if (flid < (sint)f_con.size())
			{
				sint fc = GetVecEle((void*)&xfile, MTX_FO, f_con[flid]->flid);

				if (fc > -1)
				{
					fc = CloseFConn(flid, clear_conn);
		...
		if (xfile.lid > -1)
		{
			sint p = FileStats(&xfile);
			return UnlockElement(MTX_FO, xfile.lid, false);
		}
		return -1;
	}
	...
	slint nsRest(slint i = BOT_NANO_REST, bool keep_mtx = false)
	{
		...
		if (!keep_mtx)
		{
			while (!relocks.empty())
			{
				for (uint x = 0; x < relocks.size(); x++)
				{
					sint cheq = OpenObject(relocks[x].gmtx, relocks[x].ele, relocks[x].is_meta, &relocks[x].fid, &relocks[x].lid); // returns is_meta as wasopen, uses value set in fid by CloseObject() as var arg option for openmode
	...
	sint Rest(sint i = BOT_MILLI_REST, bool keep_mtx = false)
	{
		if (debug_lvl >= 1000 && debug_m)
		{
			carr_128 instr;
		...
		if (!keep_mtx)
		{
			while (!relocks.empty())
			{
				for (uint x = 0; x < relocks.size(); x++)
				{
					sint cheq = OpenObject(relocks[x].gmtx, relocks[x].ele, relocks[x].is_meta, &relocks[x].fid, &relocks[x].lid); // returns is_meta as wasopen, uses value set in fid by CloseObject() as var arg option for openmode or is_meta
	...
	sint BQS(BOT_STMT *t_ = 0)
	{
		...
		{
			BOT_DB dbs(t_->dbname.c_str());
			sint i = -1;

			if (!t_->dbname.empty())
			{
				i = GetInVec((void*)&dbs, MTX_DBL, false, false);
			}

			if (i > -1)
			{
				for (uint xc = 0; xc < t_->ic_types.size() && xc < t_->tables.size(); xc++)
				{
	...
	sint OpenDB(sint db_lid = -1, sint x = -1, sint* was_open = 0, sint* fwas_locked = 0, bool r_to_st = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_512 outp;
		...
		BOT_DB_M ndb;
		sint gc = GetVecEle((void*)&ndb, MTX_DBL, db_lid);

		if (gc < 0)
		{
			return -1;
		}
		return OpenDB(ndb.name.c_str(), x, was_open, fwas_locked, r_to_st);
	}
	sint OpenDB(cchar *db_name_ = "", sint x = -1, sint* was_open = 0, sint* fwas_locked = 0, bool r_to_st = false)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_512 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::OpenDB(cchar* db_name_ \"%s\", sint x(%i), sint* was_open(%i), sint* fwas_locked(%i), bool r_to_st(%u))", db_name_, x, (sint)was_open, (sint)fwas_locked, (uint)r_to_st);
			oc = Output(outp.carr, 2);
		}
		...
		BOT_DB_M dbg(db_name_);
		sint gc = GetInVec((void*)&dbg, MTX_DBL, false);
		...
		if (gc < 0 && x == 3)
		{
			...
			gc = GetVecEle((void*)&dbg, MTX_DBL, ndb.lid);
		...
		/*if (dbg.omode > -1)
		{
			...
			while (dbg.omode > -1 && ct < UINT_MAX)
			{
				slint rst = nsRest((slint)BOT_NANO_REST);
				gc = GetVecEleMem((void*)&dbg.omode, MTX_DBL, dbg.lid, BOT_DB_OMODE);
				ct++;
			}
			if (ct == UINT_MAX && dbg.omode > -1)
			{
				if (debug_lvl >= 4 && debug_m)
				{
					carr_256 outp;
		...
		BOT_FILE_M nfilem(db_name_, ".db", "", lid, x);
		sint fc = GetInVec(&nfilem, MTX_FO, false, true);

		if (fc < 0)
		{
			BOT_FILE nfile(db_name_, ".db", "", lid, x);
			fc = GetInVec(&nfile, MTX_FO, true, false);

			if (fc < 0)
			{
				return -1;
			}
			fc = GetVecEle(&nfilem, MTX_FO, nfile.lid);
		...
		dbg.file_ele = nfilem.lid;
		sint xc = SetVecEleMem(&nfilem.lid, MTX_DBL, dbg.lid, BOT_DB_FELE, false);

		if (fwas_locked)
		{
			if (*fwas_locked < 0)
			{
				*fwas_locked = vtool.ValInVec(&d_mtxs, MTX_FO);

				if (*fwas_locked > -1)
				{
					sint xfdb = *fwas_locked;
					*fwas_locked = vtool.ValInVec(&d_mtxs[xfdb].h_mems, nfilem.lid);
			...
			{
				sint hc = -1;
				sint lx = LockElement(MTX_FO, nfilem.lid, &hc, true, false);
		...
		sint xh = -1;
		sint lx = LockElement(MTX_DBL, dbg.lid, &xh, true, false);
		...
		if (lx)
		{
			if (fwas_locked)
			{
				if (!*fwas_locked)
				{
					sint xc = UnlockElement(MTX_FO, nfilem.lid, false);
				}
			}
			if (debug_lvl >= 4 && debug_m)
			{
		...
		if (!d_con_)
		{
			...
			for (dbc = GetDBConn(&dbg); dbc < 0 && rc < INT32_MAX; dbc = GetDBConn(&dbg))
			{
				slint rst = nsRest((slint)BOT_NANO_REST);
				rc++;
			}
		...
		if (d_con_->name.empty())
		{
			...
			if (plim > -1)
			{
				if (dbc > -1)
				{
					...
					if (x == 1)
					{
						rc = sqlite3_open_v2(dbt.c_str(), &d_con_->db_, SQLITE_OPEN_READONLY, 0);
					}
					else if (x == 2)
					{
						rc = sqlite3_open_v2(dbt.c_str(), &d_con_->db_, SQLITE_OPEN_READWRITE, 0);
					}
					else if (x == 3)
					{
						rc = sqlite3_open_v2(dbt.c_str(), &d_con_->db_, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, 0);
					}
					else
					{
						if (debug_lvl >= 1 && debug_m)
						{
							carr_256 outp;
							rc = bot_sprintf(outp.carr, outp.siz, "invalid open opt:%i for %s", x, dbt.c_str());
							rc = Output(outp.carr, 2);
						}
						if (dbc > -1)
						{
							rc = UnlockElement(MTX_DBC, dbc, false);
						}
						if (!hlim)
						{
						...
						if (fwas_locked)
						{
							if (!*fwas_locked)
							{
								rc = UnlockElement(MTX_FO, nfilem.lid, false);
							}
						}
						return -1;
					}
					...
					d_con_->Renew(db_name_, dbg.lid, x, dbc);
					odb = INT32_MAX;
					olim++;
					nulim = olim;
					...
					if (!rc)
					{
						...
						while (!done)
						{
							...
							case SQLITE_OK:
							{
								...
								odb = vtool.AddValToVec(&d_con_->att, db_name_, true, true);
								olim++;
								nulim = olim;
								done = true;
								break;
							...
							default:
							{
								if (debug_lvl >= 1 && debug_m)
								{
									op = Output(GetError().c_str(), 1, 2, -1);
								}
								done = true;
								break;
							}
		...
		{
			dbg.omode = x;
			gc = SetVecEleMem((void*)&dbg.omode, MTX_DBL, dbg.lid, BOT_DB_OMODE, false);
			nfilem.omode = x;
			gc = SetVecEleMem((void*)&x, MTX_FO, nfilem.lid, BOT_FS_OMODE, false);
			dbg.connected = true;
			gc = SetVecEleMem((void*)&dbg.connected, MTX_DBL, dbg.lid, BOT_DB_CONN, false);
		}

		if (debug_lvl >= 4 && debug_m)
		{
			carr_256 outp;
			gc = bot_sprintf(outp.carr, outp.siz, "DB \"%s\"(%i) opened, olim is:%u", dbg.name.c_str(), dbg.lid, nulim);
			gc = Output(outp.carr, 2);
		}
		return dbg.lid;
	}
	/*sint CloseDB(cchar* db_name_ = "", sint* fwas_locked = 0, bool clear_conn = true)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_256 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::CloseDB(cchar* db_name_(%s), sint* fwas_locked(%i), bool clear_conn(%u))", db_name_, (sint)fwas_locked, (uint)clear_conn);
			oc = Output(outp.carr, 2);
		}
		...
		BOT_DB_M dbg(db_name_);
		sint gc = GetInVec((void*)&dbg, MTX_DBL);
		...
		if (gc < 0)
		{
			...
			{
				carr_256 outp;
				sint oc = bot_sprintf(outp.carr, outp.siz, "DB %i not found in list", db_name_);
				oc = Output(outp.carr, 2);
			}
			return -1;
		}
		return CloseDB(dbg.lid, fwas_locked, clear_conn);
	}*/
	sint CloseDB(sint db_lid = -1, sint* fwas_locked = 0, bool clear_conn = true)
	{
		...
		BOT_DB_M dbg;
		sint rc = GetVecEle((void*)&dbg, MTX_DBL, db_lid);
		...
		uint nulim = UINT_MAX;
		sint cdb = HasDBOpen(dbg.name.c_str());
		...
		{
			carr_256 bstmt;
			sint xc = bot_sprintf(bstmt.carr, bstmt.siz, "DETACH %s;", d_con_->att[cdb].c_str());
			cchar* tail;
			sqlite3_stmt* ppstmt;
			...
			else
			{
				...
				while (!done)
				{
					...
					case SQLITE_OK:
					{
						...
						if (clear_conn)
						{
							sint plim = vtool.ClearEleFromVec(&d_con_->att, cdb);
						...
						dbg.connected = false;
						dbg.omode = -1;
						sint xc = SetVecEleMem((void*)&dbg.connected, MTX_DBL, dbg.lid, BOT_DB_CONN, false);
						xc = SetVecEleMem((void*)&dbg.omode, MTX_DBL, dbg.lid, BOT_DB_OMODE, false);
						done = true;
						break;
					}
					case SQLITE_DONE:
					...
					case SQLITE_BUSY:
					{
						...
						slint rst = nsRest((slint)BOT_NANO_REST, true);
						break;
					}
					default:
					{
		...
		else
		{
			...
			if (!strcmp(d_con_->name.c_str(), dbg.name.c_str()))
			{
				...
				if (d_con_->att.empty())
				{
					...
					if (rc)
					{
						sint oc = Output(GetError().c_str(), 1, 2, -1);
						return rc;
					}
					else
					{
		...
		sint gc = UnlockElement(MTX_DBL, dbg.lid, false);
		...
		if (!gc)
		{
			...
			{
				carr_256 outp;
				sint oc = bot_sprintf(outp.carr, outp.siz, "unlocked DB \"%s\" olim:%i.", dbg.name.c_str(), nulim);
				oc = Output(outp.carr, 2);
			}
		}

		BOT_FILE_M nfile;
		gc = GetVecEle(&nfile, MTX_FO, dbg.file_ele);
		...
		if (fwas_locked)
		{
			if (!*fwas_locked)
			{
				...
				gc = UnlockElement(MTX_FO, nfile.lid, false);
			}
		}
		...
		/*if (d_con_)
		{
			...
			{
				rc = CloseDB(db_lid, fwas_locked, clear_conn);
			}
		}*/
		return rc;
	}

	sint OpenDBs(std::vector <std::string> *db_names_, sint opt = -1, std::vector <sint> *wos_ = 0, std::vector<sint> *fwas_ = 0, std::vector<sint> *lids_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			if (debug_lvl >= 1 && debug_m)
			{
				carr_256 outp;
				sint oc = bot_sprintf(outp.carr, outp.siz, "::OpenDBs(std::vector <std::string> *db_names_(%i), sint opt(%i), std::vector <sint> *wos_(%i), std::vector<sint>* fwas_(%i), std::vector<sint> lids_(%i))", (sint)db_names_, opt, (sint)wos_, (sint)fwas_, (sint)lids_);
				oc = Output(outp.carr, 2);
			}
		}
		if (!db_names_ || opt < 0 || !wos_ || !fwas_ || !lids_)
		{
			return -1;
		}
		if (db_names_->empty())
		...
		for (uint opens = 0; opens < db_names_->size(); opens++)
		{
			...
			else
			{
				fwas_->push_back(-1);
				wos_->push_back(-1);
				ret = OpenDB(db_names_->at(opens).c_str(), opt, &wos_->at(opens), &fwas_->at(opens));
				lids_->push_back(ret);
			}
		}
	...
	sint CloseDBs(std::vector<sint> *lids_ = 0, std::vector<sint>* fwas_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_256 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::CloseDBs(std::vector<sint>* lids_(%i), std::vector<sint>* fwas_(%i))", (sint)lids_, (sint)fwas_);
			oc = Output(outp.carr, 2);
		}
		...
		if (lids_ && fwas_)
		{
			for (sint opens = (sint)lids_->size() - 1; opens > -1; opens--)
			{
				xc = CloseDB(lids_->at(opens), &fwas_->at(opens));

				if (!xc)
				{
					lids_->at(opens) = INT32_MIN;
				}
			}
		}
		return xc;
	...
	sint Commit(std::vector<BOT_STMT>* stmts_ = 0)
	{
		...
		while (!done)
		{
			...
			if (ox > -1)
			{
				...
				std::vector<sint>  wos;
				sint odb = OpenDBs(&Comm.db_names, Comm.opt, &wos, &fwas, &lids);
				...
				if (odb > (sint)lids.size())
				{
					...
					if (!Comm.db_names.empty())
					{
						std::vector <sint> cl_eles;

						for (size_t c = 0; c < Comm.db_names.size(); c++)
						{
							if (!wos[c])
							{
								cl_eles.push_back(lids[c]);
							}
						}
						odb = CloseDBs(&cl_eles, &fwas);
					}
				}
				else
				{
	...
	sint VerifyDatabases()
	{
		...
		{
			sint of = -1;
			sint xc = OpenFile("", nms[x].c_str(), ".db", 3, &of, true);
		...
		if (!db_failed)
		{
			...
			{
				BOT_DB dbs;
				sint xc = GetVecEle(&dbs, MTX_DBL, i, false); //we could get the whole list before loop, but other threads could edit elements during iteration
	...
	sint VerifyDatabase(BOT_DB *dbs_ = 0)
	{
		...
		sint fdb = -1;
		sint hdb = -1;
		sint odb = OpenDB(dbs_->name.c_str(), 2, &hdb, &fdb);
		sint xc = 0;
		...
		if (odb > -1)
		{
			...
			for (sint cc = 0; cc < (sint)dbs_->tables.size(); cc++)
			{
				bool began = false;
	...
	sint GetSymbols(std::vector <MICSYM> *syms_ = 0, char x_cond_[] = 0, bool maint = false)
	{
		...
		sint odb = OpenDB("litebot", 1, &hdb, &fdb);
	...
	sint GetCode(std::vector<MACSYM>* codes_ = 0, char x_cond_[] = 0, bool maint = false)
	{
		...
		sint fdb = -1;
		sint hdb = -1;
		sint odb = OpenDB("code", 1, &hdb, &fdb);
	...
	sint GetLastAccess(BOT_DB *dbs_ = 0, sint *last_modi_ = 0, sint *last_modi_ns_ = 0, sint *last_maint_ = 0, sint *last_maint_ns_ = 0)
	{
		...
		if (dbs_->type > -1)
		{
			...
			sint fdb = -1;
			sint hdb = -1;
			sint odb = OpenDB(dbs_->name.c_str(), 1, &hdb, &fdb);
	...
	sint StopAllThreads()
	{
		...
		{
			sint nc = 2;
			sint xc = SetVecEleMem((void*)&nc, MTX_TTS, siz, BOT_TTS_FIN, false);
			siz--;
		}
		return 0;
	}
	...
	sint FinishThread(TINFO* tts_, sint* ret_)
	{
		...
		case 1:
		{
			...
			{
				TINFO tts;
				rc = GetVecEle(&tts, MTX_TTS, tts_->lid, false);
	...
	sint FinishThreads(std::vector<TINFO>* tts_, std::vector<sint>* rets_)
	{
		...
		while (!done)
		{
			...
			while (siz < tts_->size())
			{
				...
				TINFO tts;
				p = GetVecEle(&tts, MTX_TTS, tts_->at(siz).lid, false);
	...
	{
		sint of = -1;
		sint lx = OpenFile(BOT_MOD_PATH, "litebot", ".h", BOT_FILE_READ, &of, true);
		...
		{
			sint of = -1;
			lx = OpenFile(BOT_MOD_PATH, "litebot", ".cpp", BOT_FILE_READ, &of, true);
	...
	{
		sint of = -1;
		sint lx = OpenFile(BOT_MOD_PATH, "error122017", ".txt", BOT_FILE_READ, &of);
		...
		if (lx > -1 && lx < (sint)sizeof(f_con))
		{
			...
			lx = OpenFile(BOT_MOD_PATH, "vins122017", ".txt", BOT_FILE_APPEND, &of, true);
	...
	sint AddDlrCode(cchar* to_vin = "", cchar* in_file = "")
	{
		...
		sint of = -1;
		sint lx = OpenFile(BOT_MOD_PATH, in_file, ".txt", BOT_FILE_OVERW, &of);
