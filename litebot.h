...
private:
	...
	{
		std::time_t t_ = std::chrono::system_clock::to_time_t(*tim_);
		std::chrono::duration<sint, std::milli> ms;
		sint nsct = ms.count();
		struct tm t;
		errno_t bot_err = gmtime_s(&t, &t_);

		if (form)
		{
			carr_64 ncar;
			return bot_sprintf(ncar.carr, ncar.siz, "%s%i]", std::put_time(&t, "[%y/%m/%d][%H:%M:%S:"), nsct);
		}
		else
		{
			carr_64 ncar;
	...
	sint Output(cchar* output_ = "", ...)
	{
		...
		carr_64 tdata;
		sint xc = BotTimeP(tdata.carr, &t_);
		carr_256 instr;
