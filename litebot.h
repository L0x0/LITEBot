...
public:
	...
	void Renew(BOT_DB* val_ = 0) { Clear(); file_ele = val_->file_ele; lid = val_->lid; lok = val_->lok; type = val_->type; exists = val_->exists; c_by = val_->c_by; omode = val_->omode; name.append(val_->name.c_str()); for (uint x = 0; x < val_->tables.size(); x++) { tables.push_back(val_->tables[x]); } for (uint x = 0; x < val_->columns.size(); x++) { columns.push_back(val_->columns[x]); } }
	void Renew(cchar* nstr = "", sint ntype = -1, sint nomode = -1) { Clear(); omode = nomode; type = ntype; if (strlen(nstr) < 128) { name.append(nstr); } }

	BOT_DB(cchar* nstr = "", sint ntype = -1) { connected = false; file_ele = omode = lok = c_by = lid = exists = -1; type = ntype; if (strlen(nstr) < 128) { name.append(nstr); } };
...
public:
	...
	void Renew(cchar* nm_ = 0, cchar* nt_ = 0, cchar*np_ = 0, sint nc_by = -1, sint nomode = -1, sint nexists = -1)
	{
...
public:
	...
	BOT_FILE_M(cchar* nfn = "", cchar* nft = "", cchar* nfp = "", sint nc_by = -1, sint nomode = -1) { omode = nomode; lok = -1; size_t siz = strlen(nfn); if (siz && siz < BOT_FILENAME_MAX) { name.append(nfn); } siz = strlen(nft); if (siz && siz < 128) { type.append(nft); } siz = strlen(nfp); if (siz && siz < UINT64_MAX) { path.append(nfp); } strsiz = 0; exists = -1; c_by = nc_by; };
...
private:
	...
	sint UpdMTX(bool hasrlock = false)
	{
		bool clear = true;
	...
	sint WithdMTX(lok_req* nreq = 0, bool hasrlock = false)
	{
		if (!nreq)
		{
			return -1;
				...
				if (p > -1)
				{
					...
					if (!req)
					{
						...
						case MTX_CV:
						{
							if (!is_meta)
							{
								if (ele < (sint)bot_c_vec.d_vec.size())
								{
							...
							{
								if (ele < (sint)bot_c_vec.m_vec.size())
								{
						...
						case MTX_LOG:
						{
							if (!is_meta)
							{
								if (ele < (sint)bot_log_vec.d_vec.size())
								{
							...
							{
								if (ele < (sint)bot_log_vec.m_vec.size())
								{
						...
						case MTX_TTS:
						{
							if (!is_meta)
							{
								if (ele < (sint)tts_vec.d_vec.size())
								{
							...
							{
								if (ele < (sint)tts_vec.m_vec.size())
								{
						...
						case MTX_DBL:
						{
							if (!is_meta)
							{
								if (ele < (sint)bot_db_vec.d_vec.size())
								{
							...
							{
								if (ele < (sint)bot_db_vec.m_vec.size())
								{
						...
						case MTX_FO:
						{
							if (!is_meta)
							{
								if (ele < (sint)fileo_vec.d_vec.size())
								{
							...
							{
								if (ele < (sint)fileo_vec.m_vec.size())
								{
	...
	sint OpenObject(sint nobj = -1, sint nele = -1, sint* r_opt = 0, sint* n_opt = 0, ...)
	{
		if (nobj < 0)
		{
			return -1;
		...
		else
		{
			...
			case MTX_DBL:
			{
				...
				if (ret > -1)
				{
					ret = OpenDB(xdb.name.c_str(), opts[0], r_opt, n_opt);
				}
				break;
			}
			default:
	...
	sint CloseObject(sint nobj = -1, sint nele = -1, sint is_meta = -1, sint* opt = 0, sint* carry = 0)
	{
		if (nobj < 0)
		{
			return -1;
			...
			case MTX_DBL:
			{
				...
				if (ret > -1)
				{
					...
					if (strcmp(d_con_->name.c_str(), xdb.name.c_str()))
					{
						...
						if (ret > -1)
						{
							sint xret = CloseDB(xdb.name.c_str(), carry);
							ret = xret;
						}
					}
					else
	...
	sint GetVecEle(void* val_ = 0, sint g_opt = -1, sint ele = -1, bool cine = false, bool is_meta = true)
	{
		if (debug_lvl >= 800 && debug_m)
		{
			carr_256 outp;
	...
	sint ConMethod(BOT_FILE* val_ = 0)
	{
		if (!val_)
		{
			return -1;
	...
	sint BOTCreateFile(BOT_FILE* file_ = 0, sint* was_open = 0, sint ele = -1)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			char instr[128] = { 0 };
			sint oc = bot_sprintf(instr, 128, "::BOTCreateFile(BOT_FILE *file_(%i), sint* was_open(%i), sint ele(%i))", (sint)file_, (sint)was_open, ele);
			oc = Output(instr, 2);
		}
		if (!file_ || !was_open)
		{
		...
		case 1:
		{
			...
			if (ret > -1)
			{
				if (!*was_open)
				{
					ret = CloseDB(file_->name.c_str(), &xh);
				}
			}
			break;
		}
	...
	sint OpenFile(BOT_FILE* file_ = 0, sint *was_open = 0, bool cine = false)
	{
		...
		if (file_->omode < 0 || file_->name.empty())
		{
			return -9;
		}
		...
		sint xc = ConMethod(file_);
		xc = GetInVec(file_, MTX_FO, true, false);
		...
		if (xc)
		{
			if (cine)
			{
				xc = BOTCreateFile(file_, was_open, fc);
		...
		if (S_ISREG(file_->filestats.st_mode))
		{
			...
			case 1:
			{
				fc = OpenDB(file_->name.c_str(), file_->omode, was_open);
				break;
			}
			case 2:
			{
				...
				instr.append(file_->name.c_str());
				instr.append(file_->type.c_str());
				f_con[fc]->fstrm.open(instr.c_str(), file_->omode);
	...
	sint FileOut(BOT_FILE* file_ = 0, sint fcon = -1, sint as_opt = 0, ...)
	{
		if (!file_)
		{
			return -1;
	...
	sint CloseFile(BOT_FILE* file_ = 0, bool del = false, bool encr = false)
	{
		...
		{
			sint xh = 1;
			sint xc = CloseDB(file_->name.c_str(), &xh);
	...
	slint nsRest(slint i = BOT_NANO_REST, bool keep_mtx = false)
	{
		...
		if (!keep_mtx)
		{
			if (!relocks.empty())
			{
				for (uint x = 0; x < relocks.size(); x++)
				{
					sint cheq = OpenObject(relocks[x].gmtx, relocks[x].ele, &relocks[x].is_meta, &relocks[x].lid, relocks[x].fid, INT32_MIN); // returns is_meta as wasopen, uses value set in fid by CloseObject() as var arg option for openmode or is_meta
				}
			}
		}
	...
	sint GetDBConn(BOT_DB_M* val_ = 0)
	{
		...
		sint ret = HasDBOpen(val_->name.c_str(), val_->omode);
	...
	sint OpenDB(cchar *db_name_ = "", sint x = -1, sint* was_open = 0, sint* fwas_locked = 0)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_512 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::OpenDB(cchar* db_name_ \"%s\", sint x(%i), sint* was_open(%i), sint* fwas_locked(%i))", db_name_, x, (sint)was_open, (sint)fwas_locked);
			oc = Output(outp.carr, 2);
		}
		...
		if (gc < 0 && x == 3)
		{
			...
			gc = GetInVec((void*)&dbg, MTX_DBL, false);
		...
		if (fc < 0)
		{
			...
			fc = GetInVec(&nfilem, MTX_FO, false, true);
			...
			if (fwas_locked)
			{
				...
				{
					sint hc = -1;
					sint lx = LockElement(MTX_FO, nfilem.lid, &hc, false, true, false);
		...
		if (dbg.omode > -1)
		{
			...
			if (ct == UINT_MAX && dbg.omode < 0)
			{
				if (fwas_locked)
				{
					if (!*fwas_locked)
					{
						ct = UnlockElement(MTX_FO, nfilem.lid, false);
					}
				}
				if (debug_lvl >= 4 && debug_m)
				{
		...
		dbg.omode = x;
		sint xh = -1;
		sint lx = LockElement(MTX_DBL, dbg.lid, &xh, false, true, false);
			...
			if (plim > -1)
			{
				if (dbc > -1)
				{
					...
					else
					{
						...
						{
							carr_256 outp;
							sint op = bot_sprintf(outp.carr, outp.siz, "invalid open opt:%i for %s", x, dbt.c_str());
							op = Output(outp.carr, 2);
						}
						if (dbc > -1)
						{
		...
		if (gc > -1)
		{
			sint xc = SetVecEleMem((void*)&dbg.omode, MTX_DBL, dbg.lid, BOT_DB_OMODE, false);
			dbg.connected = true;
			xc = SetVecEleMem((void*)&dbg.connected, MTX_DBL, dbg.lid, BOT_DB_CONN, false);
		}
	...
	sint CloseDB(cchar* db_name_ = 0, sint* fwas_locked = 0)
	{
		if (debug_lvl >= 4 && debug_m)
		{
			carr_256 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::CloseDB(cchar* db_name_(%i), sint* fwas_locked(%i))", (sint)db_name_, (sint)fwas_locked);
			oc = Output(outp.carr, 2);
		}
		...
		BOT_DB dbg(db_name_);
		sint gc = GetInVec((void*)&dbg, MTX_DBL, false, false);
		...
		sint ex = HasDBOpen(db_name_, dbg.omode);
		...
		if (ex < 0)
		{
			...
			{
				carr_256 outp;
				sint oc = bot_sprintf(outp.carr, outp.siz, "DB \"%s\" connection(%i) not found.", db_name_, dbg.omode);
				oc = Output(outp.carr, 2);
			}
			return ex;
		}
			...
			else
			{
				...
				while (!done)
				{
					...
					case SQLITE_OK:
					{
						...
						BOT_FILE nfile;
						gc = GetVecEle(&nfile, MTX_FO, dbg.file_ele, false, false);
					...
					default:
					{
						if (debug_lvl >= 1 && debug_m)
						{
							gc = Output(GetError().c_str(), 1, 2, -1);
						}
						done = true;
						break;
					}
		...
		if (d_con_)
		{
			...
			{
				rc = CloseDB(d_con_->name.c_str(), fwas_locked);
			}
		}
		return rc;
	}

	sint OpenDBs(std::vector <std::string> *db_names_, sint opt = -1, std::vector <sint> *wos_ = 0, std::vector<sint> *fwas_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			if (debug_lvl >= 1 && debug_m)
			{
				carr_256 outp;
				sint oc = bot_sprintf(outp.carr, outp.siz, "::OpenDBs(std::vector <std::string> *db_names_(%i), sint opt(%i), std::vector <sint> *wos_(%i), std::vector<sint>* fwas_(%i))", (sint)db_names_, opt, (sint)wos_, (sint)fwas_);
				oc = Output(outp.carr, 2);
			}
		}
		if (!db_names_ || opt < 0 || !wos_ || !fwas_)
		{
			return -1;
		}
		if (db_names_->empty())
		...
		for (uint opens = 0; opens < db_names_->size(); opens++)
		{
			...
			else
			{
				fwas_->push_back(-1);
				wos_->push_back(-1);
				sint odb = OpenDB(db_names_->at(opens).c_str(), opt, &wos_->at(opens), &fwas_->at(opens));
	...
	sint CloseDBs(std::vector<std::string> *names_ = 0, std::vector<sint>* fwas_ = 0)
	{
		if (debug_lvl >= 1 && debug_m)
		{
			carr_256 outp;
			sint oc = bot_sprintf(outp.carr, outp.siz, "::CloseDBs(std::vector<sint>* names_(%i), std::vector<sint>* fwas_(%i))", (sint)names_, (sint)fwas_);
			oc = Output(outp.carr, 2);
		}
		...
		if (names_ && fwas_)
		{
			...
			{
				xc = CloseDB(names_->at(opens).c_str(), &fwas_->at(opens));
	...
	sint Commit(std::vector<BOT_STMT>* stmts_ = 0)
	{
		...
		while (!done)
		{
			...
			if (ox > -1)
			{
				...
				std::vector<sint>  wos;
				sint odb = OpenDBs(&Comm.db_names, Comm.opt, &wos, &fwas);
	...
	sint VerifyDatabases()
	{
		...
		if (!db_failed)
		{
			BOT_FILE dbf("litebot", ".db", "", lid, 3);
			sint of = -1;
			sint xc = OpenFile(&dbf, &of, true);
			...
			dbf.Renew("math", ".db", "", lid, 3);
			xc = OpenFile(&dbf, &of, true);
				...
				if (cc == dbs_->tables.size())
				{
					if (xc)
					{
						...
						else
						{
							if (!hdb)
							{
								odb = CloseDB(dbs_->name.c_str(), &fdb);
							}
							return 2;
						}
					}
	...
	sint AddDlrCode(cchar* to_vin = "", cchar* in_file = "")
	{
		...
		BOT_FILE nfile(in_file, ".txt", BOT_MOD_PATH, lid, BOT_FILE_OVERW);
		sint of = -1;
		sint lx = OpenFile(&nfile, &of);
