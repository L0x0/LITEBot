...
public:
	...
	void Renew(BOT_LOGDET_M* val_ = 0) { if (!val_) { return; } Clear(); logtitle.append(val_->logtitle.c_str()); startdate.append(val_->startdate.c_str()); lsiz = val_->lsiz; lok = val_->lok; type = val_->type; c_by = val_->c_by; lid = val_->lid; }
	void Renew(BOT_LOGDET* val_ = 0) { if (!val_) { return; } Clear(); logtitle.append(val_->logtitle.c_str()); startdate.append(val_->startdate.c_str()); lsiz = val_->log_q.size(); lok = val_->lok; type = val_->type; c_by = val_->c_by;lid = val_->lid; }
...
private:
	...
	sint LogPut(cchar* msg_ = "", sint option = -1)
	{
		...
		if (log_to[0])
		{
			...
			BOT_LOGDET_M Current_m(nm, dat, option);
