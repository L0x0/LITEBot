...
private:
	...
	sint OpenFile(BOT_FILE* file_ = 0, sint *was_open = 0, bool cine = false)
	{
		...
		xc = GetInVec(file_, MTX_FO, cine, false);
	...
	sint OpenDB(cchar *db_name_ = "", sint x = -1, sint* was_open = 0, sint* fwas_locked = 0)
	{
		...
		if (fc < 0)
		{
			...
			sint hc = -1;
			gc = LockElement(MTX_FO, nfilem.lid, &hc, false, true, false);
			...
			gc = SetVecEleMem(&nfilem.lid, MTX_DBL, dbg.lid, BOT_DB_FELE, false);
		}
