...
sint machine::BOTFileStats(BOT_FILE_M *file_, sint scan_lvl, bool doscan)
{
	if (debug_lvl >= 600 && debug_m)
	{
		carr_96 ncar;
		sint oc = bot_sprintf(ncar.carr, ncar.siz, "::BOTFileStats(BOT_FILE_M *file_(%i), sint scan_lvl(%i), bool doscan(%u))", (sint)file_, scan_lvl, (uint)doscan);
		oc = Output(ncar.carr, 2);
...
sint machine::BOTOpenFile(BOT_FILE_M* mfile_, sint *was_open, bool cine, bool r_to_st)
{
	...
	gc = BOTFileStats(mfile_, (sint)BOT_FSCAN_LVL, true);
