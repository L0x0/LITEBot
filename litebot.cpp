	...
	while (!done)
	{
		...
		for (size_t siz = 0; (sint)siz < LITEBot.VecSize(MTX_LOG); siz++)
		{
			...
			if (xc > -1)
			{
				if (logm.lsiz)
				{
					xc = LITEBot.LockElement(&logm, false, true);
...
{
	sint nlid = (sint)vp;
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, "ThreadMaintainence");
	TINFO_M tts;
...
void* machine::TestThread(void * vp_)
{
	...
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, ncar.carr);
	TINFO_M tts;
...
{
	sint nlid = (sint)vp;
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, "AddThisCode");
	TINFO_M tts;
	sint xc = LITEBot.GetVecEle((void*)&tts, MTX_TTS, LITEBot.lid);
...
{
	sint nlid = (sint)vp;
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, "DBMaintenance");
	TINFO_M tts;
...
{
	sint nlid = (sint)vp;
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, "LITEBotMaintenance");
	TINFO_M tts;
...
{
	sint nlid = (sint)vp;
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, nlid, "CodeMaintenance");
	TINFO_M tts;
	sint rc = LITEBot.GetVecEle((void*)&tts, MTX_TTS, LITEBot.lid);
