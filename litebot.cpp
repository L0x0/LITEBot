				...
				if (logm.lsiz)
				{
					...
					if (!rc)
					{
						...
						if (p > -1)
						{
							...
							pth.push_back('\\');
							BOT_FILE_M xfile(logm.logtitle.c_str(), ".txt", pth.c_str(), BOT_FILE_APPEND, 0, 0, -1, LITEBot.lid);
							sint ox = -1;
							rc = LITEBot.BOTOpenFile(&xfile, &ox, true);

							if (rc > -1)
							{
								sint op = LITEBot.BOTFileOUT(xfile.fcon, BOT_RTV_VSTR, &bot_log_vec.d_vec[siz].log_q, BOT_RTV_MAX);
