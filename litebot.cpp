...
{
	sint xc = (sint)vp_;
	machine LITEBot(0, 0, xc, "LogMaintenance", true, true);
				...
				if (logm.lsiz)
				{
					...
					if (!rc)
					{
						...
						if (p > -1)
						{
							...
							if (rc > -1)
							{
								sint op = LITEBot.BOTFileOUT(xfile.fcon, 3, &bot_log_vec.d_vec[siz].log_q, 0);
...
void* machine::ConsoleThread(void* vp_)
{
	...
	while (!done)
	{
		...
		rc = LITEBot.ThreadRest((slint)BOT_CNSREST, 1);

		if (rc == (sint)BOT_THRQ_CONS)
		{
			done = true;
		}
	}
