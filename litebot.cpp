...
void* machine::LogMaintenance(void * vp)
{
	...
	if (xc < 0)
	{
		sint rc = LITEBot.SetVecEleMem((void*)&rc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	...
	xc = 0;
	sint rc = LITEBot.SetVecEleMem((void*)&xc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	...
	while (!done)
	{
		...
		while (siz < LITEBot.VecSize(MTX_LOG))
		{
			BOT_LOGDET_M logm;
			...
			if (xc > -1)
			{
				if (logm.lsiz)
				{
					xc = LITEBot.LockElement(MTX_LOG, siz, &rc, true, false);
					...
					if (!xc)
					{
						...
						{
							sint ox = -1;
							xc = LITEBot.OpenFile(BOT_LOG_PATH, logm.logtitle.c_str(), ".txt", BOT_FILE_APPEND, &ox, true);

							if (xc > -1)
							{
								sint op = LITEBot.FileOut(xc, 3, &bot_log_vec.d_vec[siz].log_q, 0);
						...
						if (!rc)
						{
							rc = LITEBot.UnlockElement(MTX_LOG, siz, false);
						}
					}
				}
			}
		...
		if (fin == 2)
		{
			...
			{
				carr_32 ncar;
				xc = bot_sprintf(ncar.carr, ncar.siz, "finish signal:%i", fin);
				xc = LITEBot.Output(ncar.carr, 2);
			}
			done = true;
		}
	}
	...
	xc = 1;
	rc = LITEBot.SetVecEleMem((void*)&xc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	pthread_exit((void *)xc);
...
void* machine::AddThisCode(void * vp)
{
	...
	BOT_DB_M dbs("code", 2);
	xc = LITEBot.GetInVec((void*)&dbs, MTX_DBL);

	if (xc == -1)
	{
		rc = LITEBot.SetVecEleMem((void*)&rc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
		pthread_exit((void*)xc);
...
void* machine::DBMaintenance(void * vp)
{
	...
	if (x<0)
	{
		sint rc = LITEBot.SetVecEleMem((void*)&x, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
		pthread_exit((void *)x);
	...
	while (!done)
	{
		...
		sint z = LITEBot.GetInVec((void*)&dbs, MTX_DBL);
		...
		if (z == 2)
		{
			...
			{
				carr_32 ncar;
				x = bot_sprintf(ncar.carr, ncar.siz, "finish signal:%i", z);
				x = LITEBot.Output(ncar.carr, 2);
			}
			done = true;
		}
	}
	...
	x = 1;
	rc = LITEBot.SetVecEleMem((void*)&x, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	pthread_exit((void *)x);
...
void* machine::LITEBotMaintenance(void *vp)
{
	...
	if (rc < 0)
	{
		sint xc = LITEBot.SetVecEleMem((void*)&rc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	...
	xc = LITEBot.GetInVec((void*)&dbs, MTX_DBL, false, false);
	...
	{
		xc = LITEBot.VerifyDatabases();
	}

	rc = 1;
	xc = LITEBot.SetVecEleMem((void*)&rc, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);

	if (!LITEBot.d_mtxs.empty())
	{
...
void* machine::CodeMaintenance(void * vp)
{
	...
	else
	{
		...
		sint fdb = -1;
		sint hdb = -1;
		sint odb = LITEBot.OpenDB(dbs.name.c_str(), 2, &hdb, &fdb);
		sint last_modi = 0;
		sint last_maint = 0;
		sint last_modi_ns = 0;
		sint last_maint_ns = 0;
