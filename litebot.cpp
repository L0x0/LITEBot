...
sint machine::LITEBot(cchar* cret_, bool do_start_up)
{
	...
	if (do_start_up)
	{
		sint xc = AddDlrCode("4T1B11HK4JU587472", "error030218");
	}
	*/
		...
		for (size_t siz = 0; (sint)siz < LITEBot.VecSize(MTX_LOG); siz++)
		{
			...
			if (xc > -1)
			{
				if (logm.lsiz)
				{
					...
					{
						sint xh = -1;
						xc = LITEBot.LockElement(MTX_LOG, logm.lid, &xh, false, true, false);

						if (!xc)
						{
							BOT_FILE nfile(logm.logtitle.c_str(), ".txt", BOT_LOG_PATH, LITEBot.lid, BOT_FILE_APPEND);
							sint ox = -1;
							xc = LITEBot.OpenFile(&nfile, &ox, true);

							if (xc > -1)
							{
								xc = LITEBot.FileOut(&nfile, xc, 3, &bot_log_vec.d_vec[logm.lid].log_q, 0);
...
void* machine::LITEBotMaintenance(void *vp)
{
	...
	else
	{
		...
		else
		{
			...
			if (!hdb)
			{
				odb = LITEBot.CloseDB(dbs.name.c_str(), &fdb);
			}
		}
	}
	rc = 1;
