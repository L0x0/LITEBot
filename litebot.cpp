...
void* machine::LogMaintenance(void *vp)
{
	...
	{
		sint ret = -1;
		rc = LITEBot.SetVecEleMem((void*)&ret, MTX_TTS, LITEBot.lid, BOT_TTS_FIN, false);
	...
	while (!done)
	{
		...
		rc = LITEBot.ThreadRest(BOT_LOGREST);
...
void* machine::ThreadMaintenance(void *vp)
{
	...
	while (!done)
	{
		...
		rc = LITEBot.TM(0);
		rc = LITEBot.ThreadRest(BOT_THRREST);
...
void* machine::TestThread(void *vp_)
{
	...
	machine LITEBot(BOT_DEBUG_LVL, BOT_DEBUG_M, xc, ncar.carr, true, true);
...
void* machine::DBMaintenance(void * vp_)
{
	...
	while (!done)
	{
		...
		rc = LITEBot.ThreadRest(BOT_DBREST);
